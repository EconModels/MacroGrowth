...
ssh node-02 "cd $LOC_PATH; $EXEC -c US,UK,JP -e Q -m cese-\(kl\)e $1 &> $OUTDIR/node-02.txt" &
ssh node-03 "cd $LOC_PATH; $EXEC -c CN,ZA,SA -e Q -m cese-\(kl\)e $1 &> $OUTDIR/node-03.txt" &
ssh node-04 "cd $LOC_PATH; $EXEC -c IR,TZ,ZM -e Q -m cese-\(kl\)e $1 &> $OUTDIR/node-04.txt" &
...
ssh node-13 "cd $LOC_PATH; $EXEC -c IR,TZ,ZM -e Q -m cese-\(ek\)l $1 &> $OUTDIR/node-13.txt" &

# The next lines run the cese analyses for exergy (X)

ssh node-14 "cd $LOC_PATH; $EXEC -c US,UK,JP -e X -m cese-\(kl\)e $1 &> $OUTDIR/node-14.txt" &
ssh node-15 "cd $LOC_PATH; $EXEC -c CN,ZA,SA -e X -m cese-\(kl\)e $1 &> $OUTDIR/node-15.txt" &
ssh node-16 "cd $LOC_PATH; $EXEC -c IR,TZ,ZM -e X -m cese-\(kl\)e $1 &> $OUTDIR/node-16.txt" &
ssh node-17 "cd $LOC_PATH; $EXEC -c US,UK,JP -e X -m cese-\(le\)k $1 &> $OUTDIR/node-17.txt" &
ssh node-18 "cd $LOC_PATH; $EXEC -c CN,ZA,SA -e X -m cese-\(le\)k $1 &> $OUTDIR/node-18.txt" &
ssh node-19 "cd $LOC_PATH; $EXEC -c IR,TZ,ZM -e X -m cese-\(le\)k $1 &> $OUTDIR/node-19.txt" &
ssh node-20 "cd $LOC_PATH; $EXEC -c US,UK,JP -e X -m cese-\(ek\)l $1 &> $OUTDIR/node-20.txt" &
# Faulty R installations on nodes 21, 22, 23, and 24.
ssh node-25 "cd $LOC_PATH; $EXEC -c CN,ZA,SA -e X -m cese-\(ek\)l $1 &> $OUTDIR/node-25.txt" &
# Faulty R installation on node 26.
ssh node-27 "cd $LOC_PATH; $EXEC -c IR,TZ,ZM -e X -m cese-\(ek\)l $1 &> $OUTDIR/node-27.txt" &

# We have useful work (U) data for US, UK, and JP only.

ssh node-28 "cd $LOC_PATH; $EXEC -c US,UK,JP -e U -m cese-\(kl\)e $1 &> $OUTDIR/node-28.txt" &
ssh node-29 "cd $LOC_PATH; $EXEC -c US,UK,JP -e U -m cese-\(le\)k $1 &> $OUTDIR/node-29.txt" &
ssh node-30 "cd $LOC_PATH; $EXEC -c US,UK,JP -e U -m cese-\(ek\)l $1 &> $OUTDIR/node-30.txt" &
