...
Graphs comparing fitted and historical values can be created as shown below for the 2-factor 
Cobb-Douglas model fitted above.

```{r, fig.width = 6, fig.align = "center"}
bind_cols(EconUK, yhat(cdfit) %>% as.data.frame() %>% set_names("yhat")) %>% 
  ggplot() + 
  # Add historical data as data points
...
three-factor-of-production Cobb-Douglas model with constant returns to scale,
fitted output elasticies ($\alpha_i$) can be shown in a ternary plot.

```{r}
# This doesn't work. I'm probably using triPlot wrong.
