...
cdCoeffsData_ZA <- filter(coeffs_df_ZA, model=="cd", nestStr %in% c("iK+iL", "iK+iL+iQ"))
...
cdFittedData_ZA <- filter(fitted_df_ZA, model=="cd", nest %in% c("iK+iL", "iK+iL+iQ"))
...
cesCoeffsData_ZA <- filter(coeffs_df_ZA, model=="ces", nestStr %in% c("iK+iL", "iK+iL+iQ", "iL+iQ+iK", "iQ+iK+iL"))
standardScatterPlot(data=cesCoeffsData_ZA, mapping=aes(gamma_coef, lambda), facet_formula=country~nest, alpha=0.05) +
    scale_x_continuous(breaks=c(0.95, 1.0, 1.05)) +
    scale_y_continuous(breaks=c(0, 0.002, 0.004, 0.006, 0.008)) +    
    labs(x=expression(theta), y=expression(lambda))
@
...
standardScatterPlot(data=cesCoeffsData_ZA, mapping=aes(delta_1, sigma_1), facet_formula=country~nest, alpha=0.05) +
    coord_trans(y="sigma") +
...
standardTriPlot(data=cesCoeffsData_ZA, facet_formula=country~nest)
@

<<CES_Spaghetti_Graph, fig.pos="H", fig.align='center', fig.width=5, fig.height=3, fig.cap="CES fitted models with various nestings.">>=
cesFittedData_ZA <- filter(fitted_df_ZA, model=="ces", nest %in% c("iK+iL", "iK+iL+iQ", "iL+iQ+iK", "iQ+iK+iL"))
spaghettiPlot(data=cesFittedData_ZA, facet_formula=Country~nest, level=spaghettiGraphLevel) +
    labs(y="Indexed GDP (1991=1)")
@
...
linexCoeffsData_ZA <- filter(coeffs_df_ZA, model=="linex", energy=="iQ")
...
linexFittedData_ZA <- filter(fitted_df_ZA, model=="linex", energy=="iQ")
