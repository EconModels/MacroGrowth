...
historicalData <- subset(AllHistData, subset=Source=="Calvin2011" & Country=="ZA") # From the EconData package
...
cdFittedData_ZA <- filter(fitted_df_ZA, model=="cd", nestStr %in% c("iK+iL", "iK+iL+iQ"))
...
cesFittedData_ZA <- filter(fitted_df_ZA, model=="ces", nestStr %in% c("iK+iL", "iK+iL+iQ", "iL+iQ+iK", "iQ+iK+iL"))
