...
##
cdeGridGraphs <- function(countryAbbrev, energyType, textScaling=1.0){
  ...
  graph <- xyplot(value ~ gamma | variable, data=data_molten,
                  # 1=lambda, 2=alpha, 3=SSE, 4=beta
                  index.cond=list(c(3,1,2,4)), #orders the panels as SSE, lambda, alpha, beta.
                  strip = strip.custom(factor.levels=c("$\\lambda$", "$\\alpha$", "$SSE$", "$\\beta$"), 
                                       bg="transparent", # Sets background transparent to match the graph itself.
                                       par.strip.text=list(cex=textScaling) # Scales text in the strip.
                                       ),
                  as.table=TRUE, #indexing of panels starts in upper left and goes across rows.
                  ...
                  scales=list(cex=scaleTextSize * textScaling, #controls text size on scales.
                              ...
                              x=list(relation="free",
                                     at=alphaBetaGammaTics),
                              y=list(relation="free",  #allows each axis to be different
                                     #specifies location for tics
                                     at=list(lambdaTics, alphaBetaGammaTics, sseTics, alphaBetaGammaTics))
                              ), 
                  ylim=list(lambdaRange, alphaBetaGammaRange, sseRange, alphaBetaGammaRange), #y axis limits
                  #axis labels and scaling
