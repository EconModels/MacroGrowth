...
createSFParamsGraph <- function(factor){
  ...
  graph <- segplot(country ~ upperCI + lowerCI | parameter, 
                   ...
                   strip = strip.custom(bg="transparent", factor.levels=c("$\\lambda$", exponentParameter)),
...
#   factor: the factor of production you want to use in the single-factor model.
##
sf3DSSEGraph <- function(countryAbbrev, factor){
  # Generate the data in the form of a matrix with gamma in columns, lambda in rows, and SSE in the middle
  # First, make a data.frame with which we want to make predictions
  lambdaSeq <- seq(0.0, 0.05, by=0.01)
  mSeq <- seq(0.0, 1.0, by=0.1)

  lambdaSeq <- data.frame(lambdaSeq)
  nRowLambda <- nrow(lambdaSeq)
  colnames(lambdaSeq) <- "lambda"

  mSeq <- data.frame(mSeq)
  nRowM <- nrow(mSeq)
  colnames(mSeq) <- "m"
  
  # Make a sequence of lambda values based on the number of m values you have.
  lambda <- rep(lambdaSeq[1,1], nRowM)
  lambda <- data.frame(lambda)
  colnames(lambda) <- "lambda"
  for (i in 2:nRowLambda){
    lambdaTemp <- rep(lambdaSeq[i, 1], nRowM)
    lambdaTemp <- data.frame(lambdaTemp)
    colnames(lambdaTemp) <- "lambda"
    lambda <- rbind(lambda, lambdaTemp)
  }

  # Make repeats of the m data frame as we have number of lambda rows
  m <- mSeq
  for (i in 2:nRowLambda){
    m <- rbind(m, mSeq)
  }
  
  # Now create the data.frame of new observation locations
  newX <- cbind(lambda, m)
  ...
  sse <- c(1:(nRowLambda*nRowM)) # Build a bogus vector at the start
  
  # Loop over all the different combinations of lambda and m that are of interest for us.
  #i = 1
  for (i in 1:(nRowLambda*nRowM)){
  ...
  dataFor3DGraph <- cbind(newX, ssedf)
  
  # Adjust settings for a nicer graph
...
cdeGridGraph <- function(countryAbbrev, energyType, keyXLoc = defaultKeyXLoc, keyYLoc = defaultKeyYLoc, textScaling = 1.0){
  ...
  abgGraph <- xyplot(alpha+beta+gamma ~ gamma, data=data,
                     ...
                     lty = lineTypes, lwd = lineWidths, col = colors, #Controls line parameters
                     pch = symbols, col.symbol = colors, #Controls symbol parameters
                     key=list(text=list(c("$\\alpha$", "$\\beta$", "$\\gamma$", "$SSE$")),
                              type=graphType,
                              cex=keyTextSize * textScaling, #controls size of text in the key
                              lines=list(lty=lineTypes, lwd=lineWidths), #controls line types
                              pch=symbols, col=colors, #controls symbol (plot characters) types
                     ...
                     ylab=list(label="$\\lambda$, $\\alpha$, $\\beta$", cex=textScaling)
  )
...
partialResidualPlot <- function(countryAbbrev, energyType, textScaling=1.0){
  ...
  plot <- xyplot(cdResid ~ iEToFit, data=data,
                 ...
                 ylab=list(label="$y$ residuals", cex=textScaling)
                 )
  return(plot)  
}
@
