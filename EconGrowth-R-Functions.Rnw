...
CIvsParamPlot <- function(model, param, energyType=NA, factor=NA, textScaling=1.0){
  ...
  else if (model == "CESe"){data <- cesParamsTableDF(energyType)}
...
# 
partialResidualPlot <- function(modelType, countryAbbrev, energyType, textScaling=1.0){
...
partialResidualLattice <- function(modelType, energyType, textScaling=1.0){
  data <- do.call("rbind", lapply(countryAbbrevs, calcResiduals, modelType=modelType, energyType=energyType))
print(data)
