...
#' @param rho,rho1 creates a grid upon which searches will be made.
#' Note that the default arguments for both \code{rho} and \code{rho1} grids 
#' include the value 0.25. 
#' 0.25 is the starting value for \code{rho} and \code{rho1} used by \code{cesEst} for gradient searches
#' when values of \code{rho} and \code{rho1} are not specified.
#' So, if you accept the default arguments, there is no need to do an additional 
...
#' Extract information from cesModel objects for texreg
#' 
#' This function extracts information from a cesModel for texreg.
#' @param model a \code{cesModel} object from which data is to be extracted.
#' @return a \code{texreg} object ready for further processing.
#' @examples
#' if (require("EconData") & require("dplyr")) {
#'   extract(cesModel(iGDP ~ iK + iL + iQp + iYear, data = filter(Calvin, Country=="US"), nest = c(1,2,3)))
#' }
#' @export
extract.cesModel <- function(model){
  s <- summary(model)
  coeffs <- coef(s)
...
setMethod("extract", signature = className("cesModel", "EconModels"), definition = extract.cesModel)