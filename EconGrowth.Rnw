...
<<AIC_Analysis>>=
###############################
# Creates a xtable object that holds the AIC values for each parameter estimation that we include.
##
createAICTable <- function(){
  ...
  out <- rbind(aicCD, aicCDq, aicCDx) 
  # Create the xtable with caption and label
  out <- xtable(out, 
                caption="AIC values for all models.", 
                label="tab:AICTable", 
                digits=1 #Only one number after the decimal place
                )
  return(out)
}
@
