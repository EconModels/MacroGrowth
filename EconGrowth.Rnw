...
\noindent where $e$ is any of $q$, $x$, or $u$; $\gamma_E$, $\lambda_L$, $\lambda_E$, $\beta$, and $\sigma$ are fitting parameters; and $\mathrm{e}$ is the base of the natural logarithm. Ths sum $\alpha + \beta$ is constrained to unity.
