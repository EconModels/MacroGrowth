...
cobbDouglasModel <- function(countryAbbrev, energyType, gamma){
  ...
  if (missing(energyType)){
    ...
    if (countryAbbrev == "ZA"){
      ...
      gammaGuess <- 0.65 # Found via grid search over gamma
...
#       betaGuess <- 0.41
    ...
    if (missing(gamma)){
      # gamma is a free parameter. We need to estimate lambda, a, and b.
      ...
      # We don't want to estimate the value of gamma
...
data <- cbind(lambda, alpha, gammaGrid, SSE)
print(data)
plot(gammaGrid, SSE)
plot(gammaGrid, lambda)
plot(gammaGrid, alpha)

# Some code to debug the Cobb-Douglas models. Randy Pruim is helping on this.
