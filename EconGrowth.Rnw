...
createFactorsLatticeGraph <- function(countryName){
  dataTable <- loadData(countryName)
  graphType <- "l"
  lineTypes <- c(1, 5, 2, 4, 3) #line types. See http://en.wikibooks.org/wiki/R_Programming/Graphics
  lineWidths <- c(2, 2, 1, 1, 1) #line widths
  colors <- c("black", "black", "red", "blue", "black") #line and point colors
  lineSpec <- list(lty=lineTypes, lwd=lineWidths, col=colors)
...
cobbDouglasParamsTable <- function(){
  ...
  tableCD <- xtable(dataCD, 
                    caption="Cobb-Douglas parameters for 1980-2011 (developed nations) or 1991-2011 (others). (Parameter estimates beneath symbol. 95\\% confidence bounds to left and right.)", 
