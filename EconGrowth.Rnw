...
#   This may be any of heat (iQ), exergy (iX), or useful work (iU).
##
cobbDouglasEnergy <- function(dataTable, energyColumnName, energyString){
  ...
  # * beta = b - a
  # * gamma = 1 - max(a, b)
  iEnergy <- dataTable[energyColumnName] #grabs the energy column
  lambdaGuess <- 0.0 # guessing lambda = 0 means there is no technological progress.
...
<<cdq>>=
dataCDq <- cobbDouglasEnergy(dataTable, "iQ", "q")
...
<<cdx>>=
dataCDx <- cobbDouglasEnergy(dataTable, "iX", "x")
tableCDx <- xtable(dataCDx, caption="U.S., 1980-2011.", digit = c(4, 4, 2, 2, 3))
@
...
<<cdu>>=
dataCDu <- cobbDouglasEnergy(dataTable, "iU", "U")
tableCDu <- xtable(dataCDu, caption="U.S., 1980-2011.", digit = c(4, 4, 2, 2, 3))
@
