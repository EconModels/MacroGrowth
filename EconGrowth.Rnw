...
\usepackage{amsmath} %Allows \begin{equation*} for unnumbered equations
...
<<nlmrt tests, eval=FALSE>>=
# From https://stat.ethz.ch/pipermail/r-help/2012-November/330554.html
Laptop_sale <- c(1405,1863,2027,2669,2938,5275,6595,6943,8621,10905,12420,22400,32380,31600,34900,43163,47838,47592)
CuSale <- Laptop_sale
time <- seq_along(Laptop_sale)
Bass.Model <- Laptop_sale ~ m * ((p + q)^2/p) * (exp(-(p + q) * time)/((q /p) * exp(-(p + q) * time) + 1)^2)
# Bass.Fit <- nls(formula = Bass.Model, start = c(p = 0.03, q = 0.4, m = max(CuSale)), trace = FALSE)
# Bass.Fit
library(nlmrt)
...
print("")
print("After nlxb fit:")
Bass.Fitn
print(summary(Bass.Fitn))

Bass.Fitn <- wrapnls(formula = Bass.Model, start = c(p = 0.03, q = 0.4, m = max(CuSale)), data=xdata,trace=FALSE)
print("")
print("After wrapnls fit:")
print(summary(Bass.Fitn))
@
...
cesModel <- function(countryAbbrev, energyType){
  ...
  # to "iEToFit" and use "iEToFit" in the nls function.
  dataTable <- replaceColName(dataTable, energyType, "iEToFit")
  # Establish default guesses for sigma, gamma_E, beta, lambda_L and lambda_E.
  start <- list(sigma=0.9, gamma_E=0.01, beta=0.7, lambda_L=0.011, lambda_E=0.018) #values for Sweden
  # Establish bounds
  lower = list(sigma=-Inf, gamma_E=0.0, beta=-Inf, lambda_L=-Inf, lambda_E=-Inf)
  upper = list(sigma= Inf, gamma_E=1.0, beta= Inf, lambda_L= Inf, lambda_E= Inf)
  # Now some country-specific values.
  if (countryAbbrev == "US"){
    start <- list(sigma=0.95, gamma_E=0.1, beta=0.7, lambda_L=0.015, lambda_E=0.015) #values for Sweden
  }
  if (countryAbbrev == "UK"){
    start <- list(sigma=0.95, gamma_E=0.2, beta=0.7, lambda_L=0.02, lambda_E=0.02) #values for Sweden
    lower = list(sigma=-Inf, gamma_E=0.0, beta=-Inf, lambda_L=-0.3, lambda_E=-0.3)
    upper = list(sigma= Inf, gamma_E=1.0, beta= Inf, lambda_L= 0.3, lambda_E= 0.3)
  }
  if (countryAbbrev == "CN"){
    
  }
  # Runs a non-linear least squares fit to the data with constraints
#   model <- iGDP ~ ((1-gamma_E)^(1.0/sigma) * ((exp(lambda_L*iYear))^beta 
#                                                   * iLabor^beta
#                                                   * iCapStk^(1-beta) )^((sigma-1.0)/sigma) 
#                   + gamma_E^(1.0/sigma)*(exp(lambda_E*iYear) * iEToFit)^((sigma-1.0)/sigma))^(sigma/(sigma-1.0))
  model <- iGDP ~ ((1-gamma_E) * ((exp(lambda_L*iYear))^beta * iLabor^beta * iCapStk^(1-beta) )^((sigma-1.0)/sigma) 
                  + gamma_E * (exp(lambda_E*iYear) * iEToFit)^((sigma-1.0)/sigma))^(sigma/(sigma-1.0))
  modelCES <- nls(formula=model, data=dataTable, start=start,
                  control=nls.control(maxiter=10000, tol=1e-05, minFactor=1/1024, 
...
#   modelCES <- nlxb(model,
#                    data=dataTable,
#                    start=list(sigma=sigmaGuess, gamma_E=gamma_EGuess, beta=betaGuess, 
#                               lambda_L=lambda_LGuess, lambda_E=lambda_EGuess),
#                    trace=TRUE
#                   )
  return(modelCES)
}
print("US Q")
print(summary(cesModel("US", "Q")))

print("")
print("UK Q")
print(summary(cesModel("UK", "Q")))

print("")
print("JP Q")
print(summary(cesModel("JP", "Q")))

print("")
print("CN Q")
print(summary(cesModel("CN", "Q")))

print("")
print("ZA Q")
print(summary(cesModel("ZA", "Q")))

print("")
print("SA Q")
print(summary(cesModel("SA", "Q")))

print("")
print("IR Q")
print(summary(cesModel("IR", "Q")))

print("")
print("TZ Q")
print(summary(cesModel("TZ", "Q")))

print("")
print("ZM Q")
print(summary(cesModel("ZM", "Q")))
...
\citet{Stern:2012ey} developed a CES model of the form

\begin{equation} \label{eq:CES_model}
...
\noindent where $e$ is any of $q$, $x$, or $u$; $\sigma$, $\gamma_E$, $\beta$, $\lambda_L$, and $\lambda_E$ are fitting parameters; $\mathrm{e}$ is the base of the natural logarithm; and the sum $\alpha + \beta$ is constrained to unity.\footnote{The CES equation in \citet{Stern:2012ey} appears as
...
but the $1/\sigma$ exponent on the $(1-\gamma_E)$ and $\gamma_E$ terms seems erroneous due to $y(t_\mathrm{0}) \neq 1$.} 
...
Figure \ref{fig:LINEX_GDP_Lattice_Graph} compares LINEX model predictions with historical data.
