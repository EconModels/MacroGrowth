...
<<Country Factors Graph, echo=FALSE>>=
...
@
<<Factors_Lattice_Graph, fig.width=6.5, fig.height=7.5, fig.cap="GDP ($y$), capital stock ($k$), labor ($l$), thermal energy ($q$), exergy ($x$), and useful work ($u$) for all economies. (China's indexed GDP and indexed capital stock rise to $y = 7.3$ and $k = 9.2$ in 2011.)", echo=FALSE>>=
createFactorsLatticeGraph <- function(countryName){
  ...
  colors <- c("black", "black", "black", "red", "blue", "black") #line and point colors
  symbols <- c(1, NA_integer_, NA_integer_, NA_integer_, NA_integer_, NA_integer_)  #NA_integer_ gives no symbol.
  graph <- xyplot(iGDP+iCapStk+iLabor+iQ+iX+iU ~ Year | Country, data=dataTable,
                  ...
                  layout=c(3,3), #indicates a 3x3 arrangement of panels.
                  ...
                  key=list(text=list(c("y", "k", "l", "q", "x", "u")),
                           type=graphType,
                           cex=0.85, #controls size of text in the key
                           lines=list(lty=lineTypes, lwd=lineWidths), #controls line types
                           pch=symbols, col=colors, #controls symbol (plot characters) types
                           columns=1, x=0.01, y=0.95), #We want only one column and sets location of the key
                  scales=list(cex=0.75, #controls text size on scales.
                              tck=-0.5, #controls tick mark length. < 0 for inside the graph.
...
# that corresponds to the AllData.txt file.
##
cobbDouglasPredictionsColumn <- function(){
  preds <- lapply(countries, cobbDouglasPredictions)
...
<<CD Lattice Graph>>=
usModel <- cobbDouglasModel("US")
coefs <- coef(usModel)
# print(coef(usModel))
usPred <- predict(usModel) #See http://stackoverflow.com/questions/9918807/how-get-plot-from-nls-in-r
class(usPred)
#print(usPred)
data.frame(usPred)

allData <- loadData("All")
predictions <- cobbDouglasPredictionsColumn()
allData <- cbind(allData, predictions)
