...
# by including the gamma parameter, a fit with fixed gamma will be provided.
##
cobbDouglasModel <- function(countryAbbrev, energyType, gamma){
  ...
  if (missing(energyType)){
    ...
    if (countryAbbrev == "ZA"){
      # South Africa doesn't converge using the default guess values above. Try different values.
      alphaGuess <- 0.597 # The alpha value when energy is not included
      betaGuess <- 1.0 - alphaGuess
#       alphaGuess <- 0.4
#       gammaGuess <- 0.1
#       betaGuess <- 1.0 - alphaGuess - gammaGuess
#       alphaGuess <- 0.39
#       betaGuess <- 0.41
    }
    aGuess <- alphaGuess
    ...
    if (missing(gamma)){
      # gamma is a free parameter that we want to estimate
      ...
      # We don't want to estimat the value of gamma
...
# print(summary(modelCDe))
    return(modelCDe)
  }
}

###############################
# Does a grid search over values of gamma for a CD production function
##
cdGrid <- function(countryAbbrev, energyType, gammaGrid){
  gamma <- c(0.1, 0.2)
  gamma <- gammaGrid
...
print("US")
gammaGrid <- seq(0.05, 0.95, by=0.1)
models <- cdGrid("US", "Q", gammaGrid)
SSE <- lapply(models, function(model) summary(model)$sigma)
data <- cbind(gammaGrid, SSE)
plot(gammaGrid, SSE)
print(coef(models[[2]]))

print("ZA")
models <- cdGrid("ZA", "Q", gammaGrid)
SSE <- lapply(models, function(model) summary(model)$sigma)
print(str(coef(models[[5]])))
lambda <- lapply(models, function(model) coef(model)["lambda"])
alpha <- lapply(models, function(model) coef(model)["alpha"])
data <- cbind(gammaGrid, lambda, alpha, SSE)
plot(gammaGrid, SSE)
plot(gammaGrid, lambda)
plot(gammaGrid, alpha)
print(coef(models[[5]]))

# Some code that is helping to debug the Cobb-Douglas models. Randy Pruim is helping on this.

# print("cobbDouglasModel(US, Q) works fine:")
# us <- cobbDouglasModel("US", "Q")
# print(summary(us))
# print(confint(us))

# print("cobbDouglasModel(ZA) works fine")
# za <- cobbDouglasModel("ZA")
# print(summary(za))
# print(confint(za))

# print("cobbDouglasModel(ZA, Q) stops before convergence achieved,")
# print("and confint gives NA values:")
# za <- cobbDouglasModel("ZA", "Q")
# print(summary(za))
# print(confint(za))
# 
# print("cobbDouglasModel(UK, Q) also stops before convergence achieved,")
# print("but confint does not produce NA values:")
# uk <- cobbDouglasModel("UK", "Q")
# print(summary(uk))
# print(confint(uk))

# save(us, za, uk, file="HeunModels-US-ZA-UK.Rdata")
...
cesParamsTableA <- function(energyType){
  ...
  tableCESa <- xtable(dataCES[,c(1,2,3, 7,8,9, 10,11,12)], #Picks up gamma, delta_1, and delta
                      caption=paste("CES model (with $", tolower(energyType), "$). $\\gamma$, $\\delta_1$, and $\\delta$ parameters for 1980-2011 (US, UK, JP), 1991-2010 (CN and ZA), and 1991-2011 (SA, IR, TZ, and ZM). (Parameter estimates beneath symbol. 95\\% confidence interval bounds to left and right.)", sep=""), 
                      label=paste("tab:CES_Parameters_With_", energyType, sep=""),
...
cesParamsTableB <- function(energyType){
  ...
  colnames(dataCES) <- c(" ", "$\\gamma$",    " ", 
                         ...
                         " ", "$\\sigma$",    " ")
  rownames(dataCES) <- countryAbbrevs
  tableCESb <- xtable(dataCES[,c(4,5,6, 13,14,15, 16,17,18)], #Picks up lambda, sigma_1, and sigma
                      caption=paste("CES model (with $", tolower(energyType), "$). $\\lambda$, $\\sigma_1$, and $\\sigma$ parameters for 1980-2011 (US, UK, JP), 1991-2010 (CN and ZA), and 1991-2011 (SA, IR, TZ, and ZM). (Parameter estimates beneath symbol. 95\\% confidence interval bounds to left and right.)", sep=""), 
                      label=paste("tab:CES_Parameters_With_", energyType, sep=""),
                      digits = c(1, 4,4,4, 2,2,2, 2,2,2),
                      align = "r|ccc|ccc|ccc"  #Sets alignment of the numbers in the columns
                      )
...
createCESParamsGraph <- function(energyType){
  ...
  graph <- segplot(country ~ upperCI + lowerCI | parameter, 
                   ...
                   #set labels and bg color in strip
                   strip = strip.custom(factor.levels=c("$\\gamma$", "$\\lambda$", "$\\delta_1$",
                                                        "$\\delta$", "$\\sigma_1$", "$\\sigma$"), bg="white"),
                   col = "black", #Sets line color to black
                   lwd = 1, #Sets line width to 1.0
                   ylim = list(c(0.9, 1.1), c(-0.05, 0.05), c(0.0, 1.0),
                               c(0.0, 1.0), c(0.0, 1.0), c(0.0, 1.0)), #y axis limits
                   scales = list(cex=scaleTextSize, #controls text size on scales.
                                 tck=scaleTickSize, #controls tick mark length. < 0 for inside the graph.
                                 x=list(cex=0.75), #reduces text size so that country abbrevs are legible
                                 y=list(relation="free", #allow each axis to be different
                                        at=list(c(0.9, 1.0, 1.1), c(-0.05, 0.0, 0.05),
...
\begin{equation} \label{eq:micEconCES_model}
  y = \gamma \mathrm{e}^{\lambda(t-t_0)}  
      \left\{\delta \left[\delta_1 k^{-\rho_1} + (1-\delta_1)l^{-\rho_1} \right] + (1-\delta) e^{-\rho} \right\}
                                                                                                        ^{-1/\rho}.
\end{equation}
