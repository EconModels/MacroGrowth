...
createCountryFactorsGraph <- function(countryName){
  ...
  lineSpec <- list(lty=c(1, 5, 2, 4), #line type
                   col=c("black", "black", "black", "black"), #line colors
                   lwd=c(1, 1, 1, 1)) #line widths
  graph <- xyplot(iCapStk+iLabor+iQ+iX ~ iYear, data=dataTable,
                  key=list(text=list(c("k", "l", "q", "x")),
                           type="l",
                           lines=lineSpec,
...
createGDPComparisonGraph <- function(countryName){
  ...
  lineSpec <- list(lty=c(1, 1, 5, 2, 4), #line type
                   col=colors,
                   lwd=c(0, 1, 1, 1, 1)) #line widths. 0 suppresses the line for historical GDP.
