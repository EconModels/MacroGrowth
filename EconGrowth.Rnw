...
#
# countryName = the name of the country for which data is to be loaded. The file names is assumed to be 
#   named <countryName>Data.txt. The file is assumed to exist in a subfolder of this project called "data".
#
...
cobbDouglas <- function(countryName){
  ...
  dataCD <- rbind(upperCD, estCD, lowerCD); dataCD
...
tableCD <- cobbDouglas("US")

countries <- c("US", "CN")
tablesCD <- apply(countries, 1, cobbDouglas)
@

<<CD Results, results='asis'>>=
print(tableCD, caption.placement="top")
...
# energyColumnName is the name of the column to be used for energy.
#   This may be any of heat ("iQ"), exergy ("iX"), or useful work ("iU").
# energyString is the string to be used in table captions reprsenting the type of energy. Typically, "q", "x", or "u"
##
cobbDouglasEnergy <- function(countryName, energyColumnName, energyString){
  ...
  ciCDe <- confint(modelCDe, level = 0.95)
  print(ciCDe)
  ...
  row.names(estCDe) <- paste("C-D with ", energyString, sep="")
  # The [1] subscripts pick off the lower confidence interval
...
<<cdq>>=
countryName <- "US"
energyColumn <- "iQ"; energyString <- "q"
dataCDq <- cobbDouglasEnergy(countryName, energyColumn, energyString)
# TODO Develop a better caption for the table based on the data automatically.
tableCDq <- xtable(dataCDq, caption=paste(countryName, ", 1980-2011.", sep=""), digit = c(4, 4, 2, 2, 3))
#estCDe <- dataCDe[2]
#dataAll <- rbind(estCD, estCDe); dataAll
#tableAll <- xtable(dataAll, caption="U.S. 1980-2011, All Models.", digit = c(4, 4, 2, 2, 3))
@

<<CDq Results, results='asis'>>=

print(tableCDq, caption.placement="top")
# According to http://cran.r-project.org/web/packages/xtable/vignettes/xtableGallery.pdf, Section 3.1, I should 
# be able to use the "sanitize.text.function" parameter to allow markup in column headers. But this next
# line is not working at the present time. --MKH, 18 Jan 2012.
...
<<cdx>>=
energyColumn <- "iX"; energyString <- "x"
dataCDx <- cobbDouglasEnergy(countryName, energyColumn, energyString)
tableCDx <- xtable(dataCDx, caption=paste(countryName, ", 1980-2011.", sep=""), digit = c(4, 4, 2, 2, 3))
@

<<CDx Results, results='asis'>>=
print(tableCDx, caption.placement="top")
@

\subsection{Cobb-Douglas With U}

<<cdu>>=
energyColumn <- "iU"; energySTring <- "u"
dataCDu <- cobbDouglasEnergy(countryName, energyColumn, energyString)
tableCDu <- xtable(dataCDu, caption=paste(countryName, ", 1980-2011.", sep=""), digit = c(4, 4, 2, 2, 3))
@

<<CDu Results, results='asis'>>=
print(tableCDu, caption.placement="top")
@
