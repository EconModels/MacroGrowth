...
Bass.Fitn <- nlxb(formula = Bass.Model, start = c(p = 0.03, q = 0.4, m = max(CuSale)), data=xdata,trace=FALSE)
print("")
...
cesModel <- function(countryAbbrev, energyType){
  ...
  sigmaGuess <- 0.663 # 0.663 for Sweden
  gamma_EGuess <- 0.01 # Minimum contribution by energy is the default.
  betaGuess <- 0.7 # 0.7 for Sweden
  lambda_LGuess <- 0.011 # 0.011 for Sweden
  lambda_EGuess <- 0.018 # 0.018 for Sweden
  ...
  if (countryAbbrev == "US"){
    gamma_EGuess <- 0.147 # 0.147 for Sweden
  }
  if (countryAbbrev == "UK"){
    sigmaGuess <- 0.5
    gamma_EGuess <- 0.147 # 0.147 for Sweden
...
#   modelCES <- nls(iGDP ~ ((1-gamma_E)^(1.0/sigma) * ((exp(lambda_L*iYear))^beta 
#                                                   * iLabor^beta
#                                                   * iCapStk^(1-beta) )^((sigma-1.0)/sigma) 
#                           + gamma_E^(1.0/sigma)*(exp(lambda_E*iYear) 
#                                                   * iEToFit)^((sigma-1.0)/sigma))^(sigma/(sigma-1.0)), 
#                   data=dataTable,
...
\citet{Stern:2012ey} developed a CES model of the following form
...
\noindent where $e$ is any of $q$, $x$, or $u$; $\sigma$, $\gamma_E$, $\beta$, $\lambda_L$, and $\lambda_E$ are fitting parameters; and $\mathrm{e}$ is the base of the natural logarithm. Ths sum $\alpha + \beta$ is constrained to unity.
