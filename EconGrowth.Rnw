...
cobbDouglasParamsTableWithEnergy <- function(energyType){
  ...
  rownames(dataCD) <- countryAbbrevs
  tableCD <- xtable(dataCD, 
                    caption=paste("Cobb-Douglas (with $", tolower(energyType), "$) for 1980-2011 (US, UK, JP) or 1991-2011 (others). (Parameter estimates beneath symbol. 95\\% confidence interval bounds to left and right.)", sep=""), 
...
#                     index.cond = list(2,3,1), #orders the panels as alpha, beta, lambda, gamma
     ...
     graph <- segplot(country ~ upperCI + lowerCI | parameter, 
                      ...
                      horizontal = FALSE, #makes error bars vertical and puts the countries in the x axis
                      layout = c(2,2), #2 column, 2 row
#                      index.cond = list(2,3,1,4), #orders the panels as alpha, beta, lambda, gamma
...
createCDLatticeGraph <- function(){
  ...
  dataTable <- cbind(dataTable, predictions, predictionsQ, predictionsX) 
  graphType <- "b" #b is for both line and symbol
...
<<CD_Params_Table, results='asis', eval=FALSE>>=
