...
createCountryFactorsGraph <- function(countryName){
  ...
  graph <- xyplot(iCapStk+iLabor+iQ+iX+iU ~ Year, data=dataTable,
                  ...
                  scales=list(cex=1.0, #controls text size on scales
                              tck=scaleTickSize), #controls tick mark length
                  ylab="Indexed (1980=1)")
  return(graph)
}
@
<<Factors_Lattice_Graph, fig.width=figWidth, fig.height=figHeight, fig.cap="GDP ($y$), capital stock ($k$), labor ($l$), thermal energy ($q$), exergy ($x$), and useful work ($u$) for all economies. (China's indexed GDP and indexed capital stock rise to $y = 7.3$ and $k = 9.2$ in 2011.)", echo=FALSE>>=
createFactorsLatticeGraph <- function(){
  ...
  colors <- c("black", "black", "black", "red", "blue", "black") #line and symbol colors
...
<<CD_Lattice_Graph, fig.width=figWidth, fig.height=figHeight, fig.cap="Cobb-Douglas model results.", echo=FALSE>>=
