...
\item Heun: Add CES production function fits
      ...
      \item \st{Convert $\rho$ variables in the \citet{Henningsen:2011td} CES production function to $\sigma$ variables.}
      \item \st{Create parameter graphs for each country}
      \item \st{Parameter tables for each energy type in the appendix}
      \item \st{Fix the problems with $\sigma_1$ and $\sigma$ values in the CES parameter tables in the appendix}
