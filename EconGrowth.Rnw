...
opts_chunk$set(
  ...
  tidy=FALSE,
  comment=NA,
  echo=FALSE,    #Turns off code echo for all chunks. Set TRUE on an individual chunk to see it.
...
<<Factors_Lattice_Graph, fig.width=figWidth, fig.height=figHeight, fig.cap="Indexed GDP ($y$), capital stock ($k$), labor ($l$), thermal energy ($q$), exergy ($x$), and useful work ($u$) for all economies. (China's indexed GDP and indexed capital stock rise to $y = 7.3$ and $k = 9.2$ in 2011.)", eval=FALSE>>=
...
cobbDouglasModel <- function(countryAbbrev, energyType){
  ...
  if (missing(energyType)){
    ...
    aGuess <- alphaGuess
    bGuess <- alphaGuess + betaGuess - 0.05
...
cobbDouglasPredictions <- function(countryAbbrev, energyType){
  ...
  if (countryAbbrev == "CN" || countryAbbrev == "ZA"){
    dfNA <- data.frame(NA)
...
cobbDouglasData <- function(countryName, energyType){
  ...
  ciCD <- confint(modelCD, level = ciLevel); ciCD # Calculates confidence intervals for the CD model.
...
############################
# Aggregates the Cobb-Douglas results into a big data table. With energy.
##
cobbDouglasParamsTableWithEnergy <- function(energyType){
  ...
  rownames(dataCD) <- countryAbbrevs
  tableCD <- xtable(dataCD, 
                    caption=paste("Cobb-Douglas (with ", energyType, ") for 1980-2011 (US, UK, JP) or 1991-2011 (others). (Parameter estimates beneath symbol. 95\\% confidence interval bounds to left and right.)", sep=""), 
                    digit = c(1, 4,4,4, 2,2,2, 2,2,2, 2,2,2),
                    align = "r|ccc|ccc|ccc|ccc") #Sets alignment of the numbers in the columns
  return(tableCD)
}
@

<<CD_Params_Table_No_E, results='asis'>>=
print(cobbDouglasParamsTableNoEnergy(), caption.placement="top", sanitize.colnames.function = identity)
@

<<CD_Params_Graph, fig.width=figWidth, fig.height=3.5, fig.cap="Cobb-Douglas (without energy) model parameters. Vertical bars indicate 95\\% confidence intervals.">>=
...
createCDLatticeGraph <- function(){
  ...
  predictions <- cobbDouglasPredictionsColumn() #Get the predictions from the CD model without energy
  predictionsQ <- cobbDouglasPredictionsColumn("Q") #Predictions with Q
  predictionsX <- cobbDouglasPredictionsColumn("X") #Predictions with X
  dataTable <- cbind(dataTable, predictions, predictionsQ, predictionsX) #Now add the predictions column to the data. Column name = "predGDP".
  graphType <- "b" #b is for both line and symbol
  lineTypes <- c(0, 1, 5, 6) #line types. See http://en.wikibooks.org/wiki/R_Programming/Graphics
  lineWidths <- c(0, 1, 1, 1) #line widths. 0 means no line.
  colors <- c("black", "black", "red", "blue") #line and symbol colors
  symbols <- c(1, NA, NA, NA)  #NA gives no symbol.
  graph <- xyplot(iGDP+predGDP+predGDPQ+predGDPX ~ Year | Country, data=dataTable,
                  type=graphType,
                  index.cond=list(countryOrderForGraphs), #orders the panels.
                  ...
                  lty = lineTypes, lwd = lineWidths, col = colors, #Controls line parameters
                  pch = symbols, col.symbol = colors, #Controls symbol parameters
                  key=list(text=list(c("Actual", "No energy", "With q", "With x")),
                           type=graphType,
                           cex=keyTextSize, #controls size of text in the key
                           lines=list(lty=lineTypes, lwd=lineWidths), #controls line types
                           pch=symbols, col=colors, #controls symbol (plot characters) types
...
<<CD_Params_Table_Q, results='asis'>>=
print(cobbDouglasParamsTableWithEnergy("Q"), caption.placement="top", sanitize.colnames.function = identity)
@


<<cobbDouglasEnergy function, eval=FALSE>>=
###############################
...
<<CDq Results, results='asis', eval=FALSE>>=
