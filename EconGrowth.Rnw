...
cobbDouglasEnergy <- function(dataTable, iEnergy){
  ...
  # * beta = b - a
  # * gamma = 1 - max(a, b)
  energyName = names(iEnergy) #grabs the name of the energy column
  lambdaGuess <- 0.0 # guessing lambda = 0 means there is no technological progress.
  ...
  row.names(estCDe) <- paste("Cobb-Douglas with ", energyName, sep="")
  # The [1] subscripts pick off the lower confidence interval
...
<<cdx>>=
dataCDx <- cobbDouglasEnergy(dataTable, dataTable["iX"])
...
<<cdu>>=
dataCDu <- cobbDouglasEnergy(dataTable, dataTable["iU"])
