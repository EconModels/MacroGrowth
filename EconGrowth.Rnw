...
print(dataTable)
  ...
  zetaGuess <- 0.147 # 0.147 for Sweden
  ...
  modelCES <- nls(iGDP ~ ((1-zeta)^(1.0/sigma) * ((exp(lambda_L*iYear))^beta 
                                                  * iCapStk^(1-beta) 
                                                  * iLabor^beta)^((sigma-1.0)/sigma) 
                          + zeta^(1.0/sigma)*(exp(lambda_E*iYear) 
                                                  * iEToFit)^((sigma-1.0)/sigma))^(sigma/(sigma-1.0)), 
                  data=dataTable,
                  start=list(sigma=sigmaGuess, zeta=zetaGuess, beta=betaGuess, 
                             lambda_L=lambda_LGuess, lambda_E=lambda_EGuess),
                  control=nls.control(maxiter=10000, tol=1e-06, minFactor=1/1024, 
                                      printEval=FALSE, warnOnly=TRUE),
                  algorithm = "port",
                  lower = list(sigma=-Inf, zeta=0, beta=0.0, lambda_L=0.0, lambda_E=0.0),
                  upper = list(sigma= Inf, zeta=1, beta=1.0, lambda_L=0.1, lambda_E=0.1),
                  )
  return(modelCES)
}
model <- cesModel("US", "Q")
...
The LINEX model is of the form

\begin{equation} \label{eq:LINEX_model}
  y = e \: \mathrm{exp} \left[a_0 \left(2 - \frac{l + e}{k} \right) + a_0 c_t \left(\frac{l}{e} - 1 \right) \right],
\end{equation}

\noindent where $e$ is any of $q$, $x$, or $u$, and $a_0$ and $c_t$ are fitting parameters.
...
<<LINEXq_Params_Graph, fig.width=linexParameterGraphWidth, fig.height=linexParameterGraphHeight, fig.pos="H", fig.cap="LINEX (with $q$) model parameters. Vertical bars indicate 95\\% confidence intervals.", eval=TRUE>>=
...
<<LINEXx_Params_Graph, fig.width=linexParameterGraphWidth, fig.height=linexParameterGraphHeight, fig.pos="H", fig.cap="LINEX (with $x$) model parameters. Vertical bars indicate 95\\% confidence intervals.", eval=TRUE>>=
...
<<LINEXu_Params_Graph, fig.width=linexParameterGraphWidth, fig.height=linexParameterGraphHeight, fig.pos="H", fig.cap="LINEX (with $u$) model parameters. Vertical bars indicate 95\\% confidence intervals.", eval=TRUE>>=
