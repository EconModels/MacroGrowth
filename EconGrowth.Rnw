...
<<Factors_Lattice_Graph, fig.width=figWidth, fig.height=figHeight, fig.cap="Indexed GDP ($y$), capital stock ($k$), labor ($l$), thermal energy ($q$), exergy ($x$), and useful work ($u$) for all economies. (China's indexed GDP and indexed capital stock rise to $y = 7.3$ and $k = 9.2$ in 2011.)", echo=FALSE>>=
...
cobbDouglasModel <- function(countryAbbrev, energyType){
  ...
  betaGuess <- 0.3 #if needed
  #Run the non-linear least squares fit to the data.
  if (missing(energyType)){
    ...
    modelCDe <- nls(iGDP ~ exp(lambda*iYear) * iCapStk^min(a,b) * iLabor^abs(b-a) * iEnergy^(1.0 - max(a,b)), 
                    data = dataTable,
                    algorithm = "port",
...
#print(lapply(countryAbbrevs, cobbDouglasModel, energyType="Q"))
model <- cobbDouglasModel(countryAbbrev="ZM", energyType="Q")
print(model)
print(model$convInfo)
#print(lapply(countryAbbrevs, cobbDouglasModel))
...
cobbDouglasParamsTable <- function(){
  ...
  tableCD <- xtable(dataCD, 
                    caption="Cobb-Douglas parameters for 1980-2011 (US, UK, JP) or 1991-2011 (others). (Parameter estimates beneath symbol. 95\\% confidence interval bounds to left and right.)", 
...
<<CD Params Table, results='asis', echo=FALSE>>=
print(cobbDouglasParamsTable(), caption.placement="top", sanitize.colnames.function = identity)
@

<<CD_Params_Graph, fig.width=figWidth, fig.height=3.5, fig.cap="Cobb-Douglas (without energy) model parameters. Vertical bars indicate 95\\% confidence intervals.", echo=FALSE>>=
createCDParamsGraph <- function(){
  ...
  graph <- segplot(country ~ upperCI + lowerCI | parameter, 
                   ...
                   strip = strip.custom(factor.levels=c(expression(italic(lambda)), #sets labels in strip
                   ...
                   scales = list(cex=scaleTextSize, #controls text size on scales.
                                 ...
                                 y=list(relation="free",                        #allow each axis to be different
                                        at=list(c(0.0, 0.05, 0.1),   #tick mark locations on y axis
                                                c(0.0, 0.5, 1.0), 
                                                c(0.0, 0.5, 1.0)
                                                )
                                        )
...
<<CD_Lattice_Graph, fig.width=figWidth, fig.height=figHeight, fig.cap="Cobb-Douglas (without energy) results.", echo=FALSE>>=
...
<<CDq analysis, echo=FALSE, eval=FALSE>>=
...
<<CDq Results, results='asis', echo=FALSE, eval=FALSE>>=
...
<<CDx analysis, echo=FALSE, eval=FALSE>>=
# Note that the anlaysis of ZA is taking a long time here. Need to figure out why.
CDxTables <- lapply(countryAbbrevs, cobbDouglasEnergyTable, energyType="X")
@

<<CDx Results, results='asis', echo=FALSE, eval=FALSE>>=
...
<<CDu analysis, echo=FALSE, eval=FALSE>>=
CDuTables <- lapply(countryAbbrevs, cobbDouglasEnergyTable, energyType="U")
@

<<CDu Results, results='asis', echo=FALSE, eval=FALSE>>=
...
<<CES function, echo=FALSE, eval=FALSE>>=
...
<<CESq analysisQ, echo=FALSE, eval=FALSE>>=
countryName <- "US"
energyType <- "Q"
...
<<CESq Results, results='asis', echo=FALSE, eval=FALSE>>=
