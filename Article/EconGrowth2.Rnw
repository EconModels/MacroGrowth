...
historicalData <- subset(AllHistData, subset=Source=="Calvin2011") # From the EconData package
...
using data for each economy (see Section~\ref{sec:Economy_Types}).
...
(when the resampling is repeated 1000 times),
...
\begin{equation} \label{eq:CDe_AB_reparam}
  y = \theta \mathrm{e}^{\lambda (t-t_0)} k^{\min(a,b)} l^{|b-a|} e^{1 - \max(a,b)}
\end{equation}
...
values of $\lambda$, $a$, and $b$ in log-transform space.
If the fitted value for $a$ ($b$) fell outside the range $[0,1]$, it was 
...
Again, historical data are shown as a black line, 
the fit to historical data is shown as a white linem and 
the gray band encompasses 95\% of the fits to resampled data.
...
\begin{equation} \label{eq:LINEX_alpha}
  \alpha = \frac{k}{y}\frac{\partial y}{\partial k} = a_0 \left( \frac{l + e}{k} \right) \;,
\end{equation}

\begin{equation} \label{eq:LINEX_beta}
  \beta = \frac{l}{y} \frac{\partial y}{\partial l} = a_0 \left[ c_t \left( \frac{l}{e}\right) - \frac{l}{k} \right] \;,
\end{equation}

\noindent{}and

\begin{equation} \label{eq:LINEX_beta}
  \gamma = \frac{e}{y} \frac{\partial y}{\partial e} 
...
the constraints of $\alpha > 0$, $\beta > 0$, and $\gamma > 0$ are respected.  
Conversely, if the trajectory goes outside the triangle, one of the constraints has been violated.

The following graphs shows trajectories for China and Iran.

<<LINEX_abg_traj_graph_CN, fig.pos="H", fig.align='center', fig.width=3, fig.height=3.5, fig.cap="LINEX trajectory for China.">>=
dataCN <- fortify(OrigModels$CN$linex$`iK+iL+iQ`)
triPlot(data=dataCN, grid_lines=5, x=alpha, y=beta, z=gamma, geom=geom_line, aes_string="colour = iYear + 1990")
@

<<LINEX_abg_traj_graph_IR, fig.pos="H", fig.align='center', fig.width=3, fig.height=1.5, fig.cap="LINEX trajectory for Iran.">>=
dataIR <- fortify(OrigModels$IR$linex$`iK+iL+iQ`)
triPlot(data=dataIR, grid_lines=5, x=alpha, y=beta, z=gamma, geom=geom_line, aes_string="colour = iYear + 1990")
@

Iran is the only country for which the trajecotry stays within the triangle for the 
entire time period.
Thus, in our fitted models, the LINEX model is, for the most part, 
violating constraints on $\alpha$, $\beta$, and $\gamma$.
If the LINEX model were made to respect the constraints on $\alpha$, $\beta$, and $\gamma$,
...
We did nothing to correct this situation, because it is not clear what could be done.
...
Specifically, we with to analyze exergy ($X$) and useful work ($U$)
