...
We wish to include exergy ($X$) and useful work ($U$) energy quantifications
...
And, we will be able to assess whether any models provide better 
descriptions of economic growth with a given energy quantification.
We intend to look at additional countries as data becomes available
for the various enegy quantifications.
And, we wish to look over longer time scales.
In particular, we wish to investigate the effects of significant 
processes (such as industrialization) and events (world wars)
