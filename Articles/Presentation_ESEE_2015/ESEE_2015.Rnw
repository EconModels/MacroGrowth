...
The set of $\Delta r$ values for each combination of 
country~(PT or UK), 
model~(Cobb-Douglas or CES),
flavor~(Unadjusted or Quality-adjusted), and
nest~[($kl$)$e$, ($le$)$k$, or ($ek$)$l$, for the CES models]
forms a set of numbers (one for each year) 
that can be assessed for statistical significance
compared to zero by a t-test.
The null hypothesis of the t-test is that the mean of each group is
equal to zero. 
...
Table~\ref{tab:Dr_p_values} summarizes the mean values for $\Delta r$
...
%
<<DSRmse_CD_graph, fig.pos="H", fig.align='center', fig.width=6.5, fig.height=4, fig.cap="Change in Solow Residuals~($\\Delta\\lambda$) and mean squared error~($\\Delta mse$) for the Cobb-Douglas model relative to the unadjusted, no-energy condition.">>=
...
%
<<DSRmse_CES_graph, fig.pos="H", fig.align='center', fig.width=6.5, fig.height=4, fig.cap="Change in Solow Residuals~($\\Delta\\lambda$) and mean squared error~($\\Delta mse$) for the Cobb-Douglas model relative to the unadjusted, no-energy condition.">>=
