...
because residuals are expected to be small (close to zero).
...
for Cobb-Douglas and CES production functions.
...
\end{equation}
%
Our concern is that the residuals may have a time-dependent component 
...
Equation~\ref{eq:r_for_correlation} can be fitted via linear regression in time~($t_i$).
%
...
to be equal to each other, because
