...
temp_data <- leaf_apply(models, class=c("CDEmodel", "cesModel", "LINEXmodel"), f = function(model, id){
  ...
  df[[response]] <- model$data[[response]] # Add iGDP to the data frame
  id_list <- strsplit(id, split = "[.]")[[1]]
...
dframes <- list() # Fill with data frames for each combination of country, flavor, energy, model, and nest
...
#       model.types <- c(CD="CD")
      model.types <- c(CD="CD", CES="CES")
      for (m in model.types){
        if (m == "CD"){
          origCoeffs <- filter(AllCoeffs, model==m & energy==e & Country==Coun & flavor==flav)
          df.CD <- cbind(df, origCoeffs)
          df.CD$nest <- NA
          lambda_fd <- 
            df.CD[["dydtOvery"]] - 
            df.CD[["alpha_1"]] / df.CD[[nums[[2]]]] * df.CD[[derivNames[[2]]]] - 
            df.CD[["alpha_2"]] / df.CD[[nums[[3]]]] * df.CD[[derivNames[[3]]]]
          if (e == "withE"){
            # Subtract the energy term also
            lambda_fd <- lambda_fd - 
              df.CD[["alpha_3"]] / df.CD[[nums[[4]]]] * df.CD[[derivNames[[4]]]]
          }
          # Fraction of growth explained by Solow Residual
          solowFrac <- lambda_fd / df.CD[["dydtOvery"]]
          # Add to data frame
          df.CD$lambda_fd <- lambda_fd
          df.CD$solowFrac <- solowFrac
          # Add to the list of data frames
          dframes[[length(dframes)+1]] <- df.CD
        } else if (m == "CES"){
          if (e == "noE"){
            origCoeffs <- filter(AllCoeffs, model==m & energy==e & Country==Coun & flavor==flav)
            df.kl <- cbind(df, select(origCoeffs, -nest, -flavor, -energy))
            df.kl$nest <- "kl"
            n <- c(1,2)
            # Get the values for x1 and x2
            df.kl$x1 <- df.kl[[ nums[ unlist(n)[1]+1 ] ]] # GDP is first column, add 1 to get to the factors of production
            df.kl$x2 <- df.kl[[ nums[ unlist(n)[2]+1 ] ]]
            df.kl$dx1dt <- df.kl[[ derivNames[ unlist(n)[1]+1 ] ]]
            df.kl$dx2dt <- df.kl[[ derivNames[ unlist(n)[2]+1 ] ]]
            ...
            # Add to the list of data frames
            dframes[[length(dframes)+1]] <- df.kl
          } else if (e == "withE"){
            nests <- list(kle=c(1,2,3), lek=c(2,3,1), ekl=c(3,1,2))
            for (i in 1:3){
              n <- nests[i] # preserves name
              origCoeffs <- filter(AllCoeffs, model==m & energy==e & Country==Coun & flavor==flav & nest==names(n)[[1]])
              df.CESe <- cbind(df, select(origCoeffs, -nest, -flavor, -energy))
              df.CESe$nest <- names(n)[[1]]
              # Unpack names so that the formulae below are readable.
              df.CESe$x1 <- df.CESe[[ nums[ unlist(n)[1]+1 ] ]] # GDP is 1st column, add 1 to get factors of production
              df.CESe$x2 <- df.CESe[[ nums[ unlist(n)[2]+1 ] ]]
              df.CESe$x3 <- df.CESe[[ nums[ unlist(n)[3]+1 ] ]]
              df.CESe$dx1dt <- df.CESe[[ derivNames[ unlist(n)[1]+1 ] ]]
              df.CESe$dx2dt <- df.CESe[[ derivNames[ unlist(n)[2]+1 ] ]]
              df.CESe$dx3dt <- df.CESe[[ derivNames[ unlist(n)[3]+1 ] ]]
              # Calculate derivative terms
              df.CESe <- df.CESe %>% 
                transform(b = delta_1 * x1^(-rho_1) + (1 - delta_1) * x2^(-rho_1)) %>%
                ...
                transform(solowFrac = lambda_fd / dydtOvery)
              # Add to the list of data frames
              dframes[[length(dframes)+1]] <- df.CESe
            }
          } else {
...
temp_data <- do.call(plyr::rbind.fill, dframes)
...
#     geom_path(mapping=aes(x=Dr, y=Dlambda_fd, color=energy)) + 
    facet_grid(Country ~ flavor) +
...
We can also calculate the average values for $\Delta \lambda$ and $\Delta r$ 
as shown in Table~\ref{tab:traj.means.CD}.
%
<<TrajMeansTableFunction>>=
trajMeansTable <- function(data, caption, label){
  table <- xtable::xtable(data, caption=caption, label=label, digits=6)
  print(table, 
...
trajMeansTable(data = filter(traj.data.means, model=="CD") %>% select(-nest), 
               caption = "Average $\\Delta\\lambda$ and $\\Delta r$ for Cobb-Douglas models.",
               label="tab:traj.means.CD")
# xtable::xtable(filter(traj.data.means, model=="CD") %>% select(-nest),
#                caption="Average $\\Delta\\lambda$ and $\\Delta r$ for Cobb-Douglas models.",
#                label="tab:traj.means.CD", 
#                digits=6) %>% 
#   print(., 
...
@
%
These results for the Cobb-Douglas model indicate that 
we should perform the same analysis for the CES model.
