...
AllCoeffs_unadj_noE <- filter(AllCoeffs, flavor=="unadjusted", energy=="noE") %>% 
...
AllCoeffs <- mutate(AllCoeffs, Dlambda = abs(lambda) - abs(lambda_unadj_noE), Dmse = mse - mse_unadj_noE)
...
\subsection{Does quality-adjusting the factors of production 
            decrease the fitting residual and dynamic Solow residual?} 
...
(thereby, reducing the fitting residuals, $r$)
by calculating 
...
\begin{equation} \label{eq:delta_mse}
  \Delta mse \equiv \frac{1}{N} \sum_i{r_i^2} - mse_{Unadjusted, \, Without \, energy}.
\end{equation}
%
where~$N$ is the number of years of data.
When $\Delta \lambda$ and $\Delta mse$ are negative, 
we observe reduction in the Solow residual ($\lambda$) and
the fitting residuals ($r$).
...
@
%
Tables~\ref{tab:DSRmse_CESkle}--\ref{tab:CSRmse_CESekl} show results for the various CES nestings.
%
...
dynamicSR <- function(model, id, lag=1, pad="tail"){
  ...
  df <- model$data[c(denom, nums)] # columns are: iYear, y, k, l, and e (if present)
...
myXTable(data = filter(traj.data.means, model=="CD") %>% select(-nest), 
...
myXTable(data = filter(traj.data.means, model=="CES" & (nest=="kl" | nest=="kle")), 
         caption = "Average $\\Delta\\lambda$ and $\\Delta r$ for CES models with ($kl$)$e$ nesting.",
         label="tab:traj.means.CESkle",
         digits=6)
@
...
trajMeansTable(data = filter(traj.data.means, model=="CES" & (nest=="kl" | nest=="lek")), 
               caption = "Average $\\Delta\\lambda$ and $\\Delta r$ for CES models with ($le$)$k$ nesting.",
               label="tab:traj.means.CESlek",
               digits=6)
@
...
myXTable(data = filter(traj.data.means, model=="CES" & (nest=="kl" | nest=="ekl")), 
         caption = "Average $\\Delta\\lambda$ and $\\Delta r$ for CES models with ($ek$)$l$ nesting.",
         label="tab:traj.means.CESekl",
         digits=6)
@
