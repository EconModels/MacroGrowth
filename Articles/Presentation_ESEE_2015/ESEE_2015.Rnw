...
nests <- list(kle=c(1,2,3), lek=c(2,3,1), ekl=c(3,1,2))
model.types <- c("CD")
# model.types <- c("CD", "CES")
...
AllCoeffs <- leaf_apply(models, class = c("CDEmodel", "cesModel", "LINEXmodel"), f = function(mod, id){
  ...
  res <- rbind(list(Country=Country, flavor=flavor, energy=energy, model=model, nest=nest))
  res <- cbind(res, coeffs)
  return(res)
}) %>% do.call(plyr::rbind.fill, .)
# Relevel factors
AllCoeffs$Country <- relevelFactor(as.factor(unlist(AllCoeffs$Country)), c("PT", "UK"))
AllCoeffs$flavor <- relevelFactor(as.factor(unlist(AllCoeffs$flavor)), c("unadjusted", "adjusted"))
AllCoeffs$energy <- relevelFactor(as.factor(unlist(AllCoeffs$energy)), c("noE", "withE"))
...
fitted_and_resid_data <- leaf_apply(models, class=c("CDEmodel", "cesModel", "LINEXmodel"), f = function(mod, id){
  df <- select(mod$data, Year, iGDP, Country)
...
r_unadj_noE <- filter(fitted_and_resid_data, flavor=="Unadjusted" & energy=="Without energy") %>% 
...
fitted_and_resid_data <- merge(fitted_and_resid_data, r_unadj_noE)
...
Because we fit in log-space, the residuals ($r_i$) are defined as
%
...
\subsection{Does quality-adjusting the factors of production 
            decrease the dynamic Solow residual?} 
...
lag <- 1       # Desired lag for calculations. Set to n for differences across n years.
pad <- "tail"  # Desired padding for NA values

dframes <- list()
for (Coun in countries){
  for (flav in flavors){
    forms <- list(noE = keep_summands(formulas[[Coun]][[flav]], -3), withE = formulas[[Coun]][[flav]])
    for (form in forms){
      denom <- summands(form, n=length(summands(form))) # time variable
      nums <- all.vars(form) [1:(length(all.vars(form))-1)] # y, k, l, and e (if present)
      df <- historicalData[[Coun]][c(denom, nums)] # columns are: iYear, y, k, l, and e (if present)
      # Calculate names for the columns of finite difference derivatives
      derivNames <- lapply(1:length(nums), function(i){paste0("d", nums[[i]], "/", "dt")}) %>% unlist
      ...
      df <- cbind(derivCols, historicalData[[Coun]][ , c(nums, "Year")])
      e <- if (length(summands(form))==4) "withE" else "noE"
      df$flavor <- if (flav == "unadjusted") "Unadjusted" else "Quality-adjusted"
      df$energy <- if (e == "noE") "Without energy" else "With energy"
      # Add a column for 1/y * dy/dt
#       growth.col.name <- paste0("1/", nums[[1]], "*", derivNames[[1]])
      df[["dydtOvery"]] <- 1 / df[[nums[[1]]]] * df[[derivNames[[1]]]]
      for (m in model.types){
        origCoeffs <- filter(AllCoeffs, model==m & energy==e & Country==Coun & flavor==flav)
        df <- cbind(df, origCoeffs)
        if (m == "CD"){
          lambda_fd <- lapply(1:length(derivNames), function(i){
            # derivative term divided by its numerator.
            # derivative term is in the ith column of df
            # numerator is in i + length(derivNames) th column of df
            out <- df[[i]] / df[[i+length(derivNames)]]
            if (i > 1){
              # Need to make it negative and multiply by the corresponding alpha term.
              # Typical setup is that columns 2 is capital, and we need alpha_1 for capital.
              # So, subtract 1 from i.
              out <- - out * df[[paste0("alpha_", i-1)]]
            }
            return(out)
          }) %>% as.data.frame %>% rowSums
          df$lambda_fd <- lambda_fd
          avg.lambda_fd <- mean(df$lambda_fd, na.rm = TRUE)
          # Fraction of growth explained by Solow Residual
          df$solowFrac <- df$lambda_fd / (df[[1]] / df[[length(derivNames) + 1]])
          avg.solowFrac <- mean(df$solowFrac, na.rm = TRUE)
        } else if (m == "CES"){
          df <- data.frame()
          if (e == "noE"){
            # kl nesting
#             df$lambda_fd <- df[[1]] / 
        ...
        dframes[[length(dframes)+1]] <- df
      }
    }
  }      
}
solowResiduals <- do.call(plyr::rbind.fill, dframes)

# Grab the unadjusted, no-energy Solow residuals and create a column in solowResiduals with them. 
# Using the hint at 
# http://stackoverflow.com/questions/15059076/
# r-how-to-call-apply-like-function-on-each-row-of-dataframe-with-multiple-argum
sr_unadj_noE_col <- apply(solowResiduals[ , c("Country", "model", "energy", "Year", "flavor")], 1, function(x){
    row <- as.data.frame(filter(solowResiduals, 
                                Country==x["Country"], model==x["model"], energy=="Without energy",
                                Year==x["Year"], flavor=="Unadjusted"))
    return(row[["lambda_fd"]])
  })
solowResiduals[["sr_unadj_noE"]] <- do.call(rbind, as.list(sr_unadj_noE_col))
...
# Adjust levels before creating graphs.
...
of the Solow residual~($\lambda$) over time
as calculated by Equation~\ref{eq:lambdaij} 
with the timespan for finite differences ($t_j - t_i$) set to 
\Sexpr{lag}~\Sexpr{if (lag == 1) "year" else "years"}.
It appears that quality-adjusting the factors of production
\emph{and} adding augmenting the production function with energy
decreases the Solow residual~($\lambda$),
i.e., brings the Solow residual closer to zero.
...
data <- filter(solowResiduals, model=="CD")
ggplot(data=data) +
...
%++++++++++++++++++++++++++++++
\subsection{How does quality-adjusting and including energy affect the dynamic Solow residual?} 
\label{sec:trajectory_graphs}
%++++++++++++++++++++++++++++++

<<SR and R trajectory graph functions>>=
residualsGraph <- function(data){
  plot <- ggplot(data=data) + 
    geom_point(mapping=aes(x=Dr, y=Dlambda_fd)) + 
...
and including energy will reduce the Solow residual ($\lambda$)
and the fitting residuals ($r$) by plotting
$\Delta\lambda$ vs. $\Delta r$, where
%
\begin{equation} \label{eq:delta_lambda}
  \Delta \lambda \equiv \lambda - \lambda_{Unadjusted, \, Without \, energy}
\end{equation}
%
and
%
\begin{equation} \label{eq:delta_r}
  \Delta r \equiv r - r_{Unadjusted, \, Without \, energy}.
\end{equation}
%

<<Solow_residual_trajectories, fig.pos="H", fig.align='center', fig.width=6.5, fig.height=4, fig.cap="Dynamic Solow residual trajectories for the Cobb-Douglas model.">>=
residualsGraph(subset(traj.data, model=="CD" & Country=="UK" & energy=="With energy" & flavor=="Unadjusted"))
@
