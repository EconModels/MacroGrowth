...
fd <- leaf_apply(formulas, class = "formula", f = function(form, name){
  meta <- strsplit(name, split="[.]") %>% unlist
  Country <- meta[1]
  flavor <- meta[2]
  
  denom <- summands(form, n=length(summands(form))) # time variable
  nums <- c(summands(form, n=1, right = FALSE), summands(form, n=1:3)) # y, k, l, and e
  trimmedData <- historicalData[[Country]][c(denom, nums)] # columns are: iYear, y, k, l, and e
  df <- list()
  df <- historicalData[[Country]][ , c("Year", "Country")]
  df$dydt <- ediff(trimmedData[[2]], pad=pad, lag=lag) / ediff(trimmedData[[1]], pad=pad, lag=lag)
  df$dkdt <- ediff(trimmedData[[3]], pad=pad, lag=lag) / ediff(trimmedData[[1]], pad=pad, lag=lag)
  df$dldt <- ediff(trimmedData[[4]], pad=pad, lag=lag) / ediff(trimmedData[[1]], pad=pad, lag=lag)
  df$dedt <- ediff(trimmedData[[5]], pad=pad, lag=lag) / ediff(trimmedData[[1]], pad=pad, lag=lag)
