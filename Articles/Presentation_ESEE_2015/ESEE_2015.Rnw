...
fittedAndResidData <- function(model, id){
  df <- data.frame(select(model$data, Year)) # getData doesn't work for lm models that are created for CST=TRUE.
...
fittedGraph(filter(fitted_and_resid_data, model=="CD" & cst==FALSE))
@
%
...
fittedGraph(filter(fitted_and_resid_data, model=="CES" & cst==FALSE & (nest=="kle" | nest=="kl")))
@
%
...
fittedGraph(filter(fitted_and_resid_data, model=="CES" & cst==FALSE & (nest=="lek" | nest=="kl")))
@
%
...
fittedGraph(filter(fitted_and_resid_data, model=="CES" & cst==FALSE & (nest=="ekl" | nest=="kl")))
@
