...
if (!rerunModels){
  ...
  for (country in countries){
    form <- formulas[[country]][["unadjusted"]]
    ...
    mod <- lm(log(eval(yVar)) - alpha_CST*log(eval(kVar)) - beta_CST*log(eval(lVar)) ~ eval(tVar), 
              data = historicalData[[country]])
    attr(mod, "naturalCoeffs") <- c(lambda=coef(mod)[[2]],
                                    logscale=coef(mod)[[1]], 
                                    scale=exp(coef(mod)[[1]]),
                                    alpha_1=alphaCST,
                                    alpha_2=betaCST, 
                                    alpha_3=0)
    modelsCST[[country]][["unadjusted"]][["noE"]][["CD"]] <- mod
  }
