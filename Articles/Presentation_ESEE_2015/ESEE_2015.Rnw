...
\end{equation}
%
Equation~\ref{eq:CESkl} is a CES production function with 
...
\begin{table} \caption{Unadjusted variables.} 
...
\label{tab:CES_abg_equations} 
  \begin{center}
    \begin{tabular}{c c c c c} 
      \toprule
      Nesting         & \texttt{nest}     & $\alpha_1$ ($\alpha$)   & $\alpha_2$ ($\beta$)    & $\alpha_3$ ($\gamma$)       \\
      \midrule
...
ptCDunadjNoE <- cdModel(formula = f_PT_unadj_noE, 
                        data = dataPT, constrained = TRUE)
naturalCoef(ptCDunadjNoE)
@

<<Portugal C-D undadjusted with energy>>=
ptCDunadjWithE <- cdModel(formula = f_PT_unadj_withE, 
                          data = dataPT, constrained = TRUE)
...
\Sexpr{naturalCoef(ptCDunadjNoE)$lambda} to 
\Sexpr{naturalCoef(ptCDunadjWithE)$lambda}
when adding energy.
...
ptCDadjNoE <- cdModel(formula = f_PT_adj_noE, 
                      data = dataPT, constrained = TRUE)
naturalCoef(ptCDadjNoE)
@

<<Portugal C-D adjusted with energy>>=
ptCDadjWithE <- cdModel(formula = f_PT_adj_withE, 
                          data = dataPT, constrained = TRUE)
naturalCoef(ptCDadjWithE)
@

Using quality-adjusted inputs, the Solow residual (\texttt{lambda}) moves from 
\Sexpr{naturalCoef(ptCDadjNoE)$lambda} to
\Sexpr{naturalCoef(ptCDadjWithE)$lambda} when including energy.
...
We find that both including energy and quality-adjusting the factors of production 
...
\label{tab:lambda_CD_PT} 
  \begin{center}
    \begin{tabular}{r c c} 
      ...
      \midrule
      Without energy  & \Sexpr{naturalCoef(ptCDunadjNoE)$lambda}      & \Sexpr{naturalCoef(ptCDadjNoE)$lambda} \\
      With energy     & \Sexpr{naturalCoef(ptCDunadjWithE)$lambda}    & \Sexpr{naturalCoef(ptCDadjWithE)$lambda} \\
      \bottomrule
    \end{tabular}
  \end{center}
\end{table}

Note that the contribution of capital (\texttt{alpha\_1})
...
<<UK C-D>>=
ukCDunadjNoE <- cdModel(formula = f_UK_unadj_noE, 
                        data = dataUK, constrained = TRUE)
ukCDunadjWithE <- cdModel(formula = f_UK_unadj_withE, 
                          data = dataUK, constrained = TRUE)
ukCDadjNoE <- cdModel(formula = f_UK_adj_noE, 
                        data = dataUK, constrained = TRUE)
ukCDadjWithE <- cdModel(formula = f_UK_adj_withE, 
                          data = dataUK, constrained = TRUE)
@
  ...
  \begin{center}
    \begin{tabular}{r c c} 
      \toprule
                      & Unadjusted     & Quality-adjusted   \\
      \midrule
      Without energy  & \Sexpr{naturalCoef(ukCDunadjNoE)$lambda}      & \Sexpr{naturalCoef(ukCDadjNoE)$lambda} \\
      With energy     & \Sexpr{naturalCoef(ukCDunadjWithE)$lambda}    & \Sexpr{naturalCoef(ukCDadjWithE)$lambda} \\
      \bottomrule
    \end{tabular}
  \end{center}
\end{table}
...
\Sexpr{naturalCoef(ukCDadjWithE)$lambda * 100}%,
...
reducing it to \Sexpr{naturalCoef(ukCDadjWithE)$lambda}.

Interestingly, the Solow residual acutally increases from 
\Sexpr{naturalCoef(ukCDunadjNoE)$lambda} to
\Sexpr{naturalCoef(ukCDunadjWithE)$lambda}
...
\Sexpr{naturalCoef(ukCDunadjNoE)$alpha_1} to
\Sexpr{naturalCoef(ukCDunadjWithE)$alpha_1}.
...
\Sexpr{naturalCoef(ukCDadjNoE)$alpha_1} to
\Sexpr{naturalCoef(ukCDadjWithE)$alpha_1}.
...
<<PT CES-kl, warning=FALSE, eval=evalCES>>=
ptCESklunadjNoE <- cesModel(formula = f_PT_unadj_noE, 
                            nest = c(1,2), data = dataPT, constrained = TRUE)
naturalCoef(ptCESklunadjNoE)
ptCESkladjNoE <- cesModel(formula = f_PT_adj_noE, 
                          nest = c(1,2), data = dataPT, constrained = TRUE)
naturalCoef(ptCESkladjNoE)
@
...
First, look at unadjusted inputs using the ($kl$)$e$ nesting.

<<PT CES-kle unadjusted, warning=FALSE, eval=evalCES>>=
ptCESkleunadjWithE <- cesModel(formula = f_PT_unadj_withE, 
                            nest = c(1,2,3), data = dataPT, constrained = TRUE)
naturalCoef(ptCESkleunadjWithE)
@

Now look at quality-adjusted inputs using the ($kl$)$e$ nesting.

<<PT CES-kle quality-adjusted, warning=FALSE, eval=evalCES>>=
ptCESkleadjWithE <- cesModel(formula = f_PT_adj_withE, 
                          nest = c(1,2,3), data = dataPT, constrained = TRUE)
naturalCoef(ptCESkleadjWithE)
@
  ...
  \begin{center}
    \begin{tabular}{r c c} 
      \toprule
                      & Unadjusted     & Quality-adjusted   \\
      \midrule
      Without energy  & \Sexpr{naturalCoef(ptCESklunadjNoE)$lambda}       & \Sexpr{naturalCoef(ptCESkladjNoE)$lambda} \\
      With energy     & \Sexpr{naturalCoef(ptCESkleunadjWithE)$lambda}    & \Sexpr{naturalCoef(ptCESkleadjWithE)$lambda} \\
      \bottomrule
    \end{tabular}
  \end{center}
\end{table}
...
<<PT CES-lek unadjusted, warning=FALSE, eval=evalCES>>=
ptCESlekunadjWithE <- cesModel(formula = f_PT_unadj_withE, 
                            nest = c(2,3,1), data = dataPT, constrained = TRUE)
naturalCoef(ptCESlekunadjWithE)
@

Now look at quality-adjusted inputs using the ($le$)$k$ nesting.

<<PT CES-lek quality-adjusted, warning=FALSE, eval=evalCES>>=
ptCESlekadjWithE <- cesModel(formula = f_PT_adj_withE, 
                          nest = c(2,3,1), data = dataPT, constrained = TRUE)
naturalCoef(ptCESlekadjWithE)
@
...
\label{tab:lambda_CES-lek_PT} 
  \begin{center}
    \begin{tabular}{r c c} 
      ...
      \midrule
      Without energy  & \Sexpr{naturalCoef(ptCESklunadjNoE)$lambda}       & \Sexpr{naturalCoef(ptCESkladjNoE)$lambda} \\
      With energy     & \Sexpr{naturalCoef(ptCESlekunadjWithE)$lambda}    & \Sexpr{naturalCoef(ptCESlekadjWithE)$lambda} \\
      \bottomrule
    \end{tabular}
  \end{center}
\end{table}
...
%------------------------------
\subsubsection{($ek$)$k$ nesting} 
%------------------------------

Next, look at unadjusted inputs using the ($ek$)$l$ nesting.

<<PT CES-ekl unadjusted, warning=FALSE, eval=evalCES>>=
ptCESeklunadjWithE <- cesModel(formula = f_PT_unadj_withE, 
                            nest = c(3,1,2), data = dataPT, constrained = TRUE)
naturalCoef(ptCESeklunadjWithE)
@

Now look at quality-adjusted inputs using the ($ek$)$l$ nesting.

<<PT CES-ekl quality-adjusted, warning=FALSE, eval=evalCES>>=
ptCESekladjWithE <- cesModel(formula = f_PT_adj_withE, 
                          nest = c(3,1,2), data = dataPT, constrained = TRUE)
naturalCoef(ptCESekladjWithE)
@
...
\label{tab:lambda_CES-ekl_PT} 
  \begin{center}
    \begin{tabular}{r c c} 
      ...
      \midrule
      Without energy  & \Sexpr{naturalCoef(ptCESklunadjNoE)$lambda}       & \Sexpr{naturalCoef(ptCESkladjNoE)$lambda} \\
      With energy     & \Sexpr{naturalCoef(ptCESeklunadjWithE)$lambda}    & \Sexpr{naturalCoef(ptCESekladjWithE)$lambda} \\
      \bottomrule
    \end{tabular}
  \end{center}
\end{table}
