...
if (!rerunModels){
  ...
  for (country in countries){
    ...
    # Would like to say cdMod(formula, data, fittingParams=c(alpha_1 = 0.3, alpha_2 = 0.7, alpha_3 = 0))
    # or something like that.
    form <- keep_summands(formulas[[country]][["unadjusted"]], n=-3) # Remove energy
    ...
    modelsCST[[country]][["unadjusted"]][["noE"]][["CD"]] <- cdModCST
    # CES
    ...
    modelsCST[[country]][["unadjusted"]][["noE"]][["CES"]][["kl"]] <- cesModCST
  }
...
# Put all the cost-share theorem (CST) coefficients into a data frame
...
AllCoeffs <- plyr::rbind.fill(AllCoeffsCST, AllCoeffsNoCST)
# Get reference values for lambda and sse.
ref <- filter(AllCoeffs, cst==TRUE & flavor=="Unadjusted" & energy=="Without energy") %>% 
...
AllCoeffs <- merge(AllCoeffs, ref[c("Country", "model", "mse_ref", "lambda_ref")])
# Calculate differences relative to the unadjusted, no-energy condition
AllCoeffs <- mutate(AllCoeffs, Dlambda = abs(lambda) - abs(lambda_ref), 
                    Dmse = mse - mse_ref,
...
AllCoeffs$cst <- relevelFactor(as.factor(unlist(AllCoeffs$cst)), c("TRUE", "FALSE"))
...
fittedAndResidData <- function(model, id){
  # ******************************************
  # getData doesn't work for lm models that are created for CST=TRUE.
  # After we implement fixed values for alpha_1 and alpha_2 in our fitting functions, 
...
temp_data_CST$cst <- TRUE

temp_data <- plyr::rbind.fill( temp_data_CST, temp_data_noCST )

# Get the unadjusted, without-energy residuals from models that obey the CST
# and store them in a data frame
r_unadj_noE_CST <- filter(temp_data, flavor=="Unadjusted" & energy=="Without energy" & cst==TRUE) %>% 
...
fittedGraphCST <- function(data){
  ggplot() +
    geom_line(data=filter(data, cst==TRUE), mapping=aes(x=Year, y=iGDP)) +
...
fittedGraphNoCST(filter(fitted_and_resid_data, model=="CD" & cst==FALSE))
@
%
...
fittedGraphNoCST(filter(fitted_and_resid_data, model=="CES" & cst==FALSE & (nest=="kle" | nest=="kl")))
@
%
...
fittedGraphNoCST(filter(fitted_and_resid_data, model=="CES" & cst==FALSE & (nest=="lek" | nest=="kl")))
@
%
...
fittedGraphNoCST(filter(fitted_and_resid_data, model=="CES" & cst==FALSE & (nest=="ekl" | nest=="kl")))
@
...
residualPlot(filter(fitted_and_resid_data, model=="CD" & cst==FALSE))
@
%
...
residualPlot(filter(fitted_and_resid_data, model=="CES" & cst==FALSE & (nest=="kle" | nest=="kl")))
@
%
...
residualPlot(filter(fitted_and_resid_data, model=="CES" & cst==FALSE & (nest == "lek" | nest=="kl")))
@
%
...
residualPlot(filter(fitted_and_resid_data, model=="CES" & cst==FALSE & (nest=="ekl" | nest=="kl")))
@
...
on fitted parameters for the Cobb-Douglas model.
%
<<Effect_of_CST_CD, results="asis">>=
myXTable(filter(AllCoeffs, model=="CD" & flavor=="Unadjusted" & energy=="Without energy") %>% 
           select(Country, cst, scale, lambda, alpha_1, alpha_2, mse),
...
%
<<Effect_of_CST_CES, results="asis">>=
myXTable(filter(AllCoeffs, model=="CES" & flavor=="Unadjusted" & energy=="Without energy") %>% 
           select(Country, cst, gamma, lambda, alpha_1, alpha_2, sigma_1, mse),
...
myXTable(filter(AllCoeffs, model=="CD") %>% 
           select(Country, flavor, energy, cst, scale, lambda, alpha_1, alpha_2, alpha_3, mse),
...
coefficients for all fitted CES models.
%
<<CES_coeffs, results="asis">>=
myXTable(filter(AllCoeffs, model=="CES") %>% 
           select(Country, flavor, energy, nest, cst, gamma, lambda, alpha_1, alpha_2, alpha_3),
         caption="Model parameters for all CES models.",
         label=paste0("tab:", opts_current$get("label")), 
         digits=6)
@
%
<<CES_coeffs_sigmas, results="asis">>=
myXTable(filter(AllCoeffs, model=="CES") %>% 
           select(Country, flavor, energy, nest, cst, sigma_1, sigma, mse),
         caption="Model parameters for all CES models.",
         label=paste0("tab:", opts_current$get("label")), 
         digits=6)
@
...
myXTable(filter(AllCoeffs, model=="CES" & (nest=="kl" | nest=="kle")) %>% 
           select(Country, model, flavor, energy, nest, cst, Dlambda, Dmse),
...
%
<<DSRmse_CD_graph, fig.pos="H", fig.align='center', fig.width=6.5, fig.height=4, fig.cap="Change in Solow Residuals~($\\Delta\\lambda$) and mean squared error~($\\Delta mse$) for the Cobb-Douglas model relative to the Unadjusted, Without energy, CST case.">>=
ggplot() +
  geom_point(data=filter(AllCoeffs, cst==FALSE & model=="CD"), mapping=aes(x=Dmse, y=Dlambda, color=energy)) +
  geom_point(data=filter(AllCoeffs, cst==TRUE & model=="CD"), mapping=aes(x=Dmse, y=Dlambda, color=energy), shape=1) +
  facet_grid(Country ~ flavor) +
...
@
%
<<percDSRmse_CD_graph, fig.pos="H", fig.align='center', fig.width=6.5, fig.height=4, fig.cap="Percentage change in Solow Residuals~($\\Delta\\lambda \\, [\\%]$) and mean squared error~($\\Delta mse$) for the Cobb-Douglas model relative to the Unadjusted, Without energy, CST case.">>=
ggplot() +
  geom_point(data=filter(AllCoeffs, cst==FALSE & model=="CD"), mapping=aes(x=Dmse, y=percDlambda, color=energy)) +
  geom_point(data=filter(AllCoeffs, cst==TRUE & model=="CD"), mapping=aes(x=Dmse, y=percDlambda, color=energy), shape=1) +
  facet_grid(Country ~ flavor) +
  xlab(expression(Delta * mse)) + 
  scale_x_continuous(breaks=seq(-0.005, 0, 0.005)) +  
  ylab(expression(paste(Delta * lambda, " [%]"))) +
  scale_y_continuous(breaks=seq(-75, 25, 25)) +  
  xy_theme() + 
  theme(legend.title=element_blank())
@
%
...
%
<<DSRmse_CES_graph, fig.pos="H", fig.align='center', fig.width=6.5, fig.height=4, fig.cap="Change in Solow Residuals~($\\Delta\\lambda$) and mean squared error~($\\Delta mse$) for the CES modles relative to the Unadjusted, Without energy, CST case.">>=
ggplot() +
  geom_point(data=filter(AllCoeffs, model=="CES" & cst==FALSE), mapping=aes(x=Dmse, y=Dlambda, color=energy, shape=nest)) +
  geom_point(data=filter(AllCoeffs, model=="CES" & cst==TRUE), mapping=aes(x=Dmse, y=Dlambda, color=energy), shape=1) +
  facet_grid(Country ~ flavor) +
  xlab(expression(Delta * mse)) + 
  scale_x_continuous(breaks=seq(-0.005, 0, 0.005)) +  
  ylab(expression(Delta * lambda)) +
...
@
%
<<percDSRmse_CES_graph, fig.pos="H", fig.align='center', fig.width=6.5, fig.height=4, fig.cap="Percentage change in Solow Residuals~($\\Delta\\lambda \\, [\\%]$) and mean squared error~($\\Delta mse$) for the CES modles relative to the Unadjusted, Without energy, CST case.">>=
ggplot() +
  geom_point(data=filter(AllCoeffs, model=="CES" & cst==FALSE), mapping=aes(x=Dmse, y=percDlambda, color=energy, shape=nest)) +
  geom_point(data=filter(AllCoeffs, model=="CES" & cst==TRUE), mapping=aes(x=Dmse, y=percDlambda, color=energy), shape=1) + 
  facet_grid(Country ~ flavor) +
  xlab(expression(Delta * mse)) + 
  scale_x_continuous(breaks=seq(-0.005, 0, 0.005)) +  
  ylab(expression(paste(Delta * lambda, " [%]"))) +
...
fsbg <- function(coun, mod){
  ...
  # Change boolean cst column to meaningful words
  bar_chart_data$cst <- plyr::mapvalues(bar_chart_data$cst, from=c(TRUE, FALSE), to=c("Adhere to CST", "Reject CST"))
