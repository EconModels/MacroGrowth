...
dataPT <- transform(dataPT, kPerGDP_unadj = eval(k_PT_unadj)/iGDP)
dataPT <- transform(dataPT, lPerGDP_unadj = eval(l_PT_unadj)/iGDP)
dataPT <- transform(dataPT, ePerGDP_unadj = eval(e_PT_unadj)/iGDP)
dataUK <- transform(dataUK, kPerGDP_unadj = eval(k_UK_unadj)/iGDP)
dataUK <- transform(dataUK, lPerGDP_unadj = eval(l_UK_unadj)/iGDP)
dataUK <- transform(dataUK, ePerGDP_unadj = eval(e_UK_unadj)/iGDP)
# Quality-adjusted variables
dataPT <- transform(dataPT, kPerGDP_adj = eval(k_PT_adj)/iGDP)
dataPT <- transform(dataPT, lPerGDP_adj = eval(l_PT_adj)/iGDP)
dataPT <- transform(dataPT, ePerGDP_adj = eval(e_PT_adj)/iGDP)
dataUK <- transform(dataUK, kPerGDP_adj = eval(k_UK_adj)/iGDP)
dataUK <- transform(dataUK, lPerGDP_adj = eval(l_UK_adj)/iGDP)
dataUK <- transform(dataUK, ePerGDP_adj = eval(e_UK_adj)/iGDP)
...
\label{tab:unadjusted_variables} 
  \begin{center}
    \begin{tabular}{r l l} 
      ...
      \midrule
      Output      & \Sexpr{y_PT}       & \Sexpr{y_UK}       \\
      Capital     & \Sexpr{k_PT_unadj} & \Sexpr{k_UK_unadj} \\
      Labor       & \Sexpr{l_PT_unadj} & \Sexpr{l_UK_unadj} \\
      Energy      & \Sexpr{e_PT_unadj} & \Sexpr{e_UK_unadj} \\
      \bottomrule
    \end{tabular}
  \end{center}
\end{table}
...
\label{tab:quality_adjusted_variables} 
  \begin{center}
    \begin{tabular}{r l l} 
      ...
      \midrule
      Output      & \Sexpr{y_PT}     & \Sexpr{y_UK}     \\
      Capital     & \Sexpr{k_PT_adj} & \Sexpr{k_UK_adj} \\
      Labor       & \Sexpr{l_PT_adj} & \Sexpr{l_UK_adj} \\
      Energy      & \Sexpr{e_PT_adj} & \Sexpr{e_UK_adj} \\
      \bottomrule
    \end{tabular}
  \end{center}
\end{table}
...
unadjVars_PT <- c(y_PT, k_PT_unadj, l_PT_unadj, e_PT_unadj)
unadjVars_UK <- c(y_UK, k_UK_unadj, l_UK_unadj, e_UK_unadj)
commonVars <- c("Year", "iYear", "Country")
unadjData_PT <- historicalData[ , as.character(unique(c(unadjVars_PT, commonVars)))] %>% filter(Country=="PT")
unadjData_UK <- historicalData[ , as.character(unique(c(unadjVars_UK, commonVars)))] %>% filter(Country=="UK")
unadjData <- plyr::rbind.fill(unadjData_PT, unadjData_UK)
unadjData$adjusted <- "Unadjusted"

adjVars_PT <- c(y_PT, k_PT_adj, l_PT_adj, e_PT_adj)
adjVars_UK <- c(y_UK, k_UK_adj, l_UK_adj, e_UK_adj)
adjData_PT <- historicalData[ , as.character(unique(c(adjVars_PT, commonVars)))] %>% filter(Country=="PT")
adjData_UK <- historicalData[ , as.character(unique(c(adjVars_UK, commonVars)))] %>% filter(Country=="UK")
adjData <- plyr::rbind.fill(adjData_PT, adjData_UK)
adjData$adjusted <- "Quality-adjusted"

trimmedData <- plyr::rbind.fill(unadjData, adjData)

mdata <- melt(trimmedData, 
              measure.vars = unique(as.character(c(unadjVars_PT, unadjVars_UK, adjVars_PT, adjVars_UK))), 
              variable.name = "variable",
              value.name = "value")

# Relevel factors before plotting
mdata$Country <- relevelFactor(as.factor(mdata$Country), c("PT", "UK"))
mdata$adjusted <- relevelFactor(as.factor(mdata$adjusted), c("Unadjusted", "Quality-adjusted"))
mdata$variable <- relevelFactor(as.factor(mdata$variable), unique(as.character(
  c(y_PT, k_PT_unadj, l_PT_unadj, e_PT_unadj, k_PT_adj, l_PT_adj, e_PT_adj,
    y_UK, k_UK_unadj, l_UK_unadj, e_UK_unadj, k_UK_adj, l_UK_adj, e_UK_adj))))
