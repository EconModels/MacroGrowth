...
# This block of code sets up the time-series variables 
# to be used for each country.
# Adding this level of abstraction makes it easier 
# for us to change variables later.
# It also makes it easier to see what is going on 
# in each formula throughout the document.
# Use of substitute establishes an R "name"
# that can be a component of a formula.

vars <-   
  list( 
    PT = list(
      response = list(unadjusted = "iGDP", adjusted = "iGDP"),
      capital = list(unadjusted ="iKstkS.L", adjusted = "iKservS.L"),
      labor = list(unadjusted = "iL", adjusted = "ihLest"),
      energy = list(unadjusted = "iXpMP", adjusted = "iUMP"),
      time = list(unadjusted = "iYear", adjusted = "iYear")
    ), 
    UK = list(
      response = list(unadjusted = "iGDP", adjusted = "iGDP"),
      capital = list(unadjusted = "iKstkO.WwithRD", adjusted = "iKservO.WwithRD"), 
      labor = list(unadjusted = "iL", adjusted = "ihLest"),
      energy = list(unadjusted = "iXp", adjusted = "iU"),
      time = list(unadjusted = "iYear", adjusted = "iYear")
    )
  )

# # Economic output
# y_PT <- substitute(iGDP)
# y_UK <- substitute(iGDP)
# # Portugal undajusted inputs
# k_PT_unadj <- substitute(iKstkS.L)
# l_PT_unadj <- substitute(iL)
# e_PT_unadj <- substitute(iXpMP)
# # Portugal quality-adjusted inputs
# k_PT_adj <- substitute(iKservS.L)
# l_PT_adj <- substitute(ihLest)
# e_PT_adj <- substitute(iUMP)
# # UK unadjusted inputs
# k_UK_unadj <- substitute(iKstkO.WwithRD)
# l_UK_unadj <- substitute(iL)
# e_UK_unadj <- substitute(iXp)
# # UK quality-adjusted inputs
# k_UK_adj <- substitute(iKservO.WwithRD)
# l_UK_adj <- substitute(ihLest)
# e_UK_adj <- substitute(iU)
# # Time variables
# t_PT <- substitute(iYear)
# t_UK <- substitute(iYear)

# vars <- list()
# # Economic output
# vars$y.PT <- substitute(iGDP)
# vars$y.UK <- substitute(iGDP)
# # Portugal undajusted inputs
# vars$k.PT.unadj <- substitute(iKstkS.L)
# vars$l.PT.unadj <- substitute(iL)
# vars$e.PT.unadj <- substitute(iXpMP)
# # Portugal quality-adjusted inputs
# vars$k.PT.adj <- substitute(iKservS.L)
# vars$l.PT.adj <- substitute(ihLest)
# vars$e.PT.adj <- substitute(iUMP)
# # UK unadjusted inputs
# vars$k.UK.unadj <- substitute(iKstkO.WwithRD)
# vars$l.UK.unadj <- substitute(iL)
# vars$e.UK.unadj <- substitute(iXp)
# # UK quality-adjusted inputs
# vars$k.UK.adj <- substitute(iKservO.WwithRD)
# vars$l.UK.adj <- substitute(ihLest)
# vars$e.UK.adj <- substitute(iU)
# # Time variables
# vars$t.PT <- substitute(iYear)
# vars$t.UK <- substitute(iYear)
...
# t <- list()
# # Economic output
# y$PT <- substitute(iGDP)
# y$UK <- substitute(iGDP)
# # Portugal undajusted inputs
# k$PT$unadj <- substitute(iKstkS.L)
# l$PT$unadj <- substitute(iL)
# e$PT$unadj <- substitute(iXpMP)
# # Portugal quality-adjusted inputs
# k$PT$adj <- substitute(iKservS.L)
# l$PT$adj <- substitute(ihLest)
# e$PT$adj <- substitute(iUMP)
# # UK unadjusted inputs
# k$UK$unadj <- substitute(iKstkO.WwithRD)
# l$UK$unadj <- substitute(iL)
# e$UK$unadj <- substitute(iXp)
# # UK quality-adjusted inputs
# k$UK$adj <- substitute(iKservO.WwithRD)
# l$UK$adj <- substitute(ihLest)
# e$UK$adj <- substitute(iU)
# # Time variables
# t$PT <- substitute(iYear)
# t$UK <- substitute(iYear)
# 
# foo <- function(country = c("UK", "PT"), adjusted=TRUE, energy=TRUE, envir=parent.frame()) {
#   country <- match.arg(country)
#   type <- if (adjusted) "adj" else "unadj"
#   res <- as.formula(
#       paste(y[[country]], 
#             paste(k[[country]][[type]], l[[country]][[type]], if (energy) e[[country]][[type]] else NULL, t[[country]], sep="+"), 
#             sep="~")
#     )
...
# 
# 
# for (country in c("PT", "UK")) {
#   for (adjusted in c(TRUE, FALSE)) {
#     for (energy in c(TRUE, FALSE)) {
#       print(foo(country = country, adjusted = adjusted, energy=energy))
#     }
#   }
# }
# 
...
# 
# countries <- c(substitute(PT), substitute(UK))
# adjusteds <- c(substitute(unadj), substitute(adj))
# energies <- c(substitute(noE), substitute(withE))
# f <- list()
# for (country in c(substitute(PT), substitute(UK))){
#   for (adjusted in adjusteds){
#     for (energy in energies){
#       if (energy == substitute(noE)){
#         eval(substitute(f$ctry$adjst$ener <- y$ctry ~ k$ctry$adjst + l$ctry$adjst + t$ctry, 
#                         list(ctry=country, adjst=adjusted, ener=energy)))
#       } else if (energy == substitute(withE)){
#         eval(substitute(f$ctry$adjst$ener <- y$ctry ~ k$ctry$adjst + l$ctry$adjst + e$ctry$adjst + t$ctry, 
#                         list(ctry=country, adjst=adjusted, ener=energy)))
#       } else {
#         stop(paste("Unknown energy", energy, "in loop for f"))
#       }
#     }
#   }
# }
...
dataPT <- transform(dataPT, lPerGDP.unadj = eval(k_PT_unadj)/iGDP)
...
# f <- NULL
# f$PT$unadj$noE <- as.formula(
#   substitute( y ~ k + l + t,
#               list( y = y_PT, k = k_PT_unadj, 
#                     l = l_PT_unadj, t = t_PT)))
# f$PT$unadj$withE <- as.formula(
#   substitute( y ~ k + l + e + t,
#               list( y = y_PT, k = k_PT_unadj,
#                     l = l_PT_unadj, e = e_PT_unadj, t = t_PT)))
# f$PT$adj$noE <- as.formula(
#   substitute( y ~ k + l + t,
#               list( y = y_PT, k = k_PT_adj,
#                     l = l_PT_adj, t = t_PT)))
# f$PT$adj$withE <- as.formula(
#   substitute( y ~ k + l + e + t,
#               list( y = y_PT, k = k_PT_adj,
#                     l = l_PT_adj, e = e_PT_adj, t = t_PT)))
# f$UK$unadj$noE <- as.formula(
#   substitute( y ~ k + l + t,
#               list( y = y_UK, k = k_UK_unadj,
#                     l = l_UK_unadj, t = t_UK)))
# f$UK$unadj$withE <- as.formula(
#   substitute( y ~ k + l + e + t,
#               list( y = y_UK, k = k_UK_unadj,
#                     l = l_UK_unadj, e = e_UK_unadj, t = t_UK)))
# f$UK$adj$noE <- as.formula(
#   substitute( y ~ k + l + t,
#               list( y = y_UK, k = k_UK_adj,
#                     l = l_UK_adj, t = t_UK)))
# f$UK$adj$withE <- as.formula(
#   substitute( y ~ k + l + e + t,
#               list( y = y_UK, k = k_UK_adj,
#                     l = l_UK_adj, e = e_UK_adj, t = t_UK)))

makeFormula <- function(time.series, country, adjusted, energy, envir=parent.frame()) {
  country <- match.arg(country, names(time.series))
  if (! adjusted %in% c(FALSE, TRUE)){
    stop("adjusted must be a boolean")
  }
  if (! energy %in% c(FALSE, TRUE)){
    stop("energy must be a boolean")
  }
  type <- if (adjusted) "adjusted" else "unadjusted"
  res <- as.formula( 
    gsub( "\\+ *\\+", "+ ", 
          paste(time.series[[country]][["response"]][[type]], 
                paste(time.series[[country]][["capital"]][[type]], 
                      time.series[[country]][["labor"]][[type]], 
                      if (energy) {
                        time.series[[country]][["energy"]][[type]]
                      } else {
                        NULL
                      }, 
                      time.series[[country]][["time"]][[type]], sep="+"), 
                sep="~")
    )
  )
  environment(res) <- envir
...
for (country in countries) {
  for (adjusted in c(FALSE, TRUE)) {
    for (energy in c(FALSE, TRUE)) {
      newf <- makeFormula(vars, country = country, adjusted = adjusted, energy=energy)
      namef <- paste(country, if(adjusted) "adj" else "unadj", if(energy) "withE" else "noE", sep=".")
      newnames <- c(names(f), namef)
      f <- c(f, newf)
      names(f) <- newnames
    }
  }
...
\label{tab:unadjusted_variables} 
  \begin{center}
    \begin{tabular}{r l l} 
      ...
      \midrule
      Output      & \Sexpr{vars$PT$response$unadjusted}       & \Sexpr{vars$UK$response$unadjusted}       \\
      Capital     & \Sexpr{vars$PT$capital$unadjusted} & \Sexpr{vars$UK$capital$unadjusted} \\
      Labor       & \Sexpr{vars$PT$labor$unadjusted} & \Sexpr{vars$UK$labor$unadjusted} \\
      Energy      & \Sexpr{vars$PT$energy$unadjusted} & \Sexpr{vars$UK$energy$unadjusted} \\
      \bottomrule
    \end{tabular}
  \end{center}
\end{table}
...
\label{tab:quality_adjusted_variables} 
  \begin{center}
    \begin{tabular}{r l l} 
      ...
      Variable    & Portugal         & UK               \\
      \midrule
      Output      & \Sexpr{vars$PT$response$adjusted}       & \Sexpr{vars$UK$response$adjusted}       \\
      Capital     & \Sexpr{vars$PT$capital$adjusted} & \Sexpr{vars$UK$capital$adjusted} \\
      Labor       & \Sexpr{vars$PT$labor$adjusted} & \Sexpr{vars$UK$labor$adjusted} \\
      Energy      & \Sexpr{vars$PT$energy$adjusted} & \Sexpr{vars$UK$energy$adjusted} \\
      \bottomrule
    \end{tabular}
  \end{center}
\end{table}
...
unadjVars_PT <- c(vars$PT$response$unadjusted, 
                  vars$PT$capital$unadjusted, 
                  vars$PT$labor$unadjusted, 
                  vars$PT$energy$unadjusted)
unadjVars_UK <- c(vars$UK$response$unadjusted, 
                  vars$UK$capital$unadjusted, 
                  vars$UK$labor$unadjusted, 
                  vars$UK$energy$unadjusted)
...
adjVars_PT <- c(vars$PT$response$adjusted, 
                vars$PT$capital$adjusted, 
                vars$PT$labor$adjusted, 
                vars$PT$energy$adjusted)
adjVars_UK <- c(vars$UK$response$adjusted, 
                vars$UK$capital$adjusted, 
                vars$UK$labor$adjusted, 
                vars$UK$energy$adjusted)
...
mdata$variable <- relevelFactor(as.factor(mdata$variable), unique(
  c(vars$PT$response$unadjusted, vars$PT$capital$unadjusted, vars$PT$labor$unadjusted, vars$PT$energy$unadjusted, 
    vars$PT$response$adjusted,   vars$PT$capital$adjusted,   vars$PT$labor$adjusted,   vars$PT$energy$adjusted, 
    vars$UK$response$unadjusted, vars$UK$capital$unadjusted, vars$UK$labor$unadjusted, vars$UK$energy$unadjusted, 
    vars$UK$response$adjusted,   vars$UK$capital$adjusted,   vars$UK$labor$adjusted,   vars$UK$energy$adjusted)))
...
mdata_preeconometric <- melt(historicalData, 
                             measure.vars = c("kPerGDP_unadj", "lPerGDP_unadj", "ePerGDP_unadj",
                                              "kPerGDP_adj", "lPerGDP_adj", "ePerGDP_adj"),
                             variable.name = "variable",
                             value.name = "value")
...
mdata_preeconometric$adjusted <- lapply(mdata_preeconometric$variable, function(x) {
  suffix <- unlist(strsplit(as.character(x), split="[_]")) [2]
  if (suffix == "unadj"){
    return("Unadjusted")
  } else if (suffix == "adj"){
