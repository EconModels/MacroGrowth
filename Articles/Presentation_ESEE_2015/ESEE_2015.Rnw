...
ggplot() +
  geom_point(data=filter(AllCoeffs, model=="CD"), mapping=aes(x=Dmse, y=Dlambda, color=energy, shape=cst)) +
  facet_grid(Country ~ flavor) +
...
ggplot() +
  geom_point(data=filter(AllCoeffs, model=="CD"), mapping=aes(x=percDmse, y=percDlambda, color=energy, shape=cst)) +
  facet_grid(Country ~ flavor) +
  scale_shape_manual(values=c(1, 16)) + 
...
ggplot() +
  geom_point(data=filter(AllCoeffs, model=="CES" & cst=="Reject CST"), 
             mapping=aes(x=Dmse, y=Dlambda, color=energy, shape=nest)) +
  geom_point(data=filter(AllCoeffs, model=="CES" & cst=="Adhere to CST"), 
             mapping=aes(x=Dmse, y=Dlambda, color=energy), shape=1) +
  facet_grid(Country ~ flavor) +
  xlab(expression(Delta * mse)) + 
  scale_x_continuous(breaks=seq(-0.005, 0, 0.005)) +  
  ylab(expression(Delta * lambda)) +
...
ggplot() +
  geom_point(data=filter(AllCoeffs, model=="CES" & cst=="Reject CST"), 
             mapping=aes(x=Dmse, y=percDlambda, color=energy, shape=nest)) +
  geom_point(data=filter(AllCoeffs, model=="CES" & cst=="Adhere to CST"), 
             mapping=aes(x=Dmse, y=percDlambda, color=energy), shape=1) + 
  facet_grid(Country ~ flavor) +
  xlab(expression(Delta * mse)) + 
  scale_x_continuous(breaks=seq(-0.005, 0, 0.005)) +  
  ylab(expression(paste(Delta * lambda, " [%]"))) +
