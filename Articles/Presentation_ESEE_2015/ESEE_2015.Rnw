...
histDataForGraph <- leaf_apply(formulas, class = "formula", f=function(form, name){
  ...
  if (flavor == "unadjusted"){
    mtrimmed_historical_data$flavor <- "Unadjusted"
  } else if (flavor == "adjusted") {
    mtrimmed_historical_data$flavor <- "Quality-adjusted"
  } else {
    stop(paste("Unknown flavor", flavor, "in histDataForGraph."))
  }
