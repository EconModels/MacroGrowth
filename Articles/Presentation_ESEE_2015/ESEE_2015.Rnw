...
if (!rerunModels){
  ...
  for (country in countries){
    ...
    # Cobb-Douglas
    cdModCST <- lm(log(eval(yVar)) - alphaCST*log(eval(kVar)) - betaCST*log(eval(lVar)) ~ eval(tVar), 
...
fittedAndResidData <- function(model, id){
  ...
  df[[summands(model$formula, right = FALSE)]] <- response(model)
...
ggplot() +
  ...
  guides(shape=FALSE) +
  geom_point(data=filter(AllCoeffs, cst==TRUE & model=="CD"), mapping=aes(x=Dmse, y=Dlambda, color=energy, shape=cst)) +
  facet_grid(Country ~ flavor) +
...
ggplot() +
  ...
  scale_shape(solid=FALSE) +
  guides(shape=FALSE) +
  geom_point(data=filter(AllCoeffs, cst==TRUE & model=="CD"), mapping=aes(x=Dmse, y=percDlambda, color=energy, shape=cst)) +
  facet_grid(Country ~ flavor) +
  xlab(expression(Delta * mse)) + 
  scale_x_continuous(breaks=seq(-0.005, 0, 0.005)) +  
