...
dataPT <- transform(dataPT, lPerGDP_unadj = eval(k_PT_unadj)/iGDP)
dataPT <- transform(dataPT, ePerGDP_unadj = eval(e_PT_unadj)/iGDP)
dataUK <- transform(dataUK, kPerGDP_unadj = eval(k_UK_unadj)/iGDP)
dataUK <- transform(dataUK, lPerGDP_unadj = eval(k_UK_unadj)/iGDP)
...
dataPT <- transform(dataPT, lPerGDP_adj = eval(k_PT_adj)/iGDP)
dataPT <- transform(dataPT, ePerGDP_adj = eval(e_PT_adj)/iGDP)
dataUK <- transform(dataUK, kPerGDP_adj = eval(k_UK_adj)/iGDP)
dataUK <- transform(dataUK, lPerGDP_adj = eval(k_UK_adj)/iGDP)
...
<<Fitted_Data>>=
extract_fitted_comparison <- function(mod, id){
...
fitted_data <- leaf_apply(models, extract_fitted_comparison, class=c("CDEmodel", "cesEst", "LINEXmodel"))
# Create a data frame.
fitted_data <- do.call(rbind, fitted_data)
# Melt fitted_data so it can be used for plotting
# Relevel factors before plotting
fitted_data$Country <- relevelFactor(as.factor(fitted_data$Country), c("PT", "UK"))
fitted_data$adjusted <- relevelFactor(as.factor(fitted_data$adjusted), c("Unadjusted", "Quality-adjusted"))
fitted_data$energy <- relevelFactor(as.factor(fitted_data$energy), c("Without energy", "With Energy"))
@
...
This section shows graphs of fits to historical data.
...
<<Graphs CD, fig.pos="H", fig.align='center', fig.width=6.5, fig.height=4, fig.cap="Cobb-Douglas models. Solid line is historical GDP.">>=
fittedGraph(filter(fitted_data, model=="CD"))
@

<<Graphs CESkle, fig.pos="H", fig.align='center', fig.width=6.5, fig.height=4, fig.cap="CES models with ($kl$)$e$ nesting. Solid line is historical GDP.">>=
fittedGraph(filter(fitted_data, model=="CESkle" | model=="CESkl"))
@

<<Graphs CESlek, fig.pos="H", fig.align='center', fig.width=6.5, fig.height=4, fig.cap="CES models with ($le$)$k$ nesting. Solid line is historical GDP.">>=
fittedGraph(filter(fitted_data, model=="CESlek" | model=="CESkl"))
@

<<Graphs CESekl, fig.pos="H", fig.align='center', fig.width=6.5, fig.height=4, fig.cap="CES models with ($ek$)$l$ nesting. Solid line is historical GDP.">>=
fittedGraph(filter(fitted_data, model=="CESekl" | model=="CESkl"))
@
