...
\begin{equation} \label{eq:CD}
  y = \theta A \, {x_1}^{\alpha_1} \, {x_2}^{\alpha_2} \, {x_3}^{\alpha_3} \; ; \; A \equiv \mathrm{e}^{\lambda(t-t_0)},
\end{equation}
%
where 
$y \equiv Y/Y_{0}$,
...
$L$ is labor expressed in workers or work-hours/year, and
0 subscripts indicate values at an initial year.
The parameters $\alpha_k$ and $\alpha_l$ are output elasticities
that indicate the marginal productivity of 
capital and labor in the economy, respectively.
Typically, the constraint $\alpha_k + \alpha_l = 1$ is assumed, 
indicating constant returns to scale.%
  ...
  ($\theta$, $\lambda$, $\alpha_k$, $\alpha_l$, 
  later $\alpha_e$, $\delta_1$, $\delta$, $\sigma_1$, and $\sigma$).} 
...
\begin{equation} \label{eq:CDe}
  y = \theta A k^\alpha_k l^\alpha_l e^\alpha_e \; ; \; A \equiv \mathrm{e}^{\lambda(t-t_0)},
\end{equation}
%
...
of production: $\alpha_k + \alpha_l + \alpha_e = 1$.
...
without energy is given in Equation~\ref{eq:CESkl}.
%
...
\end{equation}
%
The generic CES production function
can be given as
%
...
$\theta$ is a fitting parameter that accounts for an atypical first year. 
