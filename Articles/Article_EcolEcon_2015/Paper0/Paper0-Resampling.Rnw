...
\title{Parameter precision in economic growth models inspired by Ecological Economics}
...
<<replay, include=TRUE, echo=FALSE>>=
<<Graphs_CESlek-fitted>>
@
...
The book entitled
\emph{The Economic Growth Engine: How Energy and Work Drive Material Prosperity} 
\citep{Ayres:2010ug} 
was a watershed moment in the understanding 
of interactions between energy and the economy.
It represents the culmination of a decade of fruitful inquiry by Ayres and Warr, 
whose greatest insights
(that energy matters in economic growth,
that ``useful work'' dissipated at the point of production
is an essential factor of production, and
that the ``Cost Share Principle'' from neoclassical economics should be rejected)
were inspired by Ecological Economics in one way or another.
...
Ideas for an Ecological Economics frame for the paper:

%
\begin{itemize}

  \item EE ideas (work of Kummel and Ayres and Warr, especially) 
        inspire us to include energy, 
        reject the CSP, and 
        quality-adjust energy. 

  \item Another insight from Ayres and Warr (and Kummel before them) 
        is that output elasticities can be functions of time. 
        Linex gives this explicitly. 
        But, so does CES.
        This means moving away from Cobb-Douglas toward CES.
        
  \item Also, inspired by Ayres and Warr, 
        we don't want to assume elasticity of substitution 
        of unity as CD does.
        (What does Linex give for the elasticity of substitution between factors of production?)

  \item But, questions remain from Ayres and Warr's work.
  
  \begin{itemize}
    
    \item Ayres and Warr used a non-standard production function.
          Will their insights hold true for standard production functions like Cobb-Douglas and CES?
          
    \item Ayres and Warr quality-adjusted only one factor of production, energy, by employing useful work.  
          What happens when all factors of production, including capital and labor, are quality-adjusted?
    
    \item Ayres and Warr expended minimal effort at statistical precision of estimated parameters. 
          How robust are their findings, from a statistical point of view?
    
  \end{itemize}

  \item We want to explore the last point above (statistical precision of estimated parameters)
        in the context of a standard production function (CES).
        
  \item However, when moving away from CD and Linex toward CES, 
        we leave the world of linear fitting behind.
        
  \item Non-linear fitting has several complications in the fitting process.
        
  \item Furthermore, standard approaches to determining parameter precision 
        for linear and non-linear fitting
        assume normal distributions for model parameters. 
        In the non-linear world of CES, 
        those assumptions aren't appropriate. 
        Need new approaches in the non-CD world.
        
  \begin{itemize}
    
    \item Need advanced techniques for fitting.
    
    \item We use customized version of Henningsen and Henningsen for fitting.

    \item Need advanced techniques for estimating precision of estimated parameters.
    
    \item We use resampling to estimate precision of estimated parameters.
    
  \end{itemize}

\end{itemize}
%
This paper explores and justifies those choices by:
%
\begin{enumerate}

  \item comparing standard techniques and our approaches for estimating parameter precision
        for the Cobb-Douglas production function, and 

  \item applying our advanced techniques for parameter estimation
        (including precision)
        to the CES production production function.

\end{enumerate}
%
for the Portuguese economy from 1960--2010.
...
is the canonical representation of indexed economic output ($y$) 
as a function of indexed factors of production~($x_1$, $x_2$, and $x_3$) and 
an additional exogenous, time-dependent term~($A$).
This production function can be expressed as
%
\begin{equation} \label{eq:CD}
  y = \theta A \, {x_1}^{\alpha_1} \, {x_2}^{\alpha_2} \, {x_3}^{\alpha_3} \; ; \; A \equiv \mathrm{e}^{\lambda t},
\end{equation}
%
where 
$y \equiv Y/Y_{0}$,
...
$\lambda$ is the so-called Solow residual,
$t$ is time measured in years,
$x_1 = k \equiv K/K_{0}$, 
$x_2 = l \equiv L/L_{0}$, 
$x_3 = e \equiv E/E_{0}$, 
$Y$ is economic output represented by GDP, 
$K$ is capital stock expressed in currency units, 
$L$ is labor expressed in workers or work-hours/year, 
$E$ is energy expressed in units of energy per time, typically TJ/year, and
0 subscripts indicate values at an initial year.
All historical data~($y$, $k$, $l$, and $e$) are indexed (by ratio)
relative to an initial year. 
Time~($t$) is indexed (by difference) relative to an initial year.
Due to indexing, $\theta$ is expected to be near unity,
but no other parameters are affected.%
  \footnote{Dimensionless, indexed quantities are represented by lower-case symbols 
  ($y$, $x$, $k$, $l$, and $e$), 
  and dimensional quantities are represented by upper-case symbols 
  ($Y$, $K$, $L$, and $E$). 
  Model parameters are represented by Greek letters
  ($\theta$, $\lambda$, $\alpha_1$, $\alpha_2$, $\alpha_3$, and
  later $\delta_1$, $\delta$, $\sigma_1$, and $\sigma$).} 

The parameters $\alpha_1$, $\alpha_2$, and $\alpha_3$ are output elasticities
that indicate the marginal productivity of 
$x_1$, $x_2$, and $x_3$ in the economy, respectively.
Typically, the constraint $\alpha_1 + \alpha_2 + \alpha_3 = 1$ is assumed, 
...
\begin{equation} \label{eq:CESgeneric}
  y = \theta \: A \: \left\{\delta B_1^{\rho/\rho_1} 
      + (1-\delta) x_3^{-\rho} \right\}^{-1/\rho}; 
      A \equiv \mathrm{e}^{\lambda t},
\end{equation}
%
where
$B_1 \equiv \delta_1 x_1^{-\rho_1} + (1-\delta_1) x_2^{-\rho_1}$, and
$x_1$, $x_2$, and $x_3$ are factors of production
and permutations of capital~($k$), labor~($l$), and energy~($e$).
The $(kl)e$ nesting has $x_1 = k$, $x_2 = l$, and $x_3 = e$; 
the $(le)k$ nesting has $x_1 = l$, $x_2 = e$, and $x_3 = k$; and
the $(ek)l$ nesting has $x_1 = e$, $x_2 = k$, and $x_3 = l$.

For the CES function, 
output elasticities~($\alpha_1$, $\alpha_2$, and $\alpha_3$) are not constant;
rather, they vary with factors of production ($x_1$, $x_2$, and $x_3$) over time.
%
\begin{equation} \label{eq:alpha_1}
  \alpha_1 \equiv
  \frac{ \frac{\partial y}{\partial x_1} }
          { \frac{y}{x_1} } 
  = 
  \frac{ \delta  \delta _1 x_1^{-\rho _1} B_1^{ ( \frac{\rho}{\rho_1}-1 ) }  }
  { \delta B_1^{ ( \frac{\rho }{\rho_1} ) } + (1 - \delta) x_3^{-\rho }}
\end{equation}
%

\begin{equation} \label{eq:alpha_2}
  \alpha_2 \equiv \frac{ \frac{\partial y}{\partial x_2} }{\frac{y}{x_2}} =
  \frac{ \delta  \left(1-\delta _1 \right) x_2^{-\rho _1} B_1^{ ( \frac{\rho }{\rho _1}-1 ) } }
  { \delta B_1^{ ( \frac{\rho }{\rho_1} ) } + (1 - \delta) x_3^{-\rho }}
\end{equation}
%
\begin{equation} \label{eq:alpha_3}
  \alpha_3 \equiv \frac{ \frac{\partial y}{\partial x_3} }{ \frac{y}{x_3} } =  
      \frac{1 - \delta}{\delta  x_3^{\rho } B_1^{(\frac{\rho }{\rho_1} ) } + 1 - \delta}
\end{equation}
%
...
Similar to the output elasticities, 
the elasticity of substitution between factors of production $x_1$ and $x_2$~($\sigma_{1,2}$) 
is time-varying and given by the Allen-Uzawa elasticity of substitution:
%
\begin{equation} \label{eq:sigma_1_2}
  \sigma_{1,2} = \frac{ (1 - \rho_1)^{-1} - (1-\rho)^{-1}}{\delta \left( \frac{y}{B_1^{-\frac{1}{\rho_1}}} \right) ^ {1 + \rho}}
                      + (1 + \rho)^{-1} \; .
\end{equation}
%

**** Add Hicks-McFadden elasticity of substitution here. ****
...
%------------------------------
\subsubsection{Cobb-Douglas model} 
\label{sec:param_estimation_CD}
%------------------------------

To estimate the parameters $\theta$, $\lambda$, $\alpha_1$, $\alpha_2$, and $\alpha_3$ 
in the Cobb-Douglas model, 
we reparameterized Equation~\ref{eq:CD} as
%
\begin{equation} \label{eq:CDe_AB_reparam}
  y = \theta \, \mathrm{e}^{\lambda t} \, x_1^{\alpha_1} \, x_2^{\alpha_2} \, x_3^{1 - \alpha_1 - \alpha_2}
\end{equation}
%
ensuring that $\alpha_1 + \alpha_2 + \alpha_3 = 1$,
thereby providing constant returns to scale.
$\alpha_3$ was recovered with $\alpha_3 = 1 - \alpha_1 - \alpha_2$.
The \texttt{R} \citep{R} function \texttt{lm} was used to estimate 
values of $\lambda$, $\theta$, $\alpha_1$, and $\alpha_2$ in log-transform space.
If the fitted value for $\alpha_1$ or $\alpha_2$ fell outside the interval $[0,1]$,
...
minimum sum of squared errors~(\sse) as the winning model:
%
...
\begin{equation} \label{eq:residual}
  r_i \equiv \ln (y_i / \hat{y}_i)
\end{equation}
%
is the $i^{\mathrm{th}}$ residual (all models assume multiplicative errors),
and
...
for capital and labor ($\hat{\alpha}_1 = 0.3$ and $\hat{\alpha}_2 = 0.7$).
Under these conditions, $\hat{\alpha}_3$ is assumed to be $0$.%
  \authNote{Need to fix the discussion of $\alpha$s and the CSP.}


%------------------------------
\subsubsection{CES model} 
\label{sec:param_estimation_CES}
%------------------------------
...
wherein the objective is minimization of the sum of squared errors~(Equation~\ref{eq:sse}).
We estimate parameters using the \texttt{R}~\citep{R} package \texttt{micEconCES}
...
Next, a gradient search (using \texttt{PORT} and \texttt{L-BFGS-B} algorithms available in \texttt{micEconCES})
is attempted wherein all parameters
($\theta$, $\lambda$, $\delta_1$, $\delta$, $\rho_1$, and $\rho$)
are allowed to float.
The starting values for parameters are taken from
...
and \texttt{PORT} does not for the same historical data
...
is a repeat of Equation~\ref{eq:CESgeneric}.
...
indicating that many parameters are unknowable
...
it would report values for $\rho_1$ and $\rho$
...
\label{tab:CES_boundary_models}
  \begin{center}
    \begin{tabular}{r c c c c l}
      \toprule
      & $\delta_1$     & $\rho_1$   & $\delta$   & $\rho$   & Boundary model \\
      \midrule
      1. & 1           & ---        & 1          & ---      & $y = \theta A x_1$
      \\
      2. & 0           & ---        & 1          & ---      & $y = \theta A x_2$
      ...
      \\
      \midrule
      4. & ---         & $\infty$   & 1          & ---      & $y = \theta A \min
      (x_1, \, x_2)$ \\
      5. & 1           & ---        & ---        & $\infty$ & $y = \theta A \min
      (x_1, \, x_3)$ \\
      6. & 0           & ---        & ---        & $\infty$ & $y = \theta A \min
      (x_2, \, x_3)$ \\
%
      7. &             & $-1$       & 1          & ---
            & $y = \theta A \left[ \delta_1 x_1 + (1 - \delta_1) x_2 \right]$ \\
...
Determining the precision of estimates for 
Cobb-Douglas parameters
($\hat\theta$, $\hat\lambda$, $\hat\alpha_1$, $\hat\alpha_2$, and $\hat\alpha_3$)
and 
CES parameters
($\hat\theta$, $\hat\lambda$, $\hat\delta_1$, $\hat\delta$, $\hat\rho_1$, 
and $\hat\rho$, but also time series for
$\hat\alpha_1$, $\hat\alpha_2$, $\hat\alpha_3$, 
$\hat\sigma_{1,2}$, and $\hat\sigma_{(1,2)3}$)
is important but challenging.
It is quite possible for substantial changes in a model parameter
...
In an investigation of the marginal productivity of energy 
in the $(kl)e$ nesting of the CES model, for example,
we may consider the null hypothesis that $\alpha_3 = 0$.  
This null hypothesis is equivalent to $\delta = 1$.
While there is an asymptotic theory for this case as well,%
  \authNote{Randy: can you provide a reference for the asymptotic theory? ---MKH}
...
distribution.
\authNote{We should point out that we are NOT doing bootstrap aggregating,
  whose purpose is to obtain better initial fits to data. 
  See \url{https://en.wikipedia.org/wiki/Bootstrap_aggregating}.
}
...
response~($\hat{y}$) the product of a residual from the original function (Cobb-Douglas or CES) fit
and
...
\authNote{Randy: edit the following equations to reflect the fact that
we're fitting in log space? ---MKH}

\begin{equation} \label{eq:resampled_y}
  \tilde Y_i' = \hat y_i \pm r_j \; ,
\end{equation}
%
where
%
\authNote{Do we need this equation for $r_i$ here? We already have Equation~\ref{eq:residual}.}%
%
\begin{equation} \label{eq:residual_2}
  r_i = y_i - \hat y_i \; .
\end{equation}
%
Both the sign~($\pm$) and the index
($j$,~typically different from $i$)
of the residual~($r_j$) in Equation~\ref{eq:resampled_y}
are chosen at random (with replacement).
We repeated the resampling process 1000 times.
...
When the distribution of these resample parameters 
(the ``bootstrap resample distribution'') 
is highly variable,
it is an indication
...
a poor model fit to historical data,
...
We choose the resampling approach to estimating parameter precision
instead of the more-common technique
...
For example, a result of $\alpha_1 = 0.25 \pm 0.3$ is nonsensical,
because $ \alpha_1 \in [0, 1]$.
Second, true confidence intervals for constrained parameters are often asymmetric
(e.g., $\alpha_1 = 0.2 + 0.1, -0.05$),
...
Looking at boostrap distribution of a parameter.
<<>>=
ggplot( aes(x = lambda, color = method), data = resampledCoeffsCD) + geom_line(stat="density") + facet_grid( foo ~ Country, scales = "free" ) 
mod <- resampleModelsCD[["PT.adjusted.noE.CD"]][["orig"]]
coef(summary(mod))
...
Teaser at the end that variability varies from one country to the next.
