...
<<CES_sigma_table_data>>=
# This chunk does some preliminary processing
@
...
sigma_inner_data <- filter(table_data, Model=="CES") %>% 
  select(Country, flavor, Energy, Nest, CSP, sigma_12, sigma_23, sigma_13)
...
myXTable(sigma_inner_data,
         caption="Factor substitution~($\\sigma$) for inner nests of CES models.",
         label=paste0("tab:", opts_current$get("label")), 
         digits=6)
@
...
sigma_outer_data <- filter(table_data, Model=="CES") %>% 
  select(Country, flavor, Energy, Nest, CSP, sigma_12_3, sigma_23_1, sigma_13_2)
...
myXTable(sigma_outer_data,
         caption="Factor substitution~($\\sigma$) for outer nests of CES models.",
         label=paste0("tab:", opts_current$get("label")), 
         digits=6)
@
