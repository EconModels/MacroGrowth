...
\begin{equation} \label{eq:delta_lambda}
  \Delta \lambda \equiv \left| \lambda \right| - \left| \lambda_{ref} \right|
\end{equation}
%
and
%
...
\end{equation}
%
where~$\lambda_{ref}$ and $mse_{ref}$ are the values 
of the Solow residual and $mse$ for the reference model, respectively.
On a percentage basis,
%
\begin{equation} \label{eq:delta_lambda_perc}
  \Delta \lambda \, [\%] = \frac{100 \, \Delta \lambda}{\left| \lambda_{ref} \right|} 
            = \frac{100 \, \left| \lambda \right|}{\left| \lambda_{ref} \right|} - 1
\end{equation}
%
...
the factor shares for all CES modeling decisions.
%
...
myXTable(filter(AllCoeffs, (model=="CES" & (nest=="kl" | nest=="kle")) | model=="exp") %>% 
...
myXTable(filter(AllCoeffs, (model=="CES" & (nest=="kl" | nest=="lek")) | model=="exp") %>% 
           select(Country, model, flavor, energy, nest, cst, Dlambda, Dmse),
...
myXTable(filter(AllCoeffs, (model=="CES" & (nest=="kl" | nest=="ekl")) | model=="exp") %>% 
           select(Country, model, flavor, energy, nest, cst, Dlambda, Dmse),
...
r.t.values <- filter(fitted_and_resid_data, model != "CD") %>% 
...
Dr.t.values <- filter(fitted_and_resid_data, model != "CD") %>% 
  group_by(Country, model, flavor, energy, nest) %>% 
