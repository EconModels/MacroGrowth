...
nResamples <- 10 # Number of resamples
# ********************************************************
opts_chunk$set(
  # ********************************************************
  eval=TRUE,
...
<<Resampling>>=
resampleCoeffsFilename <- "EcolEcon2015kleResampleCoeffs.rds"
resampleModelsFilename <- "EcolEcon2015kleResampleModels.rds"
if (rerunResampling){
  ...
  saveRDS(resampleCoeffs, file = resampleCoeffsFilename)
  # Resample models
  resampleModels <- list(PT=resamplesPT$models, UK=resamplesUK$models)
  saveRDS(resampleModels, file = resampleModelsFilename)
} else {
  resampleCoeffs <- readRDS(resampleCoeffsFilename)
  resampleModels <- readRDS(resampleModelsFilename)
}
@
...
<<Graphs_CESkle_resample_alphas, fig.pos="H", fig.align='center', fig.width=6.5, fig.height=3.2, fig.cap="Resample output elasticities for CES models with $(kl$)$e$ nesting.">>=
ReModels %>% 
  gather(factor, elasticity, oe1, oe2, oe3) %>%
  ggplot(aes(x=Year)) + 
  geom_line( 
...
Here is a table of $\alpha$ values. 
These values represent the initial points of the trajectories. 
We no longer use these values. 
Instead, we use the time-dependent values of alpha.
So, this table goes in the graveyard.
