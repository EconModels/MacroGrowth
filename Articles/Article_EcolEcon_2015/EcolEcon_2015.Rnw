...
AllCoeffsExp <- leaf_apply(modelsExp, class = c("lm"), f = function(modExp, id){
  ...
  return(res)
}) %>% bind_rows() # do.call(plyr::rbind.fill, .)
...
AllCoeffs <- 
  ...
  mutate(
    ...
    # Relevel factors
    Country = relevelFactor(Country, c("PT", "UK")),
    flavor = relevelFactor(flavor, c("Unadjusted", "Quality-adjusted")),
    energy = relevelFactor(energy, c("Without energy", "With energy")),
    model = relevelFactor(model, c("exp", "CD", "CES")),
    nest = relevelFactor(nest, c("kl", "kle", "lek", "ekl")),
    CSP = relevelFactor(CSP, c("Adhere to CSP", "Reject CSP"))
  ) %>% 
...
data_sr_sse <- filter(AllCoeffs, model=="CES")
data_sr_sse$nest <- plyr::mapvalues(data_sr_sse$nest, 
...
<<sigma_graph_function>>=
  
sigma_graph_function <- function(data){
  sigma_graph_data <- select(data, Country, CSP, flavor, energy, nest, 
                             sigma_12, sigma_23, sigma_13, 
                             sigma_12_3, sigma_23_1, sigma_13_2)
  # Change nest to a nicer-looking nest string
  sigma_graph_data$nest <- plyr::mapvalues(sigma_graph_data$nest, 
                                           from=c("kl", "kle", "lek", "ekl"), 
                                           to=c("kl", "(kl)e", "(le)k", "(ek)l"))
  # Now melt the data
  sigma_graph_data <- melt(sigma_graph_data, 
                           measure.vars = c("sigma_12", "sigma_23", "sigma_13", "sigma_12_3", "sigma_23_1", "sigma_13_2"), 
                           value.name = "value", 
                           variable.name = "variable")
  # Create a nicer-looking sigma
#   sigma_graph_data$variable <- plyr::mapvalues(sigma_graph_data$variable,
#                                                from=c("sigma_12", 
#                                                       "sigma_23",
#                                                       "sigma_13",
#                                                       "sigma_12_3",
...
#                                                     'expression(sigma["(ek)"*l])'))
  # Create labels
  sigma_graph_data$label <- paste(sigma_graph_data$CSP, sigma_graph_data$flavor, 
                                  sigma_graph_data$energy, sigma_graph_data$nest, 
                                  sigma_graph_data$variable, sep=", ")
  sigma_graph_data <- na.omit(sigma_graph_data)
...
sigma_graph_function(filter(AllCoeffs, model=="CES", Country=="PT"))
@
...
sigma_graph_function(filter(AllCoeffs, model=="CES", Country=="UK"))
@
...
<<Table_data>>=
table_data <- AllCoeffs 
table_data <- plyr::rename(table_data, c("model" = "Model", 
                                         "energy" = "Energy", 
                                         "nest" = "Nest"))
table_data$Nest <- plyr::mapvalues(table_data$Nest,
                                   from=c("kl", "kle", "lek", "ekl"),
                                   to=c("$kl$", "$(kl)e$", "$(le)k$", "$(ek)l$"))
table_data$CSP <- plyr::mapvalues(table_data$CSP, 
                                  from=c("Adhere to CSP", "Reject CSP"), 
                                  to=c("Adhere", "Reject"))
table_data$Energy <- plyr::mapvalues(table_data$Energy, 
                                     from=c("Without energy", "With energy"), 
                                     to=c("Without", "With"))
table_data$Model <- plyr::mapvalues(table_data$Model, 
                                    from=c("exp"), 
                                    to=c("Ref."))
@
...
data_CES_SR_gof <- filter(table_data, Model=="CES" | Model=="Ref.") %>% 
  select(Country, Model, CSP, flavor, Energy, Nest, scale, lambda, sse)
data_CES_SR_gof <- plyr::rename(data_CES_SR_gof, c("flavor" = "Factors of prod.",
                                                   "scale" = "$\\theta$",
                                                   "lambda" = "$\\lambda$"))
...
data_CES_alpha <- filter(table_data, Model=="CES" | Model=="Ref.") %>% 
  select(Country, Model, CSP, flavor, Energy, Nest, alpha_1, alpha_2, alpha_3)
data_CES_alpha <- plyr::rename(data_CES_alpha, c("flavor" = "Factors of prod.",
                                                 "alpha_1" = "$\\alpha_k$",
                                                 "alpha_2" = "$\\alpha_l$",
                                                 "alpha_3" = "$\\alpha_e$"))
...
sigma_inner_data <- filter(table_data, Model=="CES") %>% 
  select(Country, CSP, flavor, Energy, Nest, sigma_12, sigma_23, sigma_13)
sigma_inner_data <- plyr::rename(sigma_inner_data, c("flavor" = "Factors of prod.",
                                                     "sigma_12" = "$\\sigma_{kl}$",
                                                     "sigma_23" = "$\\sigma_{le}$",
                                                     "sigma_13" = "$\\sigma_{ek}$"))
#
...
sigma_outer_data <- filter(table_data, Model=="CES") %>% 
  select(Country, CSP, flavor, Energy, Nest, sigma_12_3, sigma_23_1, sigma_13_2)
sigma_outer_data <- plyr::rename(sigma_outer_data, c("flavor" = "Factors of prod.", 
...
data_CSP_CD <- filter(AllCoeffs, (model=="CD" & flavor=="Unadjusted" & energy=="Without energy") | model=="exp") %>% 
...
triPlot(
  data=AllCoeffs, 
  aes(x = alpha_3, y = alpha_1, z = alpha_2, 
      fill  = nest, 
      shape = nest,
      size = CSP, alpha = CSP)) +
  facet_grid(. ~ Country) +
...
data_CD_exp <- filter(AllCoeffs, model=="CD" | model=="exp") %>% 
...
This table is not used in the paper, so I'm putting it in 
