...
myXTable(filter(AllCoeffs, model=="CES" | model=="exp") %>% 
           select(Country, model, flavor, energy, nest, cst, scale, lambda, sse),
...
ggplot() +
  geom_point(data=filter(AllCoeffs, model=="CES"), mapping=aes(x=sse, y=lambda, color=energy, size=cst, shape=nest)) +
  ...
  facet_grid(Country ~ flavor) +
  scale_y_continuous(breaks=c(0, 0.01, 0.02), limits=c(0, 0.03)) +
  xlab("SSE") + 
...
myXTable(filter(AllCoeffs, (model=="CD" & flavor=="Unadjusted" & energy=="Without energy") | model=="exp") %>% 
           select(Country, model, cst, scale, lambda, alpha_1, alpha_2, sse),
...
myXTable(filter(AllCoeffs, model=="CD" | model=="exp") %>% 
           select(Country, model, flavor, energy, cst, scale, lambda, alpha_1, alpha_2, alpha_3, sse),
...
myXTable(filter(AllCoeffs, model=="CD") %>% select(Country, model, flavor, energy, cst, Dlambda, Dsse),
...
ggplot() +
  geom_point(data=filter(AllCoeffs, model=="CD"), mapping=aes(x=Dsse, y=Dlambda, color=energy, size=cst)) +
  facet_grid(Country ~ flavor) +
...
ggplot() +
  geom_point(data=filter(AllCoeffs, model=="CD"), mapping=aes(x=percDsse, y=percDlambda, color=energy, size=cst)) +
  facet_grid(Country ~ flavor) +
  scale_size_manual(values=c(2,3.5)) +
...
ggplot() +
  geom_point(data=filter(AllCoeffs, model=="CES"), mapping=aes(x=Dsse, y=Dlambda, color=energy, size=cst, shape=nest)) +
  scale_size_manual(values=c(2,3.5)) +
...
ggplot() +
  geom_point(data=filter(AllCoeffs, model=="CES"), mapping=aes(x=percDsse, y=percDlambda, color=energy, size=cst, shape=nest)) +
  scale_size_manual(values=c(2,3.5)) +
  scale_shape_manual(values=c(16,17,18,15)) +
  facet_grid(Country ~ flavor) +
  xlab(expression(paste(Delta * SSE, " [%]"))) + 
