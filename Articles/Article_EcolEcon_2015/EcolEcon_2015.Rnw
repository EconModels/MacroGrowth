...
AllCoeffs <- 
  ...
  mutate(
    ...
    CSP = relevelFactor(factor(CSP), c("Adhere to CSP", "Reject CSP"))
  ) %>% 
...
### Don't write comments like that.  Say what you are doing to reshape and why.  
### I figured you were going to use it and don't need a comment for that.
### But it not at all easy to glance at this and see what the point is.
...
histDataForGraph <- 
  leaf_apply(formulas, class = "formula", f=function(form, name){
    meta <- strsplit(name, split="[.]")[[1]] # %>% unlist  ### Why use unlist() here?
    Country <- meta[1]
    flavor <- meta[2]
    ...
    # return this.  ### No need to name and store it.
    melt(trimmed_historical_data, measure.vars = measure.vars, 
...
### Note: You an also change the order of things in the legend without changing the order in the data.
...
histDataForGraph <- 
  ...
  mutate( variable = plyr::mapvalues(
    ...
    Country = relevelFactor(factor(Country), c("PT", "UK")),
    flavor = relevelFactor(factor(flavor), c("Unadjusted", "Quality-adjusted")),
    variable = relevelFactor(factor(variable), unique(variable)) 
  )
   
### I think this exhibits poor use of white space.  See alternative below. 
if (FALSE)
...
### Here each summand has the same indentation so you can see which things are being added together.
historicalPlot(
  histDataForGraph, 
  mapping=aes(x=Year, y=value, group=variable, linetype=variable), 
  facet_formula = Country ~ flavor, 
...
fittedGraphData_kle <- 
  fitted_and_resid_data %>% 
  filter(model=="CES" & CSP=="Reject CSP" & (nest == "kl" | nest == "kle"))

fittedGraphNoCSP(fittedGraphData_kle)

### Note: I like a blank line have a chain of %>% operators to make it easy to spot where the chain ends.
@
...
### Put in a separate R chunk and use eval=FALSE, include=FALSE 
...
residualPlot(
  fitted_and_resid_data %>% 
    filter(model=="CES" & CSP=="Reject CSP" & (nest=="kle" | nest=="kl")), 
...
### A function named blah_function should return a function, not a plot.  Don't use function in the name 
### of a function unless it tells you something about the function.  All functions are functions, we don't
### need to name them like that.
...
sigma_graph_function(AllCoeffs %>% filter(model=="CES", Country=="PT"))
@
...
sigma_graph_function(AllCoeffs %>% filter(model=="CES", Country=="UK"))
@
...
fittedGraphNoCSP(fitted_and_resid_data %>% filter(model=="CD" & CSP=="Reject CSP"))
@

%
<<Graphs_CD-resid, fig.pos="H", fig.align='center', fig.width=6.5, fig.height=4, fig.cap="Fitting residuals for Cobb-Douglas models.">>=
residualPlot(fitted_and_resid_data %>% filter(model=="CD" & CSP=="Reject CSP"))
@
...
triPlot(
  data=
    AllCoeffs %>% 
    mutate( Model = interaction(nest, CSP, sep = " -- ", drop=TRUE),
            Inputs = flavor), 
  ...
  scale_size_manual(values = c(5, 3,3,3,3)) +
  # scale_alpha_manual(values = c(.2, .6,.6,.6,.6)) +
...
residualPlot(fitted_and_resid_data %>% 
               filter(model=="CES" & CSP=="Reject CSP" & (nest == "lek" | nest=="kl")),
...
residualPlot(fitted_and_resid_data %>% 
               filter(model=="CES" & CSP=="Reject CSP" & (nest=="ekl" | nest=="kl")),
...
fittedGraphData_ekl <- 
  fitted_and_resid_data %>% 
  filter(model=="CES" & CSP=="Reject CSP" & (nest == "kl" | nest == "ekl"))
