...
filenameResampleCoeffs <- ""
fittedAndResidData <- function(model, id){
  # ******************************************
  # getData doesn't work for lm models that are created for CSP="Adhere to CSP".
...
temp_data_exp <- leaf_apply(modelsExp, class=c("lm"), f = function(model, id){
  ...
  # But, the yhat() call in fittedAndResidData() will not return the correct information from the CSP models.
  # This is because modelsCSP was not created with our typical functions.
...
temp_data <- plyr::rbind.fill( temp_data_exp, temp_data_CSP, temp_data_noCSP )

# No longer using this code, because we have a different "reference" model.
# Get the unadjusted, without-energy residuals from models that adhere to the CSP
# and store them in a data frame
# r_unadj_noE_CSP <- filter(temp_data, flavor=="Unadjusted" & energy=="Without energy" & CSP=="Adhere to CSP") %>% 
#   select(Year, Country, Residuals, model)
# colnames(r_unadj_noE_CSP)[colnames(r_unadj_noE_CSP)=="Residuals"] <- "r_unadj_noE"
# Merge to get the unadjusted, without-energy values next to each country, year, and model
# fitted_and_resid_data <- merge(temp_data, r_unadj_noE_CSP)
...
fittedGraphCSP <- function(data){
  ggplot() +
    geom_line(data=filter(data, CSP=="Adhere to CSP"), mapping=aes(x=Year, y=iGDP)) +
    geom_line(data=data, mapping=aes(x=Year, y=fitted, linetype=CSP)) +
...
%
<<Graphs_CESkle-fitted, fig.pos="H", fig.align='center', fig.width=6.5, fig.height=4, fig.cap="CES models with ($kl$)$e$ nesting. Solid line is historical GDP.">>=
fittedGraphNoCSP(filter(fitted_and_resid_data, model=="CES" & CSP=="Reject CSP" & (nest=="kle" | nest=="kl")))
@
%
<<Graphs_CESlek-fitted, fig.pos="H", fig.align='center', fig.width=6.5, fig.height=4, fig.cap="CES models with ($le$)$k$ nesting. Solid line is historical GDP.">>=
fittedGraphNoCSP(filter(fitted_and_resid_data, model=="CES" & CSP=="Reject CSP" & (nest=="lek" | nest=="kl")))
@
%
<<Graphs_CESekl-fitted, fig.pos="H", fig.align='center', fig.width=6.5, fig.height=4, fig.cap="CES models with ($ek$)$l$ nesting. Solid line is historical GDP.">>=
fittedGraphNoCSP(filter(fitted_and_resid_data, model=="CES" & CSP=="Reject CSP" & (nest=="ekl" | nest=="kl")))
@
...
standardScatterPlot(data=resampleCoeffs, mapping=aes(scale, lambda), facet_formula=~Country, alpha=0.1) +
...
standardScatterPlot(data=resampleCoeffs, mapping=aes(delta, sigma), 
    ...
    scale_x_continuous(breaks=c(0, 0.5, 1.0, 2.0), limits=c(0,1)) +
    scale_y_continuous(breaks=c(0, 0.5, 1.0, 2.0), limits=c(0, 1e6)) +
    labs(x=expression(delta), y=expression(sigma[kle]))
@
