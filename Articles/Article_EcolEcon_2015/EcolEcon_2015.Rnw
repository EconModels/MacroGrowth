...
data_CES_alpha <- filter(AllCoeffs, model=="CES" | model=="exp") %>% 
  select(Country, model, flavor, energy, nest, CSP, alpha_1, alpha_2, alpha_3)
data_CES_alpha <- plyr::rename(data_CES_alpha, c("alpha_1" = "$\\alpha_1$",
                                                 "alpha_2" = "$\\alpha_2$",
                                                 "alpha_3" = "$\\alpha_3$"))
...
sigma_data <- filter(AllCoeffs, model=="CES") 
sigma_data <- plyr::rename(sigma_data, c("model" = "Model", "flavor" = "Type", "energy" = "Energy", "nest" = "Nest"))
@

<<CES_sigma, results="asis">>=
data_CES_sigma_raw <- sigma_data %>% select(Country, Model, Type, Energy, Nest, CSP, sigma_1, sigma)
...
sigma_inner_data <- sigma_data %>% select(Country, Type, Energy, Nest, CSP, sigma_12, sigma_23, sigma_13)
sigma_inner_data <- plyr::rename(sigma_inner_data, c("sigma_12" = "$\\sigma_{kl}$",
                                                     "sigma_23" = "$\\sigma_{le}$",
                                                     "sigma_13" = "$\\sigma_{ek}$"))
#
myXTable(sigma_inner_data,
         caption="Factor substitution~($\\sigma$) for CES models. Inner nests.",
         label=paste0("tab:", opts_current$get("label")), 
         digits=6)
@
%
<<CES_sigma_nest_aware_outer, results="asis">>=
sigma_outer_data <- sigma_data %>% select(Country, Type, Energy, Nest, CSP, sigma_12_3, sigma_23_1, sigma_13_2)
sigma_outer_data <- plyr::rename(sigma_outer_data, c("sigma_12_3" = "$\\sigma_{(kl)e}$",
                                                     "sigma_23_1" = "$\\sigma_{(le)k}$",
                                                     "sigma_13_2" = "$\\sigma_{(ek)l}$"))
myXTable(sigma_outer_data,
         caption="Factor substitution~($\\sigma$) for CES models. Outer nests.",
         label=paste0("tab:", opts_current$get("label")), 
         digits=6)
@
