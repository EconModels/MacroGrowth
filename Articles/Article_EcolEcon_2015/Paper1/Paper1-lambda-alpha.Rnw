...
myXTable(data_delta_table,
         caption="Distribution parameters ($\\delta_1$ and $\\delta$) for all modeling choices.",
         label=paste0("tab:", opts_current$get("label")), 
...
myXTable(data_rho_table,
         caption="Values of ($\\rho_1$ and $\\rho$) for all modeling choices.",
         label=paste0("tab:", opts_current$get("label")), 
         digits=c(1,1,1,1,1,1,1, 3, 3)
         )
@
...
<<Cost_Share_Plot, fig.pos="H", fig.align='center', fig.width=6.5, fig.height=1.5, fig.cap="Historical cost shares for capital, labor, and energy.">>=
ggplot(
  data = gather(CostShares, key=cost_share, value=value, cs_k, cs_l, cs_e)
  ) + 
...
fitted_and_resid_data %>% 
  ...
  ggplot() +
  geom_line( mapping=aes(x=Year, y=value, group=label, linetype=label, size=label)) +
...
fittedGraphData_lek <- 
  fitted_and_resid_data %>% 
  filter(model=="CES" & CSP=="w/o CSP" & (nest == "kl" | nest == "lek"))
