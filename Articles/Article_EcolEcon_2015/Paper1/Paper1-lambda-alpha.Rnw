...
#   labs(x="", y="Indexed GDP (1960 = 1)") +
  ...
  # But Paul suggests some small changes:
...
standardScatterPlot(data=resampleCoeffsCES, mapping=aes(scale, lambda), facet_formula=~Country, alpha=0.25, size=1, orig_color = "black") +
...
standardScatterPlot(data=resampleCoeffsCES, mapping=aes(delta_1, delta), 
                    facet_formula=~Country, alpha=0.2, size = 1,
                    orig_color="black") +
