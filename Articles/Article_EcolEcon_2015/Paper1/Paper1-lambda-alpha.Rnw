...
This is a table for Paper 1.

<<Other_Models_table>>=
  # Start with the exponential-only (reference) models
  OMTable <- leaf_apply(modelsExp, class="lm", f=function(mod, id){
    list(Model = "Ref.",
         Country=id[[1]],
         kl = NA,
         e = NA,
  ...
  # Add all CES models
  OMTable <- leaf_apply(models, class="cesModel", f=function(mod, id){
  ...
  list(Model = "CES",
       ...
       SSE = sum(resid(mod)^2))
  }) %>% bind_rows() %>% bind_rows(OMTable, .)
  
  # Add the off-axis models
  OMTable <- leaf_apply(other_models, class="cesModel", f=function(mod, id){
    id_components <- strsplit(id, split = "[.]")[[1]]
    print(id_components)
    adjusted_components <- strsplit(id_components[[2]], "[_]")[[1]]
    print(adjusted_components)
    list(Model = "CES",
         Country = id_components[[1]],
  ...
  OMTable <- 
    ...
    filter(OMTable, is.na(Nest) | Nest == "kl" | Nest == "kle")
