...
mod <- models$PT$adjusted$withE$CES$kle
# mod
print(mod$coefficients[["delta_1"]], digits = 20)
1 - mod$coefficients[["delta_1"]]
...
mod11 <- attr(mod, "model.attempts")[[11]]
# mod11
sum(resid(mod11)^2)
@
...
PTresample <- resampleCoeffsCES %>% filter(Country == "PT", method != "orig")
any(PTresample$delta_1 >= 1)
1 - max(PTresample$delta_1)
range95 <- quantile(PTresample$delta_1, probs=c(0.025, 0.975))
range95
1 - range95
@
...
ggplot(mapping = aes_string(x = "delta_1_plot")) +
  ...
  geom_vline(aes(xintercept = 1-range95), colour="gray50", size=0.1) +
