...
AllOE %>% 
  mutate(
    xfacet = as.character(interaction(Country, factor(flavor, labels = c("QA", "UA")), sep = " - ")),
    xfacet = relevelFactor(factor(xfacet), c("PT - UA", "PT - QA", "UK - UA", "UK - QA")),
    yfacet = as.character(interaction(factor(nest, labels = c("(ek)l", "kl", "(kl)e", "(le)k")), CSP, sep = " - ")),
