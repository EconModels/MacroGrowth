...
<<OMTable_df>>=
  ...
  data.frame(Country = id_components[[1]],
       Model = "CES",
...
<<OMTable, results="asis">>=
  OMTable %>%
    ...
    myXTable(caption="Solow residual~($\\lambda$) and sum of squared errors~(SSE) 
             for $(kl)e$ models that reject the cost share principle. 
             $\\lambda / \\lambda_{Ref.}$ is the ratio of Solow residual relative 
             to the reference (exponential-only) model.
...
<<resample_graph_sigma_sigma_1, fig.pos="H", fig.align='center', fig.width=6.5, fig.height=3.5, fig.cap="$\\sigma$ and $\\sigma_1$ values for resample models that reject the cost share theorem, include energy, use quality-adjusted factors of production, and employ the $(kl)e$ nesting.">>=
p <- standardScatterPlot(data=resampleCoeffsCES, mapping=aes(sigma_1, sigma), 
                    facet_formula=~Country, alpha=0.05, size = 1,
                    orig_color="black") +
...
<<PT-resample-meta, echo = TRUE, include = TRUE>>=
PTklemod <- models$PT$adjusted$withE$CES$kle
# print(summary(PTklemod))
# The standard error of delta_1
summary(PTklemod)$coefficients[3, 2]
print(PTklemod$coefficients[["delta_1"]], digits = 20)
1 - PTklemod$coefficients[["delta_1"]]
# Machine precision
.Machine$double.eps

# Look at SSE of the actual model and the boundary model.
# First, the actual model:
sum(resid(PTklemod)^2)
# Now, the boundary model with delta_1 = 1, namely boundary model 11.
PTklemod11 <- attr(PTklemod, "model.attempts")[[11]]
# print(summary(PTklemod11))
sum(resid(PTklemod11)^2)
@


<<PT-resample-quantiles, echo = TRUE, include = TRUE>>=
PTkleresample <- resampleCoeffsCES %>% filter(Country == "PT", method != "orig")
any(PTkleresample$delta_1 >= 1)
1 - max(PTkleresample$delta_1)
PTklerange95 <- quantile(PTkleresample$delta_1, probs=c(0.025, 0.975))
PTklerange95
1 - PTklerange95
# See if there are any coefficients that are exactly 0.5, the starting value.
any(PTkleresample$delta_1 == 0.5)
# Calculate standard deviation of the delta_1 resampling distribution.
sd(PTkleresample$delta_1)
@


<<rho_1-PT-resample-dist-graph, fig.pos="H", fig.align='center', fig.width=3.5, fig.height=2.5, fig.cap="Bootstrap resampling density distribution of $1 - \\delta_1$ for the Portuguese modeling approach that employs the $(kl)e$ nesting, quality-adjusts the factors of production, and rejects the CSP. The $\\times$ indicates location of base model value. Vertical lines show the range of the middle 95\\% of the distribution.">>=
...
ggplot(mapping = aes_string(x = "delta_1_plot")) +
  ...
  geom_vline(aes(xintercept = 1-PTklerange95), colour="gray50", size=0.1) +
