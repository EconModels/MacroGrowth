...
graph_text_size <- 8 # Points for labels, etc. on graphs.
# ********************************************************
opts_chunk$set(
  # ********************************************************
  eval=TRUE,
...
graph_text_size_style <- function(p, text_size = 12) {
  p +
    theme(
...
HistoricalData <- list(
  PT = IST %>% 
    filter(Country == "PT", Year <= 2009), # Ensures all fits are done on equal basis.
  UK = Leeds %>% 
    filter(Country == "UK", Year <= 2009)
)
...
if (rerunResampling){
  ...
  # Resample model objects
  resampleModelsCES <- list(PT=resamplesCESPT$models, UK=resamplesCESUK$models)
  # Save this object in case we need to interrogate it later.
  # Note, however, that resampleModelsCES is NOT in the repository,
  # because it can be a HUGE file when we're doing, say, 1000 resamples.
  ...
  saveRDS(ReModels, file = ReModelsFilename)
  # Resample coefficients
  resampleCoeffsCES <- bind_rows(
    bind_cols(resamplesCESPT$coeffs %>% mutate(Country="PT")), 
    bind_cols(resamplesCESUK$coeffs %>% mutate(Country="UK")))
  ...
  # We don't need to load resampelModelsCES, because we use only ReModels later.
  # resampleModelsCES <- readRDS(resampleModelsCESFilename)
  ReModels <- readRDS(ReModelsFilename)
}
@


% <<ReModels>>=
% # ReModels is the data frame that holds time series of all the resample models.
% # We save this to disk and include it in the repo, because it is much smaller than resampleModelsCES.
% if (! rerunResampling){
%   ReModels <- readRDS(ReModelsFilename)
% } else {
%   # This code is expensive, so run only if we re-ran the resampling.
%   ReModels <-
%     bind_rows(
%       leaf_apply(resampleModelsCES, 
%                  function(x,id) {mod2oedf(x, id) %>% mutate(id = id)},
...
FittedAndResid <- bind_rows(temp_data_exp, temp_data_CSP, temp_data_noCSP)
@
...
<<TableData>>=
  TableData <- 
  AllCoeffs %>%
#  rename_(Model = "model", Energy = "energy", Nest = "nest") %>%
  mutate(Nest = 
           plyr::mapvalues(
             nest,
...
# At this point, there is no reason to estimate the models every time.

fileNameOtherModels <- "EcolEcon2015kleOtherModelsCES.rds"

if (!rerunCESModels){
  # Load other models from the file on disk
  other_models <- readRDS(fileNameOtherModels)
} else {
  other_models <- list()
  # PT, unadjusted kl, quality-adjusted e
  other_models$PT$uakl_qae <- cesModel(iGDP ~ iKstkS.L + iL + iUMP + iYear, HistoricalData$PT, nest=c(1,2,3))
  # PT, quality-adjusted kl, unadjusted e
  other_models$PT$qakl_uae <- cesModel(iGDP ~ iKservS.L + ihLest + iXpMP + iYear, HistoricalData$PT, nest=c(1,2,3))
  # UK, unadjusted kl, quality-adjusted e
  other_models$UK$uakl_qae <- cesModel(iGDP ~ iKstkO.WwithRD + iL + iU + iYear, HistoricalData$UK, nest=c(1,2,3))
  # UK, quality-adjusted kl, unadjusted e
  other_models$UK$qakl_uae <- cesModel(iGDP ~ iKservO.WwithRD + ihLest + iXp + iYear, HistoricalData$UK, nest=c(1,2,3))
  saveRDS(other_models, fileNameOtherModels)
}
