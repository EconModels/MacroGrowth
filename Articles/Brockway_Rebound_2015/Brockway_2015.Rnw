...
# There are places where we need to use data from 1980 onward for the UK only.
UK1980Data <- historicalData %>% filter(Country == "UK") %>% subset(Year >= 1980) %>%
  mutate(
    # Re-normlaize to 1980.
    iYear = iYear - iYear[1],
    iY = iY / iY[1],
    iK = iK / iK[1],
    iL = iL / iL[1],
...
<<Historical_Data_Graph, fig.pos="H", fig.align='center', fig.width=6.5, fig.height=2.5, fig.cap="Historical data.", warning=FALSE>>=
...
<<sF-sK-graph, fig.pos="H", fig.align='center', fig.width=6.5, fig.height=2.5, fig.cap="$sF$ and $sK$ values.", warning=FALSE>>=
ggplot(data=historicalData %>%
         melt(measure.vars = c("sF", "sK"),
              value.name = "value", variable.name = "variable") %>%
         mutate(
           Country = relevelFactor(as.factor(Country), countries),
           variable = relevelFactor(as.factor(variable), c("sK", "sF"))
         )
       ) + 
  geom_line(mapping = aes(x=Year, y=value, group=variable, linetype=variable)) +
  facet_grid(. ~ Country) +
  scale_x_continuous(breaks=c(1960, 1980, 2000)) +
...
<<Models>>=
modelsFileName <- file.path("models", "Brockway2015Models.rds")
modelsiXpFileName <- file.path("models", "Brockway2015ModelsiXp.rds")
modelsUK1980FileName <- file.path("models", "Brockway2015ModelsUK1980.rds");
if (!rerunModels){
  # load from the file on disk
  models <- readRDS(modelsFileName)
  modelsiXp <- readRDS(modelsiXpFileName)
  modelsUK1980 <- readRDS(modelsUK1980FileName) # UK using only data from 1980 - 2010.
} else {
  ...
  # Save the models.
  saveRDS(models, file = modelsFileName)
  
  # Now models using iXp instead of iU
  modelsiXp <- lapply(countries, function(coun){
    return(cesModel(iY ~ iK + iL + iXp + iYear, filter(historicalData, Country == coun), nest = 1:3))
  })
  names(modelsiXp) <- countries
  # Save the models.
  saveRDS(modelsiXp, file = modelsiXpFileName)
  
  # Now run the UK model using only 1980--2010 data
  modelsUK1980 <- list(
    U = cesModel(iY ~ iK + iL + iU + iYear, UK1980Data),
    iXp = cesModel(iY ~ iK + iL + iXp + iYear, UK1980Data)
    )
  saveRDS(modelsUK1980, file = modelsUK1980FileName)
}

# Run these models every time. They are fast.
modelsKL <- lapply(countries, function(coun){
  return(cesModel(iY ~ iK + iL + iYear, filter(historicalData, Country==coun), nest = 1:2))
})
names(modelsKL) <- countries

modelKLUK1980 <- cesModel(iY ~ iK + iL + iYear, UK1980Data, nest = 1:2)
@
...
First, look a the UK results using all years: 
\Sexpr{historicalData %>% subset(Country == "UK") %>% select(Year) %>% min()}--%
\Sexpr{historicalData %>% subset(Country == "UK") %>% select(Year) %>% max()}.

<<resultsUK, echo=TRUE>>=
summary(models[["UK"]])
naturalCoef(models[["UK"]]) %>% glimpse()
@

The fitted model for the UK is not near any boundaries
to the economically-meaningful solution space.

Next, UK results using years 
\Sexpr{min(UK1980Data$Year)} -- \Sexpr{max(UK1980Data$Year)}.

<<resultsUK1980, echo=TRUE>>=
summary(modelsUK1980$U)
naturalCoef(modelsUK1980$U) %>% glimpse()
@

The above model with limited years is along a boundary ($\sigma_1 = \infty$) 
and very different from the model using all years.
However, the important thing for rebound is $\rho$,
which is similar between the two models.

Next, look at the US results.

<<resultsUS, echo=TRUE>>=
summary(models[["US"]])
naturalCoef(models[["US"]]) %>% glimpse()
@
...
<<Resampling>>=
resampleFileName <- file.path("models", "Brockway2015Resamples.rds")
resampleUK1980FileName <- file.path("models", "Brockway2015UK1980Resamples.rds")
if (!rerunResamples){
  # load resample data from the file on disk
    resampleCoeffs <- readRDS(resampleFileName)
    resampleCoeffsUK1980 <- readRDS(resampleUK1980FileName)
} else {
  # Run resampling on models.
  resampleCoeffs <- leaf_apply(l = models, class = c("cesModel", "plm"), f = function(mod, mod_name){
    resampledFits(mod, method = "wild", n = nResamples)$coeffs %>%
      mutate(
        Country = mod_name
      )
  }) %>% bind_rows
  ...
  # UK model for years 1980-2010
  resampleCoeffsUK1980 <- resampledFits(modelsUK1980$U, method = "wild", n = nResamples)$coeffs
  saveRDS(resampleCoeffsUK1980, file = resampleUK1980FileName)
}
@
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Rebound}
\label{sec:rebound_calculations}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%++++++++++++++++++++++++++++++
\subsection{Basic Rebound} 
\label{sec:basic_rebound}
%++++++++++++++++++++++++++++++

For the basic rebound calculations, 
we can calculate rebound by Equation~\ref{eq:rebound}.
%
\begin{equation} \label{eq:rebound}
  Re \approx 1 - \frac{\rho}{1 + \rho} = \sigma_{(kl)e}\; .
\end{equation}
%
<<calc_rebound>>=
# Add a Rebound column to the resample coefficients
resampleCoeffs <- resampleCoeffs %>%
  # Applying the approximation Re ~ 1 - rho/(1 + rho) = sigma
  mutate(Re = sigma) 
@
%
Plot resampling results for \Sexpr{nResamples} resamples in Figure~\ref{fig:rebound_graph}.

Note that the vertical axis is $\alpha_e$. We now know that this is the initial value for $\alpha_e$.
In fact, there would be a trajectory with each dot as its starting point.
%
<<rebound_graph, fig.pos="H", fig.align='center', fig.width=6.5, fig.height=2.5, fig.cap="$\\alpha_e$ vs. $Re$ for all three countries.">>=
standardScatterPlot(data=resampleCoeffs, mapping=aes(Re, alpha_3), facet_formula=~Country, alpha=0.05, orig_color = "black") +
    # scale_x_continuous(limits=c(0, 5)) + 
    scale_x_log10(limits = c(0.001,100), breaks = c(0.01, 0.1, 1, 10), labels = c(0.01, 0.1, 1, 10)) +

    scale_y_continuous(breaks=c(seq(0, 1, 0.2))) +
    geom_vline(xintercept = 1, colour="gray50", size=0.1) +
    labs(y=expression(alpha[e]))
@


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Full rebound}
\label{sec:full_rebound}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

Now, we calculate rebound by the full equation
%
\begin{equation} \label{eq:time_dependent_rebound}
  Re = 1 + \frac{s_F \left[ s_F (1 + \rho) + \rho \right] + \rho (s_F - 1 - s_k) + s_F }{(1 + s_k + s_F)(1 + \rho)}
\end{equation}
%
for each country.

<<Rebound, echo=TRUE>>=
Rebound <- leaf_apply(models, class = c("cesModel"), f = function(mod, id){
  # "id" is the Country for mod
  rho <- naturalCoef(mod)$rho
  historicalData %>% 
    filter(Country == id) %>%
    mutate(
      Re = 1 + 
              (sF * (sF*(1+rho) + rho) + rho*(sF - 1  - sK) + sF) / 
                    ( (1 + sK + sF ) * (1 + rho) )
      )
}) %>% 
  bind_rows() %>%
  mutate(
    Country = relevelFactor(as.factor(Country), rev(countries))
    )
@

<<Rebound-graph, fig.pos="H", fig.align='center', fig.width=6.5, fig.height=4.5, fig.cap="Time-dependent rebound.">>=
ggplot(Rebound) + 
  geom_line(mapping=aes(x=Year, y=Re, group=Country, linetype=Country)) +
  geom_hline(mapping=aes(yintercept=1), color = "gray70", size=0.3) +
...
If we want to do so, we could apply the same calculation to each resampled model,
which would give $n$ lines for each country in Figure~\ref{fig:Rebound-graph}.
This would be similar to Figure~9 in the CES paper.


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Primary Exergy} 
\label{sec:primary_exergy}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

We can also fit to historical data using quality-adjusted $kl$ and unadjusted $e$
in the form of primary exergy ($iXp$). 
Here are the results.

<<iXp, echo=TRUE>>=
# UK
modelsiXp$UK
# UK starting from 1980
modelsUK1980$iXp
# US
modelsiXp$US
# CN
...
# UK
sum(resid(modelsiXp$UK)^2)
# UK 1980
sum(resid(modelsUK1980$iXp)^2)
# US
sum(resid(modelsiXp$US)^2)
# CN
sum(resid(modelsiXp$CN)^2)

# Compare to sse for models that use iU
# UK
sum(resid(models$UK)^2)
# UK1980
sum(resid(modelsUK1980$U)^2)
# US
sum(resid(models$US)^2)
# CN
sum(resid(models$CN)^2)

@


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{$kl$-only models} 
\label{sec:klmodels}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

Here are some models fit with only k and l as factors of production.
...
# UK
sum(resid(modelsKL$UK)^2)
# UK1980
sum(resid(modelKLUK1980)^2)
# US
sum(resid(modelsKL$US)^2)
# CN
sum(resid(modelsKL$CN)^2)
