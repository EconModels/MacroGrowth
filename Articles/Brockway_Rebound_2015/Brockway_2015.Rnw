...
print(getwd())
historicalData <- read.csv(file = "data/BrockwayData.csv")
@
...
historicalDataForGraph$Country <- relevelFactor(as.factor(historicalDataForGraph$Country), c("UK", "US", "CN"))
...
if (!rerunModels){
  ...
  models <- lapply(c("UK", "US", "CN"), function(coun){
