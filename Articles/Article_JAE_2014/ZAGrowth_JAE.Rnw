...
coeffs_df <- loadPostProcessedData(Source = "SUN", kind = "coeffs", dir=pp_path)
coeffs_df_ZA <- subset(coeffs_df, subset=country=="ZA")
# Load fitted data from all models in one large data frame
fitted_df <- loadPostProcessedData(Source = "SUN", kind = "fitted", dir=pp_path)
fitted_df_ZA <- subset(fitted_df, subset=Country=="ZA")
# Load the original models.
oModels <- loadPostProcessedData(Source = "SUN", kind = "models", dir=pp_path)
