% This article has been prepared for publication in **** in RStudio with knitr.
...
historicalData <- REXS # From the EconData package
...
coeffs_df <- loadPostProcessedData(Source = "REXS", kind = "coeffs", dir=pp_path)
# Load fitted data from all models in one large data frame
fitted_df <- loadPostProcessedData(Source = "REXS", kind = "fitted", dir=pp_path)
# Load the original models.
oModels <- loadPostProcessedData(Source = "REXS", kind = "models", dir=pp_path)
@
...
\title{Application of Bootstrap Resampling to REXS Data}
...
The REXS data set include US, UK, JP, and AT.
(See Table~\ref{tab:Economy_Examples}.)

\begin{table} \caption{REXS economies and years of evaluation.} \label{tab:Economy_Examples} 
  \begin{center}
%     \begin{tabular}{r c c l l} 
%       \multirow{3}{*}{Developed} & \includegraphics[height=0.11in]{flag/flag_US.png} & US & USA & \multirow{3}{*}{1900--2000}\\
%                                  & \includegraphics[height=0.11in]{flag/flag_UK.png} & UK & United Kingdom & \\
%                                  & \includegraphics[height=0.11in]{flag/flag_JP.png} & JP & Japan          & \\
%                                  & \includegraphics[height=0.11in]{flag/flag_AT.png} & AT & Austria        & \\1950--2000
%     \end{tabular}
    \begin{tabular}{c c l l} 
      \includegraphics[height=0.11in]{flag/flag_US.png} & US & USA            & \multirow{3}{*}{1900--2000}\\
      \includegraphics[height=0.11in]{flag/flag_UK.png} & UK & United Kingdom & \\
      \includegraphics[height=0.11in]{flag/flag_JP.png} & JP & Japan          & \\
      \hline
      \includegraphics[height=0.11in]{flag/flag_AT.png} & AT & Austria        & 1950--2000
...
labor ($l$), and 
energy ($q$) 
for 1900--2000 (US, UK, JP) and 1950--2000 (AT). 
%
...
mdata <- melt(historicalData, 
              measure.vars = c("iGDP", "iK", "iL", "iXp", "iU"), 
...
<<CD_lambda_theta_graph, fig.pos="H", fig.align='center', fig.width=4, fig.height=4.5, fig.cap="$\\lambda$ and $\\theta$ values for Cobb-Douglas resample models.">>=
cdCoeffsData <- filter(coeffs_df, model=="cd", nestStr %in% c("iK+iL", "iK+iL+iXp", "iK+iL+iU"))
...
<<CD_Ternary_Scatter_Graph, fig.pos="H", fig.align='center', fig.width=3.9, fig.height=4, fig.cap="Cobb-Douglas resample model parameters.">>=
cdCoeffsData <- filter(coeffs_df, model=="cd", nestStr %in% c("iK+iL", "iK+iL+iXp", "iK+iL+iU"))
standardTriPlot(data=cdCoeffsData, facet_formula=country~energy)
@

<<CD_GDP_Spaghetti_Graph, fig.pos="H", fig.align='center', fig.width=4, fig.height=4.5, fig.cap="Cobb-Douglas fitted models.">>=
cdFittedData <- filter(fitted_df, model=="cd", nestStr %in% c("iK+iL", "iK+iL+iXp", "iK+iL+iU"))
...
<<CES_lambda_gamma_graph_X, fig.pos="H", fig.align='center', fig.width=5, fig.height=4.5, fig.cap="$\\lambda$ and $\\gamma$ values for CES resample models with exergy ($x$).">>=
cesCoeffsDataX <- filter(coeffs_df, model=="ces", nestStrParen %in% c("(iK + iL) + ()", 
                                                                      "(iK + iL) + (iXp)", 
                                                                      "(iL + iXp) + (iK)", 
                                                                      "(iXp + iK) + (iL)"))
standardScatterPlot(data=cesCoeffsDataX, mapping=aes(gamma_coef, lambda), facet_formula=country~nestStrParen, alpha=0.05) +
    scale_x_continuous(breaks=c(0.9, 1.0, 1.1)) +
    labs(x=expression(gamma), y=expression(lambda))
@

<<CES_sigma1_delta1_graph_X, fig.pos="H", fig.align='center', fig.width=5, fig.height=5, fig.cap="$\\sigma_1$ and $\\delta_1$ values for CES resample models with exergy ($x$).">>=
...
<<CES_sigma_delta_graph_X, fig.pos="H", fig.align='center', fig.width=4.4, fig.height=5, fig.cap="$\\sigma$ and $\\delta$ values for CES resample models using exergy ($x$).">>=
...
<<CES_ternary_graphX, fig.pos="H", fig.align='center', fig.width=5.5, fig.height=7.5, fig.cap="$\\alpha$, $\\beta$, and $\\gamma$ values for CES resample models with exergy ($x$).">>=
...
<<CES_Spaghetti_Graph_X, fig.pos="H", fig.align='center', fig.width=6.5, fig.height=8.5, fig.cap="CES fitted models with exergy ($x$).">>=
...
<<CES_lambda_gamma_graph_U, fig.pos="H", fig.align='center', fig.width=5, fig.height=4.5, fig.cap="$\\lambda$ and $\\gamma$ values for CES resample models with useful work ($u$).">>=
...
<<CES_sigma1_delta1_graph_U, fig.pos="H", fig.align='center', fig.width=5, fig.height=5, fig.cap="$\\sigma_1$ and $\\delta_1$ values for CES resample models with useful work ($u$).">>=
...
<<CES_sigma_delta_graph_U, fig.pos="H", fig.align='center', fig.width=4.4, fig.height=5, fig.cap="$\\sigma$ and $\\delta$ values for CES resample models with useful work ($u$).">>=
...
<<CES_Spaghetti_Graph_U, fig.pos="H", fig.align='center', fig.width=6.5, fig.height=8.5, fig.cap="CES fitted models with useful work ($u$).">>=
...
linexCoeffsData <- filter(coeffs_df, model=="linex", energy %in% c("iXp", "iU"))
...
linexFittedData <- filter(fitted_df, model=="linex", energy %in% c("iXp", "iU"))
...
Figures~\ref{fig:LINEX_abg_traj_graph_US} and~\ref{fig:LINEX_abg_traj_graph_UK} 
show trajectories for US and UK, respectively with $u$.

<<LINEX_abg_traj_graph_US, fig.pos="H", fig.align='center', fig.width=3, fig.height=3.5, fig.cap="LINEX trajectory for US.">>=
dataUS <- fortify(oModels$REXS$US$linex$`iK+iL+iU`)
triPlot(data=dataUS, grid_lines=5, x=alpha, y=beta, z=gamma, geom=geom_line, aes_string="colour = iYear + 1990")
@

<<LINEX_abg_traj_graph_UK, fig.pos="H", fig.align='center', fig.width=3, fig.height=1.5, fig.cap="LINEX trajectory for UK.">>=
dataUK <- fortify(oModels$REXS$UK$linex$`iK+iL+iU`)
triPlot(data=dataUK, grid_lines=5, x=alpha, y=beta, z=gamma, geom=geom_line, aes_string="colour = iYear + 1990")
@
