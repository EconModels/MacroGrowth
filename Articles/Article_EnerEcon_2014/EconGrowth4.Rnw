...
are straightforward to derive from the bottom row of Table~\ref{tab:CES_boundary_models}.
...
and the Leontief model applies.%
  ...
  $y = \gamma A \min (x_1, x_2)$, row 4 in Table~\ref{tab:CES_boundary_models}.
  }
...
\begin{table} \caption{CES boundary models (rows~1--20) and full model (bottom row). 
                       Re-ordered, but with original model numbers. old-new numbering **** DELETE EVENTUALLY ****}
  \begin{center}
    \begin{tabular}{r c c c c l} 
      \toprule
      & $\delta_1$  & $\sigma_1$ & $\delta$   & $\sigma$ & Boundary model \\
      \midrule
      01-01 & 1           & ---        & 1          & ---      & $y = \gamma A x_1$ \\
      02-02 & 0           & ---        & 1          & ---      & $y = \gamma A x_2$ \\
      06-03 & ---         & ---        & 0          & ---      & $y = \gamma A x_3$ \\
      \midrule
      03-04 & ---         & 0          & 1          & ---      & $y = \gamma A \min (x_1, \, x_2)$ \\
      07-05 & 1           & ---        & ---        & 0        & $y = \gamma A \min (x_1, \, x_3)$ \\
      08-06 & 0           & ---        & ---        & 0        & $y = \gamma A \min (x_2, \, x_3)$ \\
%
     04-07 &             & $\infty$   & 1          & ---        
            & $y = \gamma A \left[ \delta_1 x_1 + (1 - \delta_1) x_2 \right]$ \\
%
...
%
     16-12 & 0           & ---        &            & 
            & $y = \gamma A \left[ \delta x_2^{-\rho} + \left(1 - \delta \right) x_3^{-\rho} \right]^{-1/\rho}$ \\
      \midrule
     09-13 & ---         & 0          & ---        & 0        & $y = \gamma A \min (x_1, \, x_2, \, x_3)$ \\
%
     13-14 & ---         & 0          &            & $\infty$        
            & $y = \gamma A \left[ \delta \min (x_1, \, x_2) + \left( 1 - \delta \right) x_3  \right]$ \\
%
