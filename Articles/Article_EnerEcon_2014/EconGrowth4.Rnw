...
\usepackage{nicefrac}   % Gives nice fractions
...
spaghettiGraphLevel <- 1.0 # Spaghetti graph bands will encompass a certain percentage of the data.
@
...
% The following commands assist making tables for elasticity of substitution (sigma and sigma_1).
% The \firstrow macro creates the sigma_1 row.  The following arguments apply:
% #1 sigma_1 for (kl) nest
% #2 sigma_1 for (kl)e nest
% #3 sigma_1 for (ek)l nest
% #4 sigma_1 for (le)k nest
\newcommand{\firstrow}[4]{\multirow{2}{*}{$\sigma_1$} & ($\underline{k}$+$\underline{l}$)+() & #1 
                                  &  & 
                                        & &  \\
                            & ($\underline{k}$+$\underline{l}$)+($e$) & #2
                                  & ($\underline{e}$+$\underline{k}$)+($l$) & #3
                                        & ($\underline{l}$+$\underline{e}$)+($k$) & #4 \\
}

% The \secondrow macro creates the sigma row.  The following arguments apply:
% #1 sigma for (kl)e nest
% #2 sigma for (kl)e nest
% #3 sigma for (le)k nest
% #4 sigma for (le)k nest
% #5 sigma for (ek)l nest
% #6 sigma for (ek)l nest
\newcommand{\secondrow}[6]{      \multirow{3}{*}{$\sigma$} & &
                                      & ($\underline{k}$+$l$)+($\underline{e}$) & #1
                                            & ($k$+$\underline{l}$)+($\underline{e}$) & #2 \\
                                & ($\underline{l}$+$e$)+($\underline{k}$) & #3
                                      & ($l$+$\underline{e}$)+($\underline{k}$) & #4
                                            &  & \\
                                & ($e$+$\underline{k}$)+($\underline{l}$) & #5
                                      & &
                                            & ($\underline{e}$+$k$)+($\underline{l}$) & #6 \\
}

% The \thirdrow macro creates the summary row at the bottom.  The following arguments apply:
% #1 summary for elasticity of substitution between k and l
% #2 summary for elasticity of substitution between k and e
% #3 summary for elasticity of substitution between l and e
\newcommand{\thirdrow}[3]{    & \multicolumn{2}{c}{#1} & \multicolumn{2}{c}{#2} & \multicolumn{2}{c}{#3} \\ }

% The \eostable macro creates the overall table.  
% The label for the table is tab:sigma_#1, i.e., tab:sigma_<2-letter country code>.  
% The following arguments apply:
% #1 2-letter country abbreviation
% #2 sigma_1 row.  Use \firstrow macro here.
% #3 sigma row.  Use \secondrow macro here.
% #4 summary row.  Use \thirdrow macro here.
\newcommand{\eostable}[4] {\begin{table} \caption{Elasticities of substitution (#1).} \label{tab:sigma_#1} 
  \begin{center}
...
**** How does this discussion sound now? --MKH ****
...
on current and future economic performance.
...
The recent 50\% drop in oil prices, 
brought about by the US shale boom,
has led to costs exceeding revenue for many producers
and put future viability of many firms in question~\citep{Krauss:2015aa}.
For many OPEC countries, 
the price of oil is below the break-even point for national budgets,
but they may be willing to wait for a shakeout of marginal shale producers
before providing oil price support~\citep{Krauss:2014ab}.
...
Lower oil prices may be good for many world economies in the short term, 
but the long-term viablity of today's \$50/barrel oil is questionable
as the shale industry shakes out and as demand creeps upward.
...
% Mainstream economic growth theory as expressed in the Cobb-Douglas production 
% function makes no attempt, beyond growth in capital~($k$) and labor~($l$), 
% to describe the mechanisms that affect economic growth, 
% nor does it attempt, beyond different values for the fitting parameters,
...
using the \texttt{R} \citep{R} functions
...
and more importantly why we follow this particular approach :) 
(another way of saying as we are publishing 
in an economics journal with an audience limited in statistics, 
we must be very clear) --MdW ****
**** Randy: can you do a little more explaining here? 
Perhaps we should say that this is a standard way of forming the resample time series? --MKH ****
...
**** do we follow precedent here? or just a number we fell comfortable with? 
may warrant a reference and/or short explanation why we choose 1000 times. --MdW ****
**** Randy: Can you provide some additional explanation here? --MKH ****
...
minimum $sse$ (Equation~\ref{eq:sse}) as the winning model
to be presented in graphs.
...
for Cobb-Douglas resample models with and without energy.%
  \footnote{
  In all resample graphs, 
  resample points are shown as 95\% transparent black dots
  (alpha channel set at 0.05).
  The overlapping portion of twenty resample points 
  appears as dark as the type of this article.
  }
The fit to historical data is shown as gray crosshairs.

Figure~\ref{fig:CD_Ternary_Scatter_Graph} shows ternary plots%
  \footnote{
  Ternary plots are effective for communicating relationships among three variables
  that sum to a constant value.
  Each vertex is labeled with a variable ($\alpha$, $\beta$, or $\gamma$)
  whose value is unity at that location.
  For example, a data point at the top of the ternary plots in Figure~\ref{fig:CD_Ternary_Scatter_Graph}
  has $\gamma = 1$ and both $\alpha = 0$ and $\beta = 0$, 
  by the constant returns to scale assumption.
  Along each edge, the variable labeled at the opposite vertex is equal to zero.
  For example, a data point along the bottom edge 
  of the ternary plots in Figure~\ref{fig:CD_Ternary_Scatter_Graph}
  has $\gamma = 0$.
  A data point along the bottom edge and two-thirds of the way toward the $\beta$ vertex
  has $\alpha = \nicefrac{1}{3}$, $\beta = \nicefrac{2}{3}$, and $\gamma = 0$.
  }
...
**** Fix colliding x axis labels in all Figures --MKH ****

<<CD_lambda_theta_graph, fig.pos="H", fig.align='center', fig.width=2.8, fig.height=7.0, fig.cap="$\\lambda$ and $\\theta$ values for Cobb-Douglas resample models.">>=
...
standardTriPlot(data=cdCoeffsData, facet_formula=country~energy, alpha=0.05)
@
...
$\gamma$, $\lambda$, $\delta_1$, $\delta$, $\rho_1$, and $\rho$. 
...
negative values for elasticity of substitution parameters $\sigma_1$ and $\sigma$.
...
in $\rho_1$ and $\rho$ (9, 2, 1, 0.43, 0.25, 0.1, -0.1, -0.5, -0.75, -0.9, -0.99),
...
and values of $\gamma$, $\lambda$, $\delta_1$, and $\delta$ are estimated 
by gradient search with the \texttt{PORT} and \texttt{L-BFGS-B} algorithms.
In all, 121 gradient searches in $\gamma$, $\lambda$, $\delta_1$, and $\delta$
at grid points representing all combinations of $\rho_1$ and $\rho$ are attempted.
...
and $\gamma$ is set by the \texttt{cesEst} function
...
and in a manner similar to our approach 
to the Cobb-Douglas model~(see section \ref{sec:CD_Parameter_Estimation}),
we also fit along all possible CES boundaries.
...
Row~21 of Table~\ref{tab:CES_boundary_models} shows the generic three-factor CES equation,
...
yield equivalent CES boundary models,%
  \footnote{
  For example, all possible boundary parameter combinations 
  that include $\delta = 0$ yield the same boundary model, $y = \gamma A x_3$.
  }
so there are far fewer than the maximum possible number of degenerate equations~(81).%
  \footnote{
  There are three possible states (unspecified, lower boundary, upper boundary)
  for four parameters ($\delta_1$, $\sigma_1$, $\delta$, $\sigma$),
  which gives $3^4 = 81$ possible boundary models.
  }
Boundary models obtained when $\delta_1$ or $\delta$ is at an extreme value
are straightforward to derive from row~21 of Table~\ref{tab:CES_boundary_models}.
...
and the Leontief model applies.%
  ...
  $y = \gamma A \min (x_1, x_2)$, row 3 in Table~\ref{tab:CES_boundary_models}.
  }
...
and the linear model applies.%
  ...
  $y = \gamma A \left[ \delta_1 x_1 + (1 - \delta_1) x_2 \right]$,
  row 4 in Table~\ref{tab:CES_boundary_models}.
  }
...
can be estimated on that boundary using the degenerate equation.
The top portion of Table~\ref{tab:CES_boundary_models} shows 
two-parameter CES boundary models
(similar to the CES model without energy in Equation~\ref{eq:CES})
with $\delta~=~1$, $\sigma$~unknowable, and $x_3$~absent.
If the fitted parameters in a boundary model violate constraints, 
the boundary model is rejected.

\begin{table} \caption{CES boundary models (rows~1--20) and full model (bottom row). ORIGINAL.  **** DELETE EVENTUALLY ****} 
  \begin{center}
    \begin{tabular}{r c c c c l} 
      ...
      1. & 1           & ---        & 1          & ---      & $y = \gamma A x_1$ \\
      2. & 0           & ---        & 1          & ---      & $y = \gamma A x_2$ \\
      3. & ---         & 0          & 1          & ---      & $y = \gamma A \min (x_1, \, x_2)$ \\
%
      4. &             & $\infty$   & 1          & ---        
            & $y = \gamma A \left[ \delta_1 x_1 + (1 - \delta_1) x_2 \right]$ \\
%
      5. &             &            & 1          & ---
            & $y = \gamma A \left[ \delta_1 x_1^{-\rho_1} 
                    + \left( 1 - \delta_1 \right) x_2^{-\rho_1} \right]^{-1/\rho_1}$ \\
\midrule
      6. & ---         & ---        & 0          & ---      & $y = \gamma A x_3$ \\
      7. & 1           & ---        & ---        & 0        & $y = \gamma A \min (x_1, \, x_3)$ \\
      8. & 0           & ---        & ---        & 0        & $y = \gamma A \min (x_2, \, x_3)$ \\
      9. & ---         & 0          & ---        & 0        & $y = \gamma A \min (x_1, \, x_2, \, x_3)$ \\
%
     10. & 1           & ---        &            & $\infty$        
            & $y = \gamma A \left[ \delta x_1 + (1 - \delta) x_3 \right]$ \\
%
     11. & 0           & ---        &            & $\infty$        
            & $y = \gamma A \left[ \delta x_2 + (1 - \delta) x_3 \right]$ \\
%
     12. &             & $\infty$   &            & $\infty$        
            & $y = \gamma A \left\{ \delta \left[ \delta_1 x_1 + \left( 1 - \delta_1 \right) x_2 \right] 
                    + \left( 1 - \delta \right) x_3  \right\}$ \\
%
     13. & ---         & 0          &            & $\infty$        
            & $y = \gamma A \left[ \delta \min (x_1, \, x_2) + \left( 1 - \delta \right) x_3  \right]$ \\
%
     14. &             & $\infty$   & ---        & 0
            & $y = \gamma A \min \left[ \delta_1 x_1 + \left(1 - \delta_1 \right) x_2, \, x_3  \right]$ \\
%
     15. & 1           & ---        &         & 
            & $y = \gamma A \left[ \delta x_1^{-\rho} 
                    + \left(1 - \delta \right) x_3^{-\rho} \right] ^ {-1/\rho}$ \\ 
%
     16. & 0           & ---        &            & 
            & $y = \gamma A \left[ \delta x_2^{-\rho} + \left(1 - \delta \right) x_3^{-\rho} \right]^{-1/\rho}$ \\
%
     17. &             & $\infty$   &            &
            & $y = \gamma A \left\{ \delta \left[ \delta_1 x_1 
                    + \left( 1 - \delta_1 \right) x_2 \right]^{-\rho}  
                    + \left( 1 - \delta \right) x_3^{-\rho}  \right\} ^ {-1/\rho}$ \\
%
     18. & ---         & 0          &            & 
            & $y = \gamma A \left\{ \delta \left[ \min \left( x_1, \, x_2 \right) \right]^{-\rho}  
                    + \left( 1 - \delta \right) x_3^{-\rho}  \right\} ^ {-1/\rho}$ \\ 
%
     19. &             &            &            & $\infty$
...
%
     20. &            &            & ---     & 0
            & $y = \gamma A \min \left\{ \left[ \delta_1 x_1^{-\rho_1} 
                    + \left( 1 - \delta_1 \right) x_2^{-\rho_1} \right]^{-1/\rho_1}, \, x_3 \right\}$ \\ 
\midrule
         &            &            &         & 
            & $y = \gamma A \left\{ \delta \left[ \delta_1 x_1^{-\rho_1} 
                    + \left( 1 - \delta_1 \right) x_2^{-\rho_1} \right]^{\rho/\rho_1}  
                    + \left( 1 - \delta \right) x_3^{-\rho}  \right\} ^ {-1/\rho}$ \\ 
      \bottomrule
    \end{tabular}
  \end{center}
...
\begin{table} \caption{CES boundary models (rows~1--20) and full model (bottom row). 
                       Re-ordered, but with original model numbers. **** DELETE EVENTUALLY ****}
  \begin{center}
    \begin{tabular}{r c c c c l} 
      \toprule
      & $\delta_1$  & $\sigma_1$ & $\delta$   & $\sigma$ & Boundary model \\
...
\label{tab:CES_boundary_models} 
  \begin{center}
    \begin{tabular}{r c c c c l} 
      ...
      3. & ---         & ---        & 0          & ---      & $y = \gamma A x_3$ \\
      \midrule
      4. & ---         & 0          & 1          & ---      & $y = \gamma A \min (x_1, \, x_2)$ \\
      5. & 1           & ---        & ---        & 0        & $y = \gamma A \min (x_1, \, x_3)$ \\
      6. & 0           & ---        & ---        & 0        & $y = \gamma A \min (x_2, \, x_3)$ \\
%
      7. &             & $\infty$   & 1          & ---        
            & $y = \gamma A \left[ \delta_1 x_1 + (1 - \delta_1) x_2 \right]$ \\
%
...
%
     12. & 0           & ---        &            & 
            & $y = \gamma A \left[ \delta x_2^{-\rho} + \left(1 - \delta \right) x_3^{-\rho} \right]^{-1/\rho}$ \\
      \midrule
     13. & ---         & 0          & ---        & 0        & $y = \gamma A \min (x_1, \, x_2, \, x_3)$ \\
%
     14. & ---         & 0          &            & $\infty$        
            & $y = \gamma A \left[ \delta \min (x_1, \, x_2) + \left( 1 - \delta \right) x_3  \right]$ \\
%
...
and its parameters are presented in graphs herein.
In special cases, two boundary models will be equivalent.
For example, boundary model~3 is identical to boundary model~1~(2)
if $x_1$ is always less than (greater than) $x_2$ for all years.
If two or more models have the same $sse$ (to six digits), 
the model with the lowest row number in Table~\ref{tab:CES_boundary_models} 
is deemed the winning model,
thereby ensuring that the simpler model is selected.

**** Edit the next paragraphs after we finalize the appearance
of the distribution graphs. 
Also, add a discussion of the boundary models and how the reduce 
the amount of data available in the bar graphs. ****
...
The gray crosshairs on each graph shows the location 
...
<<CES_lambda_gamma_graph, fig.pos="H", fig.align='center', fig.width=6.5, fig.height=8.5, fig.cap="$\\lambda$ and $\\gamma$ values for CES resample models.">>=
...
<<CES_sigma1_delta1_graph, fig.pos="H", fig.align='center', fig.width=6.5, fig.height=8.5, fig.cap="$\\sigma_1$ and $\\delta_1$ values for CES resample models.">>=
standardScatterPlot(data=cesCoeffsData, mapping=aes(delta_1, sigma_1), 
...
standardScatterPlot(data=filter(cesCoeffsData, nestStrParen != "(iK + iL) + ()"),
...
standardTriPlot(data=cesCoeffsData, facet_formula=country~nestStrParen, alpha=0.05)
@
...
<<abg_trajectories, eval=FALSE>>=
# This code can be run to test the conclusion about the LINEX model meeting constraints.
# Need to put oModels in the environment much earlier in this file.
# This chunk of R code has eval=FALSE, because it is not a part of the paper.
abgplotdata <- fortify(oModels$Calvin$IR$linex$`iK+iL+iQp`)
triPlot(data=abgplotdata, grid_lines=5, x=alpha, y=beta, z=gamma, geom=geom_line, aes_string="colour = iYear")
@
...
% This is the original sigma table. 
% It has been replaced by macros that make for more-efficient LaTeX code.
...
% label is tab:sigma_US
\eostable{US}
          {\firstrow{C}{C}{$\sim\sim\sim$}{$\sim\sim\sim$}}
          {\secondrow{$\sim\sim\sim$}{$\sim\sim\sim$}{C}{C}{C}{C}}
          {\thirdrow{C}{C}{C}}

% label is tab:sigma_UK
\eostable{UK}
          {\firstrow{C}{$\sim\sim\sim$}{C}{C}}
          {\secondrow{C}{C}{C}{C}{$\sim\sim\sim$}{$\sim\sim\sim$}}
          {\thirdrow{C}{C}{C}}
          
% label is tab:sigma_JP
\eostable{JP}
          {\firstrow{c}{C}{$\sim\sim\sim$}{C}}
          {\secondrow{C}{C}{C}{C}{C}{C}}
          {\thirdrow{C}{C}{C}}

% label is tab:sigma_CN
\eostable{CN}
          {\firstrow{$\sim\sim\sim$}{C}{C}{S}}
%           {\secondrow{S {\scriptsize (driven by $le$?)}}{S}{C}{C}{c}{c {\scriptsize (driven by $kl$?)}}}
          {\secondrow{S\tablefootnote{Driven by $le$?}}{S}{C}{C}{c}{c\tablefootnote{Driven by $kl$?}}}
          {\thirdrow{C}{C}{S}}

% label is tab:sigma_ZA
\eostable{ZA}
          {\firstrow{c$\sim$}{$\sim\sim\sim$}{C}{$\sim\sim\sim$}}
          {\secondrow{C}{C}{C}{C}{$\sim\sim\sim$}{C\tablefootnote{Points on right are all same.}}}
          {\thirdrow{c}{C}{C}}

% label is tab:sigma_SA
\eostable{SA}
          {\firstrow{$\sim\sim\sim$}{$\sim\sim\sim$}{C}{C}}
          {\secondrow{C}{C}{$\sim\sim\sim$}{$\sim\sim\sim$}{$\sim\sim\sim$}{$\sim\sim\sim$}}
          {\thirdrow{$\sim\sim\sim$}{C}{C}}

% label is tab:sigma_IR
\eostable{IR}
          {\firstrow{$\sim\sim\sim$}{$\sim\sim\sim$}{$\sim\sim\sim$}{$\sim\sim\sim$}}
          {\secondrow{$\sim\sim\sim$}{$\sim\sim\sim$}{$\sim\sim\sim$}{$\sim\sim\sim$}{C}{C}}
          {\thirdrow{$\sim\sim\sim$}{$\sim\sim\sim$}{$\sim\sim\sim$}}

% label is tab:sigma_TZ
\eostable{TZ}
          {\firstrow{C}{C}{$\sim\sim\sim$}{C}}
          {\secondrow{$\sim\sim\sim$}{$\sim\sim\sim$}{C}{C}{C}{C}}
          {\thirdrow{C}{$\sim\sim\sim$}{C}}

% label is tab:sigma_ZM
\eostable{ZM}
          {\firstrow{$\sim\sim\sim$}{$\sim\sim\sim$}{$\sim\sim\sim$}{$\sim\sim\sim$}}
