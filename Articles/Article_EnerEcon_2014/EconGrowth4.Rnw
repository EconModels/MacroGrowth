...
In addition to the above trials, we also fit along all boundaries.
Table~\ref{tab:CES_boundary_models} shows the degenerate forms of CES that are fitted,
assuming
%
...
\end{equation}
%
where $x_1$, $x_2$, and $x_3$ are permutations of $k$, $l$, and $e$.
...
is deemed the winning model to be presented in graphs.
