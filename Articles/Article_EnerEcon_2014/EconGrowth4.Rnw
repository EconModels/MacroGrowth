...
In addition to the above trials, we fit along all boundaries.
Table~\ref{tab:CES_boundary_models} shows the degenerate forms of CES on each boundary.
Equation~\ref{eq:ces_generic} accompanies Table~\ref{tab:CES_boundary_models}
and provides the generic CES equation
in which factors of production ($x_1$, $x_2$, $x_3$) 
...
(because it has been eliminated from the model), and
an empty space indicates that the parameter is to be estimated on that boundary.
...
\label{tab:CES_boundary_models} 
  \begin{center}
    \begin{tabular}{c c c c l} 
      \toprule
      $\delta_1$  & $\delta$   & $\sigma_1$ & $\sigma$ & Boundary model \\
      \midrule
      1           & 1          & ---        & ---      & $y = \gamma A x_1$ \\
      0           & 1          & ---        & ---      & $y = \gamma A x_2$ \\
      ---         & 0          & ---        & ---      & $y = \gamma A x_3$ \\
      ---         & 1          & 0          & ---      & $y = \gamma A \min (x_1, \, x_2)$ \\
...
%
                  & 1          &            & ---
            & $y = \gamma A \left[ \delta x_1^{-\rho_1} 
                    + \left( 1 - \delta \right) x_2^{-\rho_1} \right]^{-1/\rho_1}$ \\
%
      0           &            & ---        & 
