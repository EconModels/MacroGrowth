...
$\delta$~=~0~or~1, and
$\sigma$~=~0~or~$\infty$.
Equation~\ref{eq:ces_generic} shows the generic three-factor CES equation,
wherein factors of production ($x_1$,~$x_2$,~$x_3$) 
are permutations of ($k$,~$l$,~$e$).
%
\begin{equation} \label{eq:ces_generic}
...
of 20 degenerate equations found along parameter boundaries.
...
are straightforward to derive from Equation~\ref{eq:ces_generic}.
When $\sigma_1$ or $\sigma$ is 0, 
factors of production are complimentary,
and the Leontief model applies.%
  \footnote{
  For the case of $\sigma_1 = 0$ and $\delta~=~1$, the Leontief model is
  $y = \gamma A \min (x_1, x_2)$.
  }
When $\sigma_1$ or $\sigma$ is $\infty$, 
factors of production are subsitutible,
and the linear model applies.%
  \footnote{
  For the case of $\sigma_1 = \infty$ and $\delta~=~1$, the linear model is
...
\begin{table} \caption{Boundary models for the CES equation.} 
...
The fit with lowest $sse$ of all above trials 
is deemed the winning model and presented in graphs.
