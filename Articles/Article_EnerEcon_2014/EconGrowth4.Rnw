...
and in a manner similar to our approach to the Cobb-Douglas model,
we fit along all possible CES boundaries.
...
the model with the lowest row number in Table~\ref{tab:CES_boundary_models} is deemed the winner,
thereby ensuring that the simplest model is selected.
