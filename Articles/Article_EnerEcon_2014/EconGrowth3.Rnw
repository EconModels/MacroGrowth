...
\newcommand{\firstrow}[4]{\multirow{2}{*}{$\sigma_1$} & ($\underline{kl}$) & #1 
                                  &  & 
                                        & &  \\
                            & ($\underline{kl}$)$e$ & #2
                                  & ($\underline{ek}$)$l$ & #3
...
economy. 
That mainstream economic growth theory ignores energy is, therefore, 
...
for the Chinese economy. **** More detail here about nesting? --MKH ****  
...
rebound and backfire effects.~\citep{Sorrell:2007um}
...
The \emph{cost share theorem} from mainstream economics states that 
output elasticities 
$\alpha$, $\beta$, and $\gamma$ 
(if augmenting the production function by energy)
can be calibrated from national accounts data as the 
cost shares of capital, labor, and energy, respectively.
...
Although no market ever exhibits perfect competition, 
in our opinion, energy markets are \emph{least} likely to be ``perfect.''
Many governments have parastatal or full control 
of electricity generation and/or fuel markets.  
...
If energy markets are farther from ``perfect'' 
than capital and/or labor markets, 
then energy is the factor of production least likely to have its
output elasticity~($\gamma$) be well-represented 
by cost share as reported in national accounts.
...
we will further test whether different types of countries 
...
**** The previous sentences sound like we'll show that some groups of countries
have similar charactersitics. 
That's what we thought we would find, 
but our new conclusion is that one must treat each country 
individually. 
We should consider revising the previous few sentences. --MKH ****

**** Suggest deleting the next two sentences:
...
**** We need to think about the order of the subsections below. --MKH ****
...
For other developed economies (Japan), the results of CD and CES models point in opposite directions.  
...
and~\ref{fig:CES_sigma1_delta1_graph}. 
In the tables,
...
where an indication of substitutability between labor and energy is observed~(S and s).
...
that is rarely reported in the literature.
For some economies (Iran and Zambia), 
...
Presumably the best data in the world 
(United States, United Kingdom, and Japan) exhibits large uncertainty. 
(See United States and United Kingdom rows in Figures~\ref{fig:CD_Ternary_Scatter_Graph}  
...
%++++++++++++++++++++++++++++++
\subsection{Implications of parameter uncertainty} 
...
**** Move this paragraph elsewhere? --MKH ****
...
easy parameter estimation process.
...
LINEX struggles, especially with the Saudi Arabian and Tanzanian economies.%
  ...
  CES contains 6 fitted parameters; Cobb-Douglas, 4; and LINEX, 3.
  We expect models with more parameters to provide a better fit to historical data.
  }$^,$%
  \footnote{
  We also note that in contrast to our approach, shared by \citet{Warr:2012cg},
  of fitting constant $a_0$ and $c_t$ values, 
...
The energy-augmented CES production function presents ambiguity to the energy analyst
in the form of the choice of factor nesting.
We showed above that nesting can significantly affect 
the interpretation of macroecnomic dynamics, especially 
the relative importance of factors of production~(\ref{fig:CES_ternary_graph}) and 
...
exhibits similar structural ambiguity.

Thus, Cobb-Douglas or LINEX are preferred on this criterion.
...
smaller uncertainties in fitted parameters than the CES production function.
...
provides an easy way to respect fitted parameter constraints, and
...
On the 40-node \texttt{dahl} supercomputer at Calvin College~\citep{Adams:2008aa},
...
it is very difficult to enforce constraints during the fitting process.%
  \footnote{
  See \citet{Kummel:2011aa} for one approach to respecting parameter constraints 
  for the LINEX function. 
  }
Indeed, we found in Section~\ref{sec:LINEX_Parameter_Estimation}
...
Thus, none of the prodcution functions are preferred over the others on this criterion.
...
If you need to represent rebound effects (not just backfire), 
...
Unfortunately, we resampling analyses show that estimated values 
...
If the LINEX function is to be used, we recommend that 
\citeapos{Kummel:2011aa} fitting approach (logistic curves for $a_0$ and $c_t$) be employed
and that the results be thoroughly tested and compared against other production functions.
