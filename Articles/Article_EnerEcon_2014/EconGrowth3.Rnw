...
for the Chinese economy. **** Martin: More detail here about nesting? --MKH ****  
...
the mathematical structure of the CES model itself.%
  \footnote{
  Numbers in parentheses indicate rows in Table~\ref{tab:ces_sensitivity_table}.
  }
...
$\rho_1$ cancels.
As a result, 
$sse$ becomes increasingly insensitive to $\rho_1$ (and, therefore, $\sigma_1$)
as $\delta_1 \rightarrow 0$ or $\delta_1 \rightarrow 1$.
Decreasing sensitivity leads to reduced precision in estimating $\sigma_1$.
%
(3)~If $\delta = 0$, $\delta_1$ and $\rho_1$ are eliminated.
Furthermore, with $\delta = 0$, $\rho$ cancels, 
and $sse$ becomes insensitive to $\rho$ and, therefore, $\sigma$.
Thus, as $\delta \rightarrow 0$, the precision with which 
$\delta_1$, $\sigma_1$, and $\sigma$ are estimated declines.
%
(4)~If $\delta = 1$, $\rho$ cancels, 
and $sse$ becomes insensitive to values of $\sigma$.
Given that both (a)~ the \texttt{cesEst} fitting algorithm
still uses a meaningless value for $\rho$ when $\delta \rightarrow 1$
and (b)~the ratio $\rho/\rho_1$ appears in the CES equation,
the value of $\rho_1$ will also become highly uncertain 
...
Thus, as $\sigma_1 \rightarrow 0$, estimation of $\delta_1$ becomes increasingly imprecise.
...
\end{equation}
%
and $\delta$ is eliminated.
Thus, as $\sigma \rightarrow 0$, estimation of $\delta$ becomes increasingly imprecise.
