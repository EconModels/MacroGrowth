...
are beneficial as an explanation for growth (TZ) or strong indication otherwise (ZM).  
...
**** If we stick with these tables, do we want to use $\sim\sim\sim$ to indicate uncertainty? ****
...
Here is a first cut at some criteria and a recommendation.

Allows assymetric uncertainty estimation via resampling: all work fine.

Best match to historical data: CES. But CD is close. LinEx struggles.
...
Easy to respect constraints during fitting process: CD and CES easy. LinEx difficult.
...
If precise fitting to historical data is paramount (and you're unconcerned about uncertainties
in fitted coefficients), CES is the best.

If you need to represent rebound effects (not just backfire), 
CES must be employed, 
because the CES function was found by \citet{Saunders:2008aa} to be the only one of the production
functions studied here that is capable of representing rebound rebound in addition to backfire.
However, its ability to do so is tied to the values of the elasticity of substitution.
Unfortunately, we find that value of the elasticity of substitution is often highly uncertain.
Thus, whether the CES function is helpful for estimating both rebound and backfire effects
is questionable.
...
**** Do we want to summarize the discussion here? --MKH ****
