...
print(getwd())
print(file.exists("data_postprocessed/Calvin_Coeffs.Rdata"))
...
spaghettiPlot(data=subset(cesFittedData, Country=="CN" & nestStrParen=="(iK + iL) + (iQp)"), level=spaghettiGraphLevel) +
    labs(y="Indexed GDP (1980 = 1 or 1991 = 1")
@
...
dataCN <- fortify(oModels$Calvin$CN$linex$`iK+iL+iQp`)
...
dataIR <- fortify(oModels$Calvin$IR$linex$`iK+iL+iQp`)
