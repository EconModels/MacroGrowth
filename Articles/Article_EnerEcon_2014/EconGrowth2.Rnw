...
coeffs_df <- tbl_df(Calvin_Coeffs)
# Load fitted data from all models in one large data frame
fitted_df <- tbl_df(Calvin_Fitted)

# Load the original models.
oModels <- Calvin_Models
@
...
dataCN <- fortify(oModels$Calvin$CN$linex$`iK+iL+iQ`)
...
dataIR <- fortify(oModels$Calvin$IR$linex$`iK+iL+iQ`)
