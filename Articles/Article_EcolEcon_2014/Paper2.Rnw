...
The term $\mathrm{e}^{\lambda (t-t_0)}$ is known as total factor productivity~($A$),
...
Assuming that parameters $\theta$, $\alpha$, and $\beta$ are known from parameter estimation,
...
The fraction of GDP growth explained by the Solow residual~($f$)
can be given as
%
...
%
\begin{equation} \label{eq:f_finite_diff}
...
\end{equation}
%
which is an estimate to the true value of $f$ given by Equation~\ref{eq:frac_growth_explained_by_solow}.
...
the fraction of economic growth explained by the Solow residual ($f$)
by $f_{ij}$
with $i$ and $j$ in Equation~\ref{eq:f_finite_diff} separated by \Sexpr{lag} years.
...
standardScatterPlot(data=subset(cesCoeffsDataXp, nestStrParen != "(iK + iL) + ()"),
...
cesFittedDataUS_xp <- subset(AllFitted, subset = model=="ces" & Country=="US" & (energy=="iXp" | energy=="none"))
...
cesFittedDataUK_xp <- subset(AllFitted, subset = model=="ces" & Country=="UK" & (energy=="iXp" | energy=="none"))
...
cdFittedDataJP_xp <- subset(AllFitted, subset = model=="ces" & Country=="JP" & (energy=="iXp" | energy=="none"))
...
cdFittedDataAT_xp <- subset(AllFitted, subset = model=="ces" & Country=="AT" & (energy=="iXp" | energy=="none"))
...
cdFittedDataPT_xp <- subset(AllFitted, subset = model=="ces" & Country=="PT" & (energy=="iXp" | energy=="none"))
...
cdFittedDataCN_xp <- subset(AllFitted, subset = model=="ces" & Country=="CN" & (energy=="iXp" | energy=="none"))
spaghettiPlot(data=cdFittedDataCN_xp, facet_formula=Source~nestStrParen, level=spaghettiGraphLevel) +
    labs(y="Indexed GDP (1960=1)") +
  scale_x_continuous(breaks=c(1960, 1980, 2000))
@

<<CES_Residual_Plot, fig.pos="H", fig.align='center', fig.width=6, fig.height=6, fig.cap="CES with $x_p$ Residual Plots.">>=
# Use residuals_df to make a graph.
ces_xp_residuals_df <- subset(residuals_df, subset = modelType=="ces" & (energyType=="iXp" | energyType=="none"))
