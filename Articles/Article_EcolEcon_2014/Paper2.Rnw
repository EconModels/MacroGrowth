...
REXS1960_Coeffs <- loadPostProcessedData(Source = "REXS1960", kind = "coeffs")
IST_Coeffs <- loadPostProcessedData(Source = "IST", kind = "coeffs")
Leeds_Coeffs <- loadPostProcessedData(Source = "Leeds", kind = "coeffs")
...
REXS1960_Fitted <- loadPostProcessedData(Source = "REXS1960", kind = "fitted")
IST_Fitted <- loadPostProcessedData(Source = "IST", kind = "fitted")
Leeds_Fitted <- loadPostProcessedData(Source = "Leeds", kind = "fitted")
...
Discuss economies here. EU15 and MX?
...
<<Historical_Data_1, fig.pos="H", fig.align='center', fig.width=6.6, fig.height=3, fig.cap="Historical data.", eval=TRUE>>=
...
historicalPlot(mdata, 
  ...
  xlab("") + 
  ylab("Indexed (1960=1 or 1978=1)")
@

Figure~\ref{fig:Historical_Data_2} shows the same data in a different form.

<<Historical_Data_2, fig.pos="H", fig.align='center', fig.width=8, fig.height=3, fig.cap="Historical data.", eval=TRUE>>=
historicalPlot(mdata, 
               facet_formula=Country~variable, 
               lines_formula=Source+value~Year, 
               line_types=c(1,2,3)) +  
  xlab("") + 
  ylab("Indexed (1960=1 or 1978=1)")
@
...
standardScatterPlot(data=cdCoeffsData, mapping=aes(scale, lambda), facet_formula=energy~Source, alpha=0.05) +
    scale_x_continuous(limits=c(0.95, 1.05), breaks=c(0.95, 1.0)) +
...
<<CD_GDP_Spaghetti_Graph, fig.pos="H", fig.align='center', fig.width=4, fig.height=4.5, fig.cap="UK Cobb-Douglas fitted models.">>=
...
<<LINEXq_Resample_Graph, fig.pos="H", fig.align='center', fig.width=4, fig.height=4, fig.cap="UK LINEX model (with energy) resampling results.">>=
...
standardScatterPlot(data=linexCoeffsData, aes(a_0, c_t), facet_formula=energy~Source, alpha=0.05) +
    scale_x_continuous(breaks=c(0.6, 0.8, 1.0)) +
    labs(x=expression(a[0]), y=expression(c[t]))
@

<<LINEX_Spaghetti_Graph, fig.pos="H", fig.align='center', fig.width=4, fig.height=4, fig.cap="UK LINEX fitted models.">>=
...
REXS1960_Models <- loadPostProcessedData(Source = "REXS1960", kind = "models")
...
IST_Models <- loadPostProcessedData(Source = "IST", kind = "models")
...
Leeds_Models <- loadPostProcessedData(Source = "Leeds", kind = "models")
