...
standardScatterPlot(data=subset(cesCoeffsDataXf, nestStrParen != "(iK + iL) + ()"),
                    mapping=aes(x=delta, y=sigma, color=Source), 
                    facet_formula=country~nestStrParen, alpha=0.05) +
    coord_trans(y="sigma") +
    scale_x_continuous(breaks=c(0, 0.5, 1.0)) +
...
# Use residuals_df to make a graph.
ces_u_residuals_df <- subset(residuals_df, subset = modelType=="ces" & (energyType=="iU" | energyType=="none"))
