...
Graph(s) of historical data here. 
...
<<CD_lambda_theta_graph, fig.pos="H", fig.align='center', fig.width=4, fig.height=4.5, fig.cap="$\\lambda$ and $\\theta$ values for UK Cobb-Douglas resample models.">>=
cdCoeffsData <- subset(AllCoeffs, subset = model=="cd")
standardScatterPlot(data=cdCoeffsData, mapping=aes(scale, lambda), facet_formula=energy~Source, alpha=0.05) +
...
<<CD_Ternary_Scatter_Graph, fig.pos="H", fig.align='center', fig.width=3.9, fig.height=4, fig.cap="UK Cobb-Douglas resample model parameters.">>=
standardTriPlot(data=cdCoeffsData, facet_formula=energy~Source)
@

<<CD_GDP_Spaghetti_Graph, fig.pos="H", fig.align='center', fig.width=4, fig.height=4.5, fig.cap="Cobb-Douglas fitted models.">>=
cdFittedData <- subset(AllFitted, subset = model=="cd")
spaghettiPlot(data=cdFittedData, facet_formula=energy~Source, level=spaghettiGraphLevel) +
...
<<LINEXq_Resample_Graph, fig.pos="H", fig.align='center', fig.width=4, fig.height=4, fig.cap="LINEX model (with energy) resampling results.">>=
linexCoeffsData <- subset(AllCoeffs, subset = model=="linex")
standardScatterPlot(data=linexCoeffsData, aes(a_0, c_t), facet_formula=energy~Source, alpha=0.05) +
    labs(x=expression(a[0]), y=expression(c[t]))
@

<<LINEX_Spaghetti_Graph, fig.pos="H", fig.align='center', fig.width=4, fig.height=4, fig.cap="LINEX fitted models.">>=
linexFittedData <- subset(AllFitted, subset = model=="linex")
spaghettiPlot(data=linexFittedData, facet_formula=energy~Source, level=spaghettiGraphLevel) +
    labs(y="Indexed GDP (1960=1)")
@
...
Figure~\ref{fig:LINEX_abg_traj_graph_UK}
show trajectories for the UK and useful work ($u$) from the Leeds data set.

<<LINEX_abg_traj_graph_REXS_UK_U, fig.pos="H", fig.align='center', fig.width=3, fig.height=3.5, fig.cap="LINEX trajectory for REXS, UK, $u$.">>=
...
<<LINEX_abg_traj_graph_IST_UK_U, fig.pos="H", fig.align='center', fig.width=3, fig.height=3.5, fig.cap="LINEX trajectory for IST, UK, $u$.">>=
...
<<LINEX_abg_traj_graph_Leeds_UK_U, fig.pos="H", fig.align='center', fig.width=3, fig.height=3.5, fig.cap="LINEX trajectory for Leeds, UK, $u$.">>=
