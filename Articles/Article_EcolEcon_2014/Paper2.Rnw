...
<<Historical_Data_1, fig.pos="H", fig.align='center', fig.width=6.6, fig.height=6, fig.cap="Historical data.", eval=TRUE>>=
...
<<Historical_Data_2, fig.pos="H", fig.align='center', fig.width=8, fig.height=6, fig.cap="Historical data.", eval=TRUE>>=
historicalPlot(mdata, 
...
<<CD_lambda_theta_graph, fig.pos="H", fig.align='center', fig.width=6, fig.height=4.5, fig.cap="$\\lambda$ and $\\theta$ values for Cobb-Douglas resample models.">>=
...
standardScatterPlot(data=cdCoeffsData, mapping=aes(x=scale, y=lambda, color=Source), 
    ...
    scale_x_continuous(limits=c(0.8, 1.2), breaks=c(0.9, 1.0, 1.1)) +
    scale_y_continuous(limits=c(-0.1, 0.2), breaks=c(0.0, 0.1, 0.2)) +    
    labs(x=expression(theta), y=expression(lambda)) +
    guides(colour=guide_legend(override.aes = aes(alpha=1))) #+
#     geom_density2d()
@

<<CD_Ternary_Scatter_Graph, fig.pos="H", fig.align='center', fig.width=6, fig.height=4, fig.cap="Cobb-Douglas resample model parameters.">>=
standardTriPlot2(data=cdCoeffsData, 
                mapping=aes(color=Source),
                facet_formula=country~energy) +
    guides(colour=guide_legend(override.aes = aes(alpha=1)))
@

<<CD_GDP_Spaghetti_Graphs_US, fig.pos="H", fig.align='center', fig.width=4, fig.height=2.5, fig.cap="US Cobb-Douglas fitted models.">>=
