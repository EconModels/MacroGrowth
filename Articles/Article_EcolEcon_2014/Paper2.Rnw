...
\begin{equation} \label{eq:frac_growth_explained_by_solow}
  f \equiv \frac{\lambda}{\frac{1}{y}\frac{\mathrm{d}y}{\mathrm{d}t}} 
            = 1 - \frac{\alpha \frac{1}{k}\frac{\mathrm{d}k}{\mathrm{d}t} 
                      + \beta \frac{1}{l}\frac{\mathrm{d}l}{\mathrm{d}t} 
                      + (1 - \alpha - \beta) \frac{1}{e}\frac{\mathrm{d}e}{\mathrm{d}t}}
                  {\frac{\lambda}{\frac{1}{y}\frac{\mathrm{d}y}{\mathrm{d}t}}}
\end{equation}
%
...
<<CD_solowPerc_time_series, fig.pos="H", fig.align='center', fig.width=6.5, fig.height=6, fig.cap='Fraction of GDP growth explained by Solow residual~($\\lambda$).'>>=
# Create a time series graph of the solowPerc values
ggplot(data=fdlambda, aes(x=Year, y=solowFrac, color=Source)) + 
  geom_point(size = 1.0) +
  facet_grid(Country ~ energyType) +
  labs(x = "", y = "Fraction of GDP growth explained by Solow residual") + 
