...
historicalData <- plyr::rbind.fill(REXS1960, IST, Leeds) # From the EconData package
...
extract_data <- function(x, name) {
  ...
  data.frame(year = (attr(x, which="data"))$Year, resid = resid(x), name=name, Source=Source, Country=Country,
             modelType=modelType, factorString=factorString, 
             nestStrParen=parseFactorString(factorString)$nestStrParen, energyType=energyType)
}
...
%
Note that the $\rho$ ($\sigma$) and $\delta$ parameters have different
...
\end{equation}
%
to ensure $\alpha + \beta = 1$ for constant returns to scale. 
...
cdCoeffsData <- filter(AllCoeffs, model=="cd")
...
<<CD_finite_diff_calcs, fig.pos="H", fig.align='center', fig.width=6.5, fig.height=6, fig.cap='Solow residual time series.'>>=
...
lag <- 5       # Desired lag for calculations. Set to n for differences across n years.
pad <- "tail"  # Desired padding for NA values
fd <- m_histData %>% group_by(Source, Country, energyType) %>% arrange(Year) %>% 
  mutate(dydt = ediff(iGDP, pad=pad, lag=lag) / ediff(Year, pad=pad, lag=lag)) %>%
  mutate(dkdt = ediff(iK, pad=pad,   lag=lag) / ediff(Year, pad=pad, lag=lag)) %>%
  mutate(dldt = ediff(iL, pad=pad,   lag=lag) / ediff(Year, pad=pad, lag=lag)) %>%
  mutate(dedt = ediff(iE, pad=pad,   lag=lag) / ediff(Year, pad=pad, lag=lag))

origCDKLCoeffs <- filter(AllCoeffs, method=="orig", model=="cd")
...
labg <- apply(fd[ , c('Country', 'Source', 'energyType')], 1, function(x){
    row <- as.data.frame(filter(origCDKLCoeffs, 
...
fdlambda <- mutate(fd, lambda_diff = dydt/iGDP - (alpha/iK*dkdt + beta/iL*dldt + gamma/iE*dedt))

# Calculate the fraction of GDP growth in each year that is explained by the solow residual
fdlambda <- mutate(fdlambda, solowFrac = lambda_diff / (dydt/iGDP))

# Calculate average values for lambda and solowFrac
avgs <- fdlambda %>% 
  group_by(Source, Country, energyType) %>% 
  summarise(
    avg.lambda_diff = mean(lambda_diff, na.rm=TRUE),
    avg.solowFrac = mean(solowFrac, na.rm=TRUE)
    )

@

Given the estimates for $\alpha$, $\beta$, and $\gamma$ for the Cobb-Douglas model,
we can estimate values for $\lambda$ by $\lambda_{i,j}$,
with $i$ and $j$ in Equation~\ref{eq:lambdaij} separated by \Sexpr{lag} years.
The results are shown in Figure~\ref{fig:CD_lambda_time_series}.
Note that data spanning years~$i \rightarrow j$ is plotted at year~$i$.
...
We can also estimate values for 
the fraction~($f$) of economic growth explained by the Solow residual
by $f_{ij}$
with $i$ and $j$ in Equation~\ref{eq:f_finite_diff_2} separated by \Sexpr{lag} years.
The results are shown in Figure~\ref{fig:CD_solowPerc_time_series}. 
Note that data spanning years~$i \rightarrow j$ is plotted at year~$i$.

<<CD_solowPerc_time_series, fig.pos="H", fig.align='center', fig.width=6.5, fig.height=6, fig.cap='Fraction of GDP growth explained by Solow residual.'>>=
# Create a time series graph of the solowPerc values
ggplot(data=fdlambda, aes(x=Year, y=solowFrac, color=Source)) + 
  geom_point(size = 1.0) +
  facet_grid(Country ~ energyType) +
  labs(x = "", y = "f") + 
...
# Use this result several times later. Saves lots of time doing it this way.
cdFittedData <- filter(AllFitted, model=="cd")

cdFittedDataUS <- filter(cdFittedData, Country=="US")
spaghettiPlot(data=cdFittedDataUS, facet_formula=Source~energy, level=spaghettiGraphLevel) +
    labs(y="Indexed GDP (1960=1)") +
  scale_x_continuous(breaks=c(1960, 1980, 2000))
@

<<CD_GDP_Spaghetti_Graphs_UK, fig.pos="H", fig.align='center', fig.width=5, fig.height=4.5, fig.cap="UK Cobb-Douglas fitted models.">>=
cdFittedDataUK <- filter(cdFittedData, Country=="UK")
spaghettiPlot(data=cdFittedDataUK, facet_formula=Source~energy, level=spaghettiGraphLevel) +
    labs(y="Indexed GDP (1960=1)") +
  scale_x_continuous(breaks=c(1960, 1980, 2000))
@

<<CD_GDP_Spaghetti_Graphs_JP, fig.pos="H", fig.align='center', fig.width=4, fig.height=2, fig.cap="JP Cobb-Douglas fitted models.">>=
cdFittedDataJP <- filter(cdFittedData, Country=="JP")
...
cdFittedDataAT <- filter(cdFittedData, Country=="AT")
spaghettiPlot(data=cdFittedDataAT, facet_formula=Source~energy, level=spaghettiGraphLevel) +
    labs(y="Indexed GDP (1960=1)") +
  scale_x_continuous(breaks=c(1960, 1980, 2000))
@

<<CD_GDP_Spaghetti_Graphs_PT, fig.pos="H", fig.align='center', fig.width=5, fig.height=2, fig.cap="PT Cobb-Douglas fitted models.">>=
cdFittedDataPT <- filter(cdFittedData, Country=="PT")
spaghettiPlot(data=cdFittedDataPT, facet_formula=Source~energy, level=spaghettiGraphLevel) +
    labs(y="Indexed GDP (1960=1)") +
  scale_x_continuous(breaks=c(1960, 1980, 2000))
@

<<CD_GDP_Spaghetti_Graphs_CN, fig.pos="H", fig.align='center', fig.width=4, fig.height=2, fig.cap="CN Cobb-Douglas fitted models.">>=
cdFittedDataCN <- filter(cdFittedData, Country=="CN")
...
cd_residuals_df <- filter(residuals_df, modelType=="cd")
...
are shown in 
Figures~\ref{fig:CES_lambda_gamma_graph_Xp}--\ref{fig:CES_sigma_delta_graph_Xp}. 
The white crosshair on each graph shows the location 
of the fit to historical data. 
In Figures~\ref{fig:CES_sigma1_delta1_graph_Xp} and~\ref{fig:CES_sigma_delta_graph_Xp},
...
Figures \ref{fig:CES_sigma1_delta1_graph_Xp} and \ref{fig:CES_sigma_delta_graph_Xp}
are equivalent.
...
%------------------------------
\subsubsection{CES with primary exergy ($x_p$)} 
\label{sec:ces_xp}
%------------------------------

The following graphs show results for the CES model using primary exergy ($x_p$).

<<CES_lambda_gamma_graph_Xp, fig.pos="H", fig.align='center', fig.width=6.5, fig.height=6.0, fig.cap="$\\lambda$ and $\\gamma$ values for CES resample models with primary exergy ($x_p$).">>=
cesCoeffsDataXp <- filter(AllCoeffs, model=="ces", nestStrParen %in% c("(iK + iL) + ()", 
                                                                      "(iK + iL) + (iXp)", 
                                                                      "(iL + iXp) + (iK)", 
                                                                      "(iXp + iK) + (iL)"))
standardScatterPlot(data=cesCoeffsDataXp, mapping=aes(x=gamma_coef, y=lambda, color=Source), 
...
<<CES_sigma1_delta1_graph_Xp, fig.pos="H", fig.align='center', fig.width=6.5, fig.height=6.5, fig.cap="$\\sigma_1$ and $\\delta_1$ values for CES resample models with primary exergy ($x_p$).">>=
standardScatterPlot(data=cesCoeffsDataXp, mapping=aes(x=delta_1, y=sigma_1, color=Source), 
                    facet_formula=country~nestStrParen, alpha=0.05) +
    coord_trans(y="sigma") +
...
<<CES_sigma_delta_graph_Xp, fig.pos="H", fig.align='center', fig.width=5.5, fig.height=6, fig.cap="$\\sigma$ and $\\delta$ values for CES resample models using primary exergy ($x_p$).">>=
standardScatterPlot(data=filter(cesCoeffsDataXp, nestStrParen != "(iK + iL) + ()"),
                    mapping=aes(x=delta, y=sigma, color=Source), 
                    facet_formula=country~nestStrParen, alpha=0.05) +
    coord_trans(y="sigma") +
    scale_x_continuous(breaks=c(0, 0.5, 1.0)) +
    scale_y_continuous(breaks=c(0, 0.5, 1.0, 2.0)) +
...
Figure~\ref{fig:CES_ternary_graphXp} shows 
$\alpha$, $\beta$, and $\gamma$ parameters for the CES model for all nests and countries.
Figure~\ref{fig:CES_ternary_graphXp} is comparable to
the ternary graph for the Cobb-Douglas models, 
Figure~\ref{fig:CD_Ternary_Scatter_Graph}.

<<CES_ternary_graphXp, fig.pos="H", fig.align='center', fig.width=6.5, fig.height=6, fig.cap="$\\alpha$, $\\beta$, and $\\gamma$ values for CES resample models with primary exergy ($x_p$).", eval=TRUE>>=
standardTriPlot(data=cesCoeffsDataXp, 
                mapping=aes(color=Source), 
                facet_formula=country~nestStrParen) +
  guides(colour=guide_legend(override.aes = aes(alpha=1)))
@

Figures~\ref{fig:CES_GDP_Spaghetti_Graph_US_xp}--\ref{fig:CES_GDP_Spaghetti_Graph_CN_xp} 
compare CES model predictions 
to historical data for all nestings, countries, and data sources. 
As with the Cobb-Douglas results, 
historical data are shown as a black line, 
the fit to historical data is shown as a white line, and 
the gray band encompasses \Sexpr{spaghettiGraphLevel*100}\% of the fits to resampled data.

<<CES_GDP_Spaghetti_Graph_US_xp, fig.pos="H", fig.align='center', fig.width=5, fig.height=3.5, fig.cap="US CES with $x_p$ fitted models.">>=
# Use this result later. Do it once to speed things up.
cesFittedData_xp <- filter(AllFitted, model=="ces", energy %in% c("iXp", "none"))

cesFittedDataUS_xp <- filter(cesFittedData_xp, Country=="US")
spaghettiPlot(data=cesFittedDataUS_xp, facet_formula=Source~nestStrParen, level=spaghettiGraphLevel) +
    labs(y="Indexed GDP (1960=1)") +
  scale_x_continuous(breaks=c(1960, 1980, 2000))
@

<<CES_GDP_Spaghetti_Graph_UK_xp, fig.pos="H", fig.align='center', fig.width=5, fig.height=4.5, fig.cap="UK CES with $x_p$ fitted models.">>=
cesFittedDataUK_xp <- filter(cesFittedData_xp, Country=="UK")
spaghettiPlot(data=cesFittedDataUK_xp, facet_formula=Source~nestStrParen, level=spaghettiGraphLevel) +
    labs(y="Indexed GDP (1960=1)") +
  scale_x_continuous(breaks=c(1960, 1980, 2000))
@

<<CES_GDP_Spaghetti_Graph_JP_xp, fig.pos="H", fig.align='center', fig.width=5, fig.height=2.5, fig.cap="JP CES with $x_p$ fitted models.">>=
cdFittedDataJP_xp <- filter(cesFittedData_xp, Country=="JP")
spaghettiPlot(data=cdFittedDataJP_xp, facet_formula=Source~nestStrParen, level=spaghettiGraphLevel) +
    labs(y="Indexed GDP (1960=1)") +
  scale_x_continuous(breaks=c(1960, 1980, 2000))
@

<<CES_GDP_Spaghetti_Graph_AT_xp, fig.pos="H", fig.align='center', fig.width=5, fig.height=3.5, fig.cap="AT CES with $x_p$ fitted models.">>=
cdFittedDataAT_xp <- filter(cesFittedData_xp, Country=="AT")
spaghettiPlot(data=cdFittedDataAT_xp, facet_formula=Source~nestStrParen, level=spaghettiGraphLevel) +
    labs(y="Indexed GDP (1960=1)") +
  scale_x_continuous(breaks=c(1960, 1980, 2000))
@

<<CES_GDP_Spaghetti_Graph_PT_xp, fig.pos="H", fig.align='center', fig.width=5, fig.height=2.5, fig.cap="PT CES with $x_p$ fitted models.">>=
cdFittedDataPT_xp <- filter(cesFittedData_xp, Country=="PT")
spaghettiPlot(data=cdFittedDataPT_xp, facet_formula=Source~nestStrParen, level=spaghettiGraphLevel) +
    labs(y="Indexed GDP (1960=1)") +
  scale_x_continuous(breaks=c(1960, 1980, 2000))
@

<<CES_GDP_Spaghetti_Graph_CN_xp, fig.pos="H", fig.align='center', fig.width=5, fig.height=2.5, fig.cap="CN CES with $x_p$ fitted models.">>=
cdFittedDataCN_xp <- filter(cesFittedData_xp, Country=="CN")
spaghettiPlot(data=cdFittedDataCN_xp, facet_formula=Source~nestStrParen, level=spaghettiGraphLevel) +
    labs(y="Indexed GDP (1960=1)") +
  scale_x_continuous(breaks=c(1960, 1980, 2000))
@

<<CES_Residual_Plot_xp, fig.pos="H", fig.align='center', fig.width=6, fig.height=6, fig.cap="CES with $x_p$ Residual Plots.">>=
# Use residuals_df to make a graph.
ces_xp_residuals_df <- filter(residuals_df, modelType=="ces", energyType %in% c("iXp", "none"))
ggplot(data=ces_xp_residuals_df, aes(x=year, y=resid, color=Source)) + 
  geom_point(size = 1.0) +
  facet_grid(Country ~ nestStrParen) +
  labs(x = "", y = "Residuals") +
  scale_x_continuous(breaks=c(1960, 1980, 2000)) + 
  xy_theme()
@
...
%------------------------------
\subsubsection{CES with final exergy ($x_f$)} 
\label{sec:ces_xf}
%------------------------------

The following graphs show results for the CES model using final exergy ($x_f$).

<<CES_lambda_gamma_graph_Xf, fig.pos="H", fig.align='center', fig.width=6.5, fig.height=6.0, fig.cap="$\\lambda$ and $\\gamma$ values for CES resample models with final exergy ($x_f$).">>=
# We need to keep only those combinations of country and Source that have final exergy.
temp <- filter(AllCoeffs, model=="ces", nestStrParen %in% c("(iK + iL) + (iXf)", 
                                                            "(iL + iXf) + (iK)", 
                                                            "(iXf + iK) + (iL)"))
countrySource <- paste0(temp$country, "+", temp$Source)
rm(temp) # No need to keep this data frame around
# Keep only unique combinations of country and Source in the temp data frame.
keep <- unique(countrySource)
cesCoeffsDataXf <- filter(AllCoeffs, model=="ces", nestStrParen %in% c("(iK + iL) + ()", 
                                                                      "(iK + iL) + (iXf)", 
                                                                      "(iL + iXf) + (iK)", 
                                                                      "(iXf + iK) + (iL)"))
cesCoeffsDataXf$countrySource <- paste0(cesCoeffsDataXf$country, "+", cesCoeffsDataXf$Source)
cesCoeffsDataXf <- filter(cesCoeffsDataXf, countrySource %in% keep)

standardScatterPlot(data=cesCoeffsDataXf, mapping=aes(x=gamma_coef, y=lambda, color=Source), 
                    facet_formula=country~nestStrParen, alpha=0.05) +
    scale_x_continuous(breaks=c(0.9, 1.0, 1.1)) +
    labs(x=expression(gamma), y=expression(lambda)) + 
    guides(colour=guide_legend(override.aes = aes(alpha=1)))
...
<<CES_sigma1_delta1_graph_Xf, fig.pos="H", fig.align='center', fig.width=6.5, fig.height=6.5, fig.cap="$\\sigma_1$ and $\\delta_1$ values for CES resample models with final exergy ($x_f$).">>=
standardScatterPlot(data=cesCoeffsDataXf, mapping=aes(x=delta_1, y=sigma_1, color=Source), 
                    facet_formula=country~nestStrParen, alpha=0.05) +
    coord_trans(y="sigma") +
    scale_x_continuous(breaks=c(0, 0.5, 1.0)) +
    scale_y_continuous(breaks=c(0, 0.5, 1.0, 2.0)) +
...
<<CES_sigma_delta_graph_Xf, fig.pos="H", fig.align='center', fig.width=5.5, fig.height=6, fig.cap="$\\sigma$ and $\\delta$ values for CES resample models using final exergy ($x_f$).">>=
standardScatterPlot(data=filter(cesCoeffsDataXf, nestStrParen != "(iK + iL) + ()"),
                    mapping=aes(x=delta, y=sigma, color=Source), 
                    facet_formula=country~nestStrParen, alpha=0.05) +
    coord_trans(y="sigma") +
    scale_x_continuous(breaks=c(0, 0.5, 1.0)) +
...
Figure~\ref{fig:CES_ternary_graphXf} shows 
$\alpha$, $\beta$, and $\gamma$ parameters for the CES model for all nests and countries.
Figure~\ref{fig:CES_ternary_graphXf} is comparable to
earlier ternary graphs, 
Figures~\ref{fig:CD_Ternary_Scatter_Graph} and~\ref{fig:CES_ternary_graphXp}.

<<CES_ternary_graphXf, fig.pos="H", fig.align='center', fig.width=6.5, fig.height=6, fig.cap="$\\alpha$, $\\beta$, and $\\gamma$ values for CES resample models with final exergy ($x_f$).", eval=TRUE>>=
standardTriPlot(data=cesCoeffsDataXf, 
                mapping=aes(color=Source), 
                facet_formula=country~nestStrParen) +
  guides(colour=guide_legend(override.aes = aes(alpha=1)))
@

Figures~\ref{fig:CES_GDP_Spaghetti_Graph_UK_xf}--\ref{fig:CES_GDP_Spaghetti_Graph_PT_xf} 
compare CES model predictions with final exergy ($x_f$)
to historical data for all nestings, countries, and data sources. 
As with the previous results, 
historical data are shown as a black line, 
the fit to historical data is shown as a white line, and 
the gray band encompasses \Sexpr{spaghettiGraphLevel*100}\% of the fits to resampled data.

<<CES_GDP_Spaghetti_Graph_UK_xf, fig.pos="H", fig.align='center', fig.width=5, fig.height=4.5, fig.cap="UK CES with $x_f$ fitted models.">>=
cesFittedData_xf <- filter(AllFitted, model=="ces", energy %in% c("iXf", "none"))
cesFittedData_xf$CountrySource <- paste0(cesFittedData_xf$Country, "+", cesFittedData_xf$Source)
cesFittedData_xf <- filter(cesFittedData_xf, CountrySource %in% keep)

cesFittedDataUK_xf <- filter(cesFittedData_xf, Country=="UK")
spaghettiPlot(data=cesFittedDataUK_xf, facet_formula=Source~nestStrParen, level=spaghettiGraphLevel) +
    labs(y="Indexed GDP (1960=1)") +
  scale_x_continuous(breaks=c(1960, 1980, 2000))
@

<<CES_GDP_Spaghetti_Graph_AT_xf, fig.pos="H", fig.align='center', fig.width=5, fig.height=4.5, fig.cap="AT CES with $x_f$ fitted models.">>=
cesFittedDataAT_xf <- filter(cesFittedData_xf, Country=="AT")
spaghettiPlot(data=cesFittedDataAT_xf, facet_formula=Source~nestStrParen, level=spaghettiGraphLevel) +
    labs(y="Indexed GDP (1960=1)") +
  scale_x_continuous(breaks=c(1960, 1980, 2000))
@

<<CES_GDP_Spaghetti_Graph_PT_xf, fig.pos="H", fig.align='center', fig.width=5, fig.height=4.5, fig.cap="PT CES with $x_f$ fitted models.">>=
cesFittedDataPT_xf <- filter(cesFittedData_xf, Country=="PT")
spaghettiPlot(data=cesFittedDataPT_xf, facet_formula=Source~nestStrParen, level=spaghettiGraphLevel) +
    labs(y="Indexed GDP (1960=1)") +
  scale_x_continuous(breaks=c(1960, 1980, 2000))
@

<<CES_Residual_Plot_xf, fig.pos="H", fig.align='center', fig.width=6, fig.height=6, fig.cap="CES with $x_f$ Residual Plots.">>=
# Use residuals_df to make a graph.
ces_xf_residuals_df <- filter(residuals_df, modelType=="ces", energyType %in% c("iXf", "none"))
ces_xf_residuals_df$CountrySource <- paste0(ces_xf_residuals_df$Country, "+", ces_xf_residuals_df$Source)
ces_xf_residuals_df <- filter(ces_xf_residuals_df, CountrySource %in% keep)

ggplot(data=ces_xf_residuals_df, aes(x=year, y=resid, color=Source)) + 
  geom_point(size = 1.0) +
  facet_grid(Country ~ nestStrParen) +
  labs(x = "", y = "Residuals") +
  scale_x_continuous(breaks=c(1960, 1980, 2000)) + 
...
%------------------------------
\subsubsection{CES with useful work ($u$)} 
\label{sec:ces_u}
%------------------------------

The following graphs show results for the CES model using useful work ($u$).

<<CES_lambda_gamma_graph_U, fig.pos="H", fig.align='center', fig.width=6.5, fig.height=6.0, fig.cap="$\\lambda$ and $\\gamma$ values for CES resample models with useful work ($u$).">>=
cesCoeffsDataU <- filter(AllCoeffs, model=="ces", nestStrParen %in% c("(iK + iL) + ()", 
                                                                      "(iK + iL) + (iU)", 
                                                                      "(iL + iU) + (iK)", 
                                                                      "(iU + iK) + (iL)"))
standardScatterPlot(data=cesCoeffsDataU, mapping=aes(x=gamma_coef, y=lambda, color=Source), 
                    facet_formula=country~nestStrParen, alpha=0.05) +
    scale_x_continuous(breaks=c(0.9, 1.0, 1.1)) +
    labs(x=expression(gamma), y=expression(lambda)) + 
    guides(colour=guide_legend(override.aes = aes(alpha=1)))
...
<<CES_sigma1_delta1_graph_U, fig.pos="H", fig.align='center', fig.width=6.5, fig.height=6.5, fig.cap="$\\sigma_1$ and $\\delta_1$ values for CES resample models with useful work ($u$).">>=
standardScatterPlot(data=cesCoeffsDataU, mapping=aes(x=delta_1, y=sigma_1, color=Source), 
                    facet_formula=country~nestStrParen, alpha=0.05) +
    coord_trans(y="sigma") +
    scale_x_continuous(breaks=c(0, 0.5, 1.0)) +
    scale_y_continuous(breaks=c(0, 0.5, 1.0, 2.0)) +
...
<<CES_sigma_delta_graph_U, fig.pos="H", fig.align='center', fig.width=5.5, fig.height=6, fig.cap="$\\sigma$ and $\\delta$ values for CES resample models using useful work ($u$).">>=
standardScatterPlot(data=filter(cesCoeffsDataU, nestStrParen != "(iK + iL) + ()"),
                    mapping=aes(x=delta, y=sigma, color=Source), 
                    facet_formula=country~nestStrParen, alpha=0.05) +
    coord_trans(y="sigma") +
    scale_x_continuous(breaks=c(0, 0.5, 1.0)) +
...
Figure~\ref{fig:CES_ternary_graphU} shows 
$\alpha$, $\beta$, and $\gamma$ parameters for the CES model with $u$ for all nests and countries.
Figure~\ref{fig:CES_ternary_graphU} is comparable to
earlier ternary graphs, 
Figures~\ref{fig:CD_Ternary_Scatter_Graph}, \ref{fig:CES_ternary_graphXp}, and~\ref{fig:CES_ternary_graphXf}.

<<CES_ternary_graphU, fig.pos="H", fig.align='center', fig.width=6.5, fig.height=6, fig.cap="$\\alpha$, $\\beta$, and $\\gamma$ values for CES resample models with final exergy ($u$).", eval=TRUE>>=
standardTriPlot(data=cesCoeffsDataU, 
                mapping=aes(color=Source), 
                facet_formula=country~nestStrParen) +
  guides(colour=guide_legend(override.aes = aes(alpha=1)))
@

Figures~\ref{fig:CES_GDP_Spaghetti_Graph_US_u}--\ref{fig:CES_GDP_Spaghetti_Graph_CN_u} 
compare CES with $u$ model predictions 
to historical data for all nestings, countries, and data sources. 
As with previous graphs, 
historical data are shown as a black line, 
the fit to historical data is shown as a white line, and 
the gray band encompasses \Sexpr{spaghettiGraphLevel*100}\% of the fits to resampled data.

<<CES_GDP_Spaghetti_Graph_US_u, fig.pos="H", fig.align='center', fig.width=5, fig.height=3.5, fig.cap="US CES with $u$ fitted models.">>=
# Use this result several times later.
cesFittedData_u <- filter(AllFitted, model=="ces", energy %in% c("iU", "none"))

cesFittedDataUS_u <- filter(cesFittedData_u, Country=="US")
spaghettiPlot(data=cesFittedDataUS_u, facet_formula=Source~nestStrParen, level=spaghettiGraphLevel) +
    labs(y="Indexed GDP (1960=1)") +
  scale_x_continuous(breaks=c(1960, 1980, 2000))
@

<<CES_GDP_Spaghetti_Graph_UK_u, fig.pos="H", fig.align='center', fig.width=5, fig.height=3.5, fig.cap="UK CES with $u$ fitted models.">>=
# Use this result several times later.
cesFittedDataUK_u <- filter(cesFittedData_u, Country=="UK")
spaghettiPlot(data=cesFittedDataUK_u, facet_formula=Source~nestStrParen, level=spaghettiGraphLevel) +
    labs(y="Indexed GDP (1960=1)") +
  scale_x_continuous(breaks=c(1960, 1980, 2000))
@

<<CES_GDP_Spaghetti_Graph_JP_u, fig.pos="H", fig.align='center', fig.width=5, fig.height=3.5, fig.cap="JP CES with $u$ fitted models.">>=
# Use this result several times later.
cesFittedDataJP_u <- filter(cesFittedData_u, Country=="JP")
spaghettiPlot(data=cesFittedDataJP_u, facet_formula=Source~nestStrParen, level=spaghettiGraphLevel) +
    labs(y="Indexed GDP (1960=1)") +
  scale_x_continuous(breaks=c(1960, 1980, 2000))
@

<<CES_GDP_Spaghetti_Graph_AT_u, fig.pos="H", fig.align='center', fig.width=5, fig.height=3.5, fig.cap="AT CES with $u$ fitted models.">>=
# Use this result several times later.
cesFittedDataAT_u <- filter(cesFittedData_u, Country=="AT")
spaghettiPlot(data=cesFittedDataAT_u, facet_formula=Source~nestStrParen, level=spaghettiGraphLevel) +
    labs(y="Indexed GDP (1960=1)") +
  scale_x_continuous(breaks=c(1960, 1980, 2000))
@

<<CES_GDP_Spaghetti_Graph_PT_u, fig.pos="H", fig.align='center', fig.width=5, fig.height=3.5, fig.cap="PT CES with $u$ fitted models.">>=
# Use this result several times later.
cesFittedDataPT_u <- filter(cesFittedData_u, Country=="PT")
spaghettiPlot(data=cesFittedDataPT_u, facet_formula=Source~nestStrParen, level=spaghettiGraphLevel) +
    labs(y="Indexed GDP (1960=1)") +
  scale_x_continuous(breaks=c(1960, 1980, 2000))
@

<<CES_GDP_Spaghetti_Graph_CN_u, fig.pos="H", fig.align='center', fig.width=5, fig.height=3.5, fig.cap="CN CES with $u$ fitted models.">>=
# Use this result several times later.
cesFittedDataCN_u <- filter(cesFittedData_u, Country=="CN")
spaghettiPlot(data=cesFittedDataCN_u, facet_formula=Source~nestStrParen, level=spaghettiGraphLevel) +
    labs(y="Indexed GDP (1960=1)") +
  scale_x_continuous(breaks=c(1960, 1980, 2000))
@

<<CES_Residual_Plot_u, fig.pos="H", fig.align='center', fig.width=6, fig.height=6, fig.cap="CES with $i$ Residual Plots.">>=
# Use residuals_df to make a graph.
ces_u_residuals_df <- filter(residuals_df, modelType=="ces", energyType %in% c("iU", "none"))
ggplot(data=ces_u_residuals_df, aes(x=year, y=resid, color=Source)) + 
  geom_point(size = 1.0) +
  facet_grid(Country ~ nestStrParen) +
  labs(x = "", y = "Residuals") +
  scale_x_continuous(breaks=c(1960, 1980, 2000)) + 
...
linexCoeffsData <- filter(AllCoeffs, model=="linex")
...
linexCoeffsData <- filter(AllCoeffs, model=="linex", country=="PT")
standardScatterPlot(data=linexCoeffsData,
                    mapping=aes(x=a_0, y=c_t, color=Source),
                    facet_formula=country~energy, alpha=0.05) +
    scale_y_continuous(limits=c(-10.0, 10.0), breaks=c(10.0, -5.0, 0.0, 5.0, 10.0)) +
...
linexFittedData <- filter(AllFitted, model=="linex")

linexFittedDataUS <- filter(linexFittedData, Country=="US")
...
linexFittedDataUK <- filter(linexFittedData, Country=="UK")
...
linexFittedDataJP <- filter(linexFittedData, Country=="JP")
...
linexFittedDataAT <- filter(linexFittedData, Country=="AT")
...
linexFittedDataPT <- filter(linexFittedData, Country=="PT")
...
linexFittedDataCN <- filter(linexFittedData, Country=="CN")
...
# Use residuals_df to make a graph.
linex_residuals_df <- filter(residuals_df, modelType=="linex")
...
Certain combinations of economy, parameter values, CES nesting structure, and data source
come with very large resampling spreads, 
indicating large fitted parameter uncertainties. 
Often, there is virtually no useful information 
...
(United States, United Kingdom, Japan, and Austria) exhibit large uncertainty. 

Some of the uncertainty in parameter estimates is explained by 
the mathematical structure of the CES model itself. 
Equations~\ref{eq:CES}--\ref{eq:CESekl} indicate that 
if $\delta$ or $\delta_1$ achieves an extreme value (0 or 1), 
the corresponding elasticity of substitution ($\sigma$ or $\sigma_1$) becomes meaningless. 
If $\delta$ approaches unity, values of $\delta_1$ and $\sigma_1$ become meaningless.
Thus, large uncertainty in $\sigma$, $\sigma_1$, and $\delta_1$ can be expected for any CES model 
...
Some of the uncertainty in parameter estimates may be explained by structural change 
in the economies over the time period under study (1960--2011). 
