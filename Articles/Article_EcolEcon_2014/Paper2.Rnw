...
\end{equation}
%
which is an estimate to the instantaneous value of $f$ 
...
Note that data spanning year~$i$ to year~$j$ is plotted at year~$i$ in 
Figures~\ref{fig:CD_lambda_time_series} and~\ref{fig:CD_solow_Perc_time_series}.
