...
test_that("cdModel() without energy fits are correct", {
  ...
  testData <-
    ...
    mutate(perfect = exp(lambda * iYear) * iK^alpha * iL^beta,
           ...
           fitGDPnoe = perfect * rlnorm(nrow(testData), 0, 1E-5)
           )
  ...
  testData <-
    ...
    mutate(
      ...
      fitGDPnoebeta = perfect * rlnorm(rlnorm(nrow(testData), 0, 1E-5))
    )
...
test_that("cdModel() without energy is correct at edges", {
  ...
  testData <-
    testData %>%
    mutate(fitGDPnoe_alpha1 = exp(lambda * iYear) * iK^alpha * iL^beta)
  ...
  testData <-
    testData %>%
    mutate(fitGDPnoe_beta1 = exp(lambda * iYear) * iK^alpha * iL^beta)
...
test_that("cdModel() with energy fits are correct", {
  ...
  testData <-
    EconUK %>%
    mutate(fitGDPe = exp(lambda * iYear) * iK^alpha * iL^beta * iXu^gamma)
  ...
  testData <-
    testData %>%
    mutate(fitGDPalpha0 = exp(lambda * iYear) * iK^alpha * iL^beta * iXu^gamma)
  ...
  testData <-
    testData %>%
    mutate(fitGDPbeta0 = exp(lambda * iYear) * iK^alpha * iL^beta * iXu^gamma)
  ...
  testData <-
    testData %>%
    mutate(fitGDPgamma0 = exp(lambda * iYear) * iK^alpha * iL^beta * iXu^gamma)
  ...
  testData <-
    testData %>%
    mutate(fitGDPalpha1 = exp(lambda * iYear) * iK^alpha * iL^beta * iXu^gamma)
  ...
  gamma <- -0.1
  testData <- testData %>%
    mutate(fitGDPbeta1 = exp(lambda * iYear) * iK^alpha * iL^beta * iXu^gamma)
  ...
  testData <-
    testData %>%
    mutate(fitGDPgamma1 = exp(lambda * iYear) * iK^alpha * iL^beta * iXu^gamma)
...
test_that("linexModel() fits are correct", {
  ...
  testData <-
    ...
    mutate(rho_l = iL / iXu,
           rho_k = iK / (0.5 * (iL + iXu)),
           fitGDP = iXu * exp(a_0 * (2 * (1 - 1 / rho_k) + c_t * (rho_l - 1)))
    )
...
test_that("sfModel() fits are correct", {
  ...
  testData <-
    ...
    mutate( fitGDP =  exp(lambda * iYear) * iK^m,
            fitGDPU = exp(lambda * iYear) * iXu^m,
