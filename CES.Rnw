...
modelUKQ_LM <- cesEst(data=dataUK, yName="iGDP", xNames=c("iCapStk", "iLabor", "iQ"), tName="iYear")
summary(modelUKQ_LM)
@

In the above example, we obtain $\rho_1 = $ \Sexpr{coef(modelUKQ_LM)["rho_1"]} 
which violates the constraint $\rho_1 \in (-1,0) \cup (0,\infty)$.
