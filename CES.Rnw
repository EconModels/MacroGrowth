...
\usepackage{amssymb} % To provide check marks
...
<<>>=
extractRSSdata <- function(object, ...) {
  res <- expand.grid( object$rho1Values, object$rhoValues )
  names(res) <- c('rho1', 'rho')
  res <- transform(res, RSS = as.vector(object$rssArray))
  res
}
@
...
modelUKQ_LM <- cesEstPlus(data=dataUK, 
                      yName="iGDP", 
...
However, we should note that we are not respecting the constraints here. 
To assess whether the models are actually telling us 
something different, we need to be sure to respect constraints
with the CES model.
...
for the CES model, namely ($le$)($k$), as shown in Equation~\ref{eq:ceslek}.
...
In Equation~\ref{eq:ceslek}, $\delta \rightarrow 1$ indicates that capital stock ($k$)
is unnecessary.

<<>>=
modelUKQ_LM2 <- cesEstPlus(data=dataUK, 
                      yName="iGDP", 
...
that capital stock ($k$) is unnecessary, thereby agreeing with 
...
However, we need to be careful about convergence and respecting 
constraints.
...
<<>>=
modelUKQ_LBFGSB <- cesEstPlus(data=dataUK, 
                          yName="iGDP", 
                          xNames=c("iCapStk", "iLabor", "iQ"), 
                          tName="iYear",
                          algorithm="L-BFGS-B")
...
capability of \texttt{cesEstPlus}. Grid search is accessed by specifying
...
<<>>=
modelUKQ_grid <- cesEstPlus(data=dataUK, 
                        yName="iGDP", 
                        xNames=c("iCapStk", "iLabor", "iQ"), 
                        tName="iYear",
                        algorithm="L-BFGS-B",
...
<<>>=
modelUKQ_grid_detail <- cesEstPlus(data=dataUK, 
                        yName="iGDP", 
                        xNames=c("iCapStk", "iLabor", "iQ"), 
                        tName="iYear",
                        algorithm="L-BFGS-B",
                        rho=seq(from=100, to=120, length.out=20), 
                        rho1=seq(from=1, to=2, length.out=20))
summary(modelUKQ_grid_detail)
levelplot( RSS ~ rho1 + rho, data=extractRSSdata(modelUKQ_grid_detail), 
           region=TRUE, 
...
<<>>=
modelUK <- cesEstPlus(data=dataUK, 
                        yName="iGDP", 
...
obtained with standard nesting ($kl$)$e$ vs. a differnet nesting, namely
($le$)$k$. To explore that further, we fit all 9 countries with 
...
We use the \texttt{L-BFGS-B} method (as shown in Section~\ref{sec:CES-with-constraints}),
which respects constraints on the model parameters, and thermal energy ($q$) is 
used for the energy quantification.

<<>>=
nestingOptions <- c(kle="(kl)e", lek="(le)k", ekl="(ek)l")
fitCES <- function(countryAbbrev, nest, energyType="Q", algorithm=c("PORT","L-BFGS-B")){
  ...
  model <- cesModel(data=data, energyType=energyType, 
                  ...
                  # tName="iYear",
                  algorithm=algorithm)
  res <- naturalCoef( model ) 
  res <- transform( res, nest=nest, country=countryAbbrev, nest=nest, converge=model$convergence ) 
  return(res)
}
...
# Ignore the leading .id column
allModels <- subset( allModels, select= -.id )
allModels <- transform( allModels, max.sse = pmax(sse.PORT, sse.L.BFGS.B))
# Sort rows based on country
allModels_ordered <- allModels[order(allModels$country) , ] 
# Reorder the columns to put the meta information first
allModels_ordered <- allModels_ordered[c("country", "nest", "converge", "gamma", "lambda", 
                                         "delta_1", "delta", "rho_1", "rho", "algorithm", "sse","max.sse")]
# Create the LaTeX table
CESTable <- xtable(allModels_ordered, caption="CES coefficients for three nesting options.", 
                   label="tab:CES-coeffs-table", digits=3,
                   table.placement="H")
@

<<CESResultsTable, results='asis'>>=
print(CESTable,
      caption.placement="top", 
      table.placement="H",
      include.rownames=FALSE,
      hline.after=c(-1,0,3,6,9,12,15,18,21,24,nrow(CESTable)),
      digits=3
      )
@

One way to assess the data in Table~\ref{tab:CES-coeffs-table} is to compare indications 
from the Cobb-Douglas and CES models. 
Table~\ref{tab:cd-ces-comparison} does so.

\begin{table}[H] \caption{Comparison of indications from Cobb-Douglas and CES models with energy.} \label{tab:cd-ces-comparison} 
  \begin{center}
    \begin{tabular}{c|l|l|c}
Country & CD                                     & CES                                            & Agree? \\
\hline
US      & $\gamma = 0 \rightarrow e$ unnecessary & ($kl$)$e$, ($le$)$k \rightarrow e$ unnecessary & \checkmark \\
\hline
UK      & $\alpha = 0 \rightarrow k$ unnecessary & ($kl$)$e$, ($le$)$k \rightarrow k$ unnecessary & \checkmark \\
\hline
JP      & $\gamma = 0 \rightarrow e$ unnecessary & ($ek$)$l \rightarrow e$ unnecessary            & \checkmark $\:$ but no CES convergence\\
\hline
CN      & $\gamma = 0 \rightarrow e$ unnecessary & No indications                                 & Note: no CES convergence\\
\hline
ZA      & $\beta  = 0 \rightarrow l$ unnecessary & No indications                                 &      \\
\hline
SA      & No indications                         & No indications                                 & \checkmark \\
\hline
IR      & $\gamma = 0 \rightarrow e$ unnecessary & All nestings $\rightarrow e$ unnecessary       & \checkmark \\
\hline
TZ      & $\beta  = 0 \rightarrow l$ unnecessary & No indications                                 &      \\
\hline
\multirow{2}{*}{ZM} & $\beta  = 0 \rightarrow l$ unnecessary & \multirow{2}{*}{All nestings $\rightarrow l$ unnecessary} & \multirow{2}{*}{-- (partial)}\\
        & $\gamma = 0 \rightarrow e$ unnecessary &                                                &        \\

    \end{tabular}
  \end{center}
\end{table}

Table~\ref{tab:cd-ces-comparison} looks better than I was expecting. 
Note that in Section~\ref{sec:CES-without-constraints},
I was not assessing the indications from the CES model 
\emph{while also respecting the constraints}. 
When the constraints are respected, both Cobb-Douglas and CES
give similar indications for most economies.

I suspect that clearing up the non-convergence issues 
for the UK, JP, and CN will provide additional insights.
Martin may be able to provide additional insight on the South African (ZA)
and Tanzanian (TZ) situations, as ZA and TZ are the only countries with both 
(a) dissimilar indications from Cobb-Douglas and CES and 
(b) convergence of the CES models for all nestings.
