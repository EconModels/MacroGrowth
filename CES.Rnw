...
# Make all the models
kleModels <- lapply(countryAbbrevs, fitCES, nest="(kl)e")
lekModels <- lapply(countryAbbrevs, fitCES, nest="(le)k")
eklModels <- lapply(countryAbbrevs, fitCES, nest="(ek)l")

# Now, extract coefficients to make a table
coef(kleModels["US"])
@
