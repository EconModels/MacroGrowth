...
This approach correctly respects the constraints, 
...
<<>>=
modelUKQ_grid <- cesEst(data=dataUK, yName="iGDP", xNames=c("iCapStk", "iLabor", "iQ"), tName="iYear")
modelUKQ_grid <- cesEst(data=dataUK, 
                        yName="iGDP", 
                        xNames=c("iCapStk", "iLabor", "iQ"), 
                        tName="iYear",
                        method="L-BFGS-B",
                        rho=seq(from=1, to=20, by=2), 
                        rho1=seq(from=-0.5, to=2, by=0.5))
