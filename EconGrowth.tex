...
\hlcomment{  # * gamma = 1 - max(a, b)}
  ...
  modelCDe <- \hlfunctioncall{nls}(iGDP ~ \hlfunctioncall{exp}(lambda*iYear) * 
                  ...
                  data = dataTable)
  aicCDe <- \hlfunctioncall{AIC}(modelCDe, k=2); aicCDe \hlcomment{# Checks validity of the model. AIC stands for Akaike's Information Criterion}
  
  summaryCDe <- \hlfunctioncall{summary}(modelCDe); summaryCDe \hlcomment{# Gives the nls summary table}
  
\hlcomment{  # Provides confidence intervals on lambda, a, and b. But, we need CIs on alpha and beta.}
  ciCDe <- \hlfunctioncall{confint}(modelCDe, level = 0.95); ciCDe
...
\hlcomment{  # Report results with SE}
  beta.est <- \hlfunctioncall{deltaMethod}(modelCDe, \hlstring{"b-a"}); beta.est # Reports results for beta, because beta = b - a.
  gamma.est <- \hlfunctioncall{deltaMethod}(modelCDe, \hlstring{"1-b"}); gamma.est # Reports results for gamma, because gamma = 1 - b.
  
\hlcomment{  # Now calculate confidence intervals.}
  dofCDe <- summaryCDe$df[2]; dofCDe \hlcomment{# Gives the degrees of freedom for the model.}
  tvalCDe <- \hlfunctioncall{qt}(0.975, df = dofCDe); tvalCDe
  betaCICDe <- \hlfunctioncall{with}(beta.est, Estimate + \hlfunctioncall{c}(-1.0, 1.0) * tvalCDe * SE); betaCICDe \hlcomment{# Gives the confidence interval on beta.}
  gammaCICDe <- \hlfunctioncall{with}(gamma.est, Estimate + \hlfunctioncall{c}(-1.0, 1.0) * tvalCDe * SE); gammaCICDe \hlcomment{# Gives the confidence interval on alpha.}
  
\hlcomment{  # Combine all estimates and their confidence intervals into data frames with intelligent row names}
  estCDe <- \hlfunctioncall{data.frame}(lambda = lambda, alpha = alpha, beta = beta, gamma = gamma); estCDe
\hlcomment{  #row.names(estCDe) <- "Cobb-Douglas with e: $y = e^\{\textbackslash{}\textbackslash{}lambda t\}k^\{\textbackslash{}\textbackslash{}alpha\}l^\{\textbackslash{}\textbackslash{}beta\}e^\{\textbackslash{}\textbackslash{}gamma\}$"}
  \hlfunctioncall{row.names}(estCDe) <- \hlstring{"CobbDouglas with e"}
\hlcomment{  # The [1] subscripts pick off the lower confidence interval}
...
\hlcomment{  # Now create the data for a table.}
  dataCDe <- \hlfunctioncall{rbind}(lowerCDe, estCDe, upperCDe); dataCDe
