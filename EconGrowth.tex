...
createCountryInputGraph <- \hlfunctioncall{function}(countryName)\{
  ...
  graph <- \hlfunctioncall{xyplot}(iGDP+iQ+iX+iU ~ iYear, data=dataTable,
                  type=\hlstring{"l"},
                  scales=\hlfunctioncall{list}(cex=1.5, tck=-1),
                  ylab=\hlstring{"\hlfunctioncall{Indexed} (1980=1)"},
                  auto.key=\hlfunctioncall{list}(columns=1))
...
\end{alltt}
\end{kframe}
\includegraphics[width=\maxwidth]{figure/creategraph} 
